...
the BurnerMap founder's honest read.

- **Typing to the Angel works like a messenger**: the input and Send stay
  visible above the keyboard (they used to hide behind it — measured and
  fixed twice until the geometry admitted no double-counting).
...
- **App-first framing everywhere**: the site and README now say plainly
  that every tool works with no AI installed — the Angel is the optional
  layer. Copy de-flourished after real-user feedback.
