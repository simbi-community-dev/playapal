...
Playa Pal was built in twelve days before Burning Man 2026 — the first
commit landed Aug 13 and 0.8.0 shipped Aug 24 — and its versions map to the
real eras of that history: each tenth is a thing the app couldn't do the day
before. 1.0 is earned when Black Rock City itself has field-tested it.
...
when mail is waiting — the unread count rides the chip — and on People
otherwise.
...
— silently stopping the radio you had just started and handing its
...
re-assert briefly ducked whatever was playing — a once-a-second breathing
of the volume, heard in the field. The session is now re-asserted only
when it is measurably wrong, and the same gate learned to notice a lost
loudspeaker route — the mystery of old messages playing quietly out of
...
— its own high-priority channel, on by default, with one Settings row
that jumps straight to the phone's own notification controls. No servers
...
the channel is dropped, on both platforms, and the counters say so — a
lo-fi rung being honest about its bandwidth instead of lying about time.
...
linked directly carry these calls with no Wi-Fi at all — field-measured
this week, "super high quality" — and the help now says so.
...
notification for a press that was already over (now it doesn't — a held
press with a silent mic is still told the truth). The Android updater
...
**The iPhone talk button now proves the microphone is alive — and heals
it when it isn't.** 0.8.4 cured how the audio engine is *born*; the last
...
survives that looking healthy while its microphone half is quietly dead —
...
full quality with no Wi-Fi at all — the same no-network trick the
Androids already do together. Cross-platform without Wi-Fi stays
...
silently left out of every build to date — found, fixed, and pinned so
it can never happen quietly again.

**Curiosity gets a tap.** The paragraphs of tiny print that sat on every
screen moved behind a calm circled **?** — one tap opens the full
...
something is happening now. And Settings grew a **text size** dial —
four steps, applied everywhere instantly, for sun-blind eyes and
left-in-the-tent reading glasses.

**Voice notes grow up.** A graze of the record button no longer mints an
empty voicemail — a note needs a full second of holding, and a shorter
press says so and discards cleanly. Received notes became a full-width
...
surgery closed four race conditions before they could bite — including
one that could have rotated a healthy connection out from under a
podmate on a false reading, and one crash reachable from network input.

**The walkie stops flapping.** With no Wi-Fi anywhere — the playa's
...
reads **(quiet)** — not (lo-fi), which promised live audio over a link
that had none — with one sentence saying your voice may not be reaching
them while messages still go. Names dropping off the channel and coming
back gets called what it is: the dust, not a fault. A call whose invites
never left your phone ends with "Couldn't get through — their link went
...
talk button. The walkie hint now leads with what to expect — some pairs
of phones link at full quality, some always come through rougher, both
carry your voice — and a podmate you can talk to but not video-call gets
...
surface — README, site, in-app help, the call's own ending — now says so
...
iPhone's audio engine dead in a way it never noticed — press-to-talk
read "no audio" and incoming voice played nothing while the
...
network — a camp router or somebody's hotspot — and on the playa that is
the thing you most often don't have. Now the pod finds its own way
...
rides the walkie's own frames, codec 0x6 — older builds drop it
silently) — ring, answer
or decline, hang up from either side, mute the camera, flip it. There is
...
walk the map, and a podmate calling you rings right there — their name,
Answer, Not now. Answering opens the call where you stand. Closing the
...
is on, who is on the channel with you, and turns it off in one tap — a
walkie quietly eating your battery where you can't see it is not something
...
reach is actually good for — *voice*, *lo-fi voice*, *messages now*,
*when you pass by*, *notes keep*. It speaks in what you can do, never in
...
The only thing that ever switched the radio on was *Share my position* —
so two phones sitting a foot apart, both apps open, with that switch off,
said nothing to each other at all. Not slowly: not at all. Messages and
...
nobody can see where you are — not that your message is going nowhere.
...
between looks. Both halves now hold the same, much shorter pause — with
the same guard against a crowded camp turning that into a storm.
...
from an older phone is labelled honestly — no play button, no duration,
the reason on the row and read aloud with it — and tapping it says *ask
them to send it again* instead of playing silence.
...
*read* a code — one photo, inside Playa Pal, no trip out to the camera app
and back. A scanned card installs through the same importer a tapped link
...
histories — the Temple, Rangers, art cars, gifting, how Black Rock City
really works — each one crediting the video it came from and pointing you
at the original. Culture and history questions had nowhere to land
...
audio buffers were the last straw — crashes through setup and hard at
...
deterministic answering — event authority, exact-title routing, temporal
clarification, camp history — merged into one, then repaired under three
...
Man history goes to the guide, not the schedule — and an imperative
...
**Pods.** Pick your people and your phones keep track of each other — no
signal, no server, nothing typed twice. Start a pod or join one with its
code — new codes are 4-digit PINs ("4207", typeable in gloves), and pods
...
far. Runs on Bluetooth between phones — an Android keeps sharing from
...
away. Now the pod's name and its member roster gossip over the mesh —
...
**The answering machine.** Leave the pod a message — typed, or hold the
mic and talk. It rides from phone to phone whenever podmates pass in
...
**A pod you can hand over.** A pod used to be four digits read aloud —
the only shareable thing in the app you couldn't hand over by pointing a
camera at a screen. Now it has a QR and a link, and both carry the pod's
...
Wi-Fi — a hotspot with zero internet is enough. The mic runs only while
...
podmate's shows a different number you know the truth — two routers
...
Settings — the pod one tap from anywhere, with an unread badge that
counts only mail that actually arrived. Camp regrouped behind four
labelled panes — Board, Share, Knowledge, Friends — on a strip that
...
chip, scrim and map ink got both palettes — and a test now fails the
build if a future color forgets to.
...
the first time — groundwork, honestly labelled: the matching embedder
...
scatter was mostly right — a share control belongs beside the thing it
...
only in color — the consent chips, the QR mode picker — now carry a mark
...
table, not a password. None of this touches the internet. iPhones share while the app is open — a plugged-in
Android carries the quiet hours.
...
events list, so a placed, registered camp that hosts no public events —
spotted in the field within the hour — wasn't offered. The picker now
