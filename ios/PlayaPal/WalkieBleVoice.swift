...
final class WalkieBleVoice: NSObject {
  ...
  /// nothing that is working: no connected peripheral is cancelled, no
  /// published service is removed, and a peer that already has a proven
  /// voice pipe keeps it. What it changes is the scan (a CoreBluetooth
  ...
  func refresh() {
    queue.async { [self] in
      ...
      for p in voicePeers.values where !p.ready && !p.connecting {
  ...
  private func handleIdent(_ peer: VoicePeer, _ value: Data?) {
    ...
    if peer.hash == Self.unknownSender {
      ...
      // the peripheral's stable identifier for the scan's damper, then
      // re-key — unless that identity is already held by a live entry,
      // in which case this dial was the second road to a reached phone.
      if let id = peer.peripheral?.identifier {
...
extension WalkieBleVoice: CBCentralManagerDelegate {
  ...
  func centralManager(
    ...
    if hash == Self.unknownSender,
       let known = provenIdentity[peripheral.identifier],
       let p = voicePeers[known], p.ready || p.connecting {
