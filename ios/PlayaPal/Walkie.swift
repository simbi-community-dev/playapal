...
final class Walkie: RCTEventEmitter {
  ...
  /**
   LIVE-NOT-LATE (field-measured 2026-08-26, three phones on the BLE lo-fi
   rung): P7 keyed, an iPhone lit "someone is talking" at once and then
   emitted the AUDIO TEN SECONDS LATE. The transport was fine. The receiver
   was the bug — when the pipe cannot hold real-time pace, a receiver that
   schedules EVERY frame it gets simply walks its own playback further and
   further into the past, and the drift is monotonic: it never catches up on
   its own, because nothing in a stream of 20 ms frames ever arrives early.

   So this is the deepest backlog a receiver will hold. A frame that would
   land more than this far ahead of what has actually PLAYED means the queue
   is FULL — and a full queue is emptied from the front, so the newest frame
   is the one that plays and the stale tail is what goes (see flushStaleTail
   for which end goes and why). Evicting is the rung being honest about its
   bandwidth, exactly like the lo-fi badge; playing ten-second-old voice is a
   lie about liveness, and a walkie that lies about liveness is worse than
   silence on the playa — you answer a question nobody is still asking.

   400 ms is chosen as "a beat behind, not a conversation behind": it
   absorbs a Wi-Fi retry burst or a 60 ms GATT lane hiccup without ever
   letting a listener fall out of turn-taking. Mirrored in
   WalkieModule.kt (MAX_LEAD_MS) and pinned by a test that reads both
   files — one number, two languages.
   */
  private static let maxLeadMs = 400
  /// The bound in the unit the player actually counts in. DERIVED, never a
  /// second literal: a bound that can drift from its own milliseconds is
  /// two bounds.
  private static let maxLeadFrames = Int(sampleRate) * maxLeadMs / 1000
  /// How often an eviction run is allowed to say so. The log line is for
  /// the next bench — it names the rung's real ceiling — so it must be
  /// readable, and 50 frames a second would drown the console it prints to.
  private static let lateLogSeconds: TimeInterval = 3
  ...
  /// THE ARBITER'S STATE STREAM, and a deliberately NEW name.
  ///
  /// `WalkieAdvertiserSettled` was an EDGE wearing four fields, and the
  /// capability policy (S9) turns its absence into a fact rather than a
  /// silence: a build that emits the old name emits a body this JS cannot
  /// read, and the honest answer to that is INCOMPATIBLE — park the hold
  /// with a reason — never "an event I will keep waiting for". Renaming
  /// is what makes the two eras impossible to confuse: nothing decodes
  /// half of one as the other.
  private static let airtimeEvent = "WalkieAirtimeState"
  ...
  /// MONOTONIC, not wall-clock (codex cross-family review, 2026-08-26):
  /// a backward clock correction — the exact thing drifted playa phones
  /// receive — would stretch a Date()-based throttle window arbitrarily,
  /// and with it the post-call heal latency this throttle bounds.
  /// systemUptime never runs backward.
  private var lastSessionAssert: TimeInterval = 0
  /// False after a failed re-assert, so the next frame retries at the
  /// 1 s floor instead of the failure reading as healthy steady state.
  private var sessionAssertHealthy = true
  /**
   SCHEDULED-BUT-NOT-YET-PLAYED frames — the LIVE-NOT-LATE measurement (see
   maxLeadMs), and the reason it is kept by hand: AVAudioPlayerNode has no
   queue-depth getter at all. Nothing asks it how much voice is still
   waiting to come out, so we count both ends ourselves — up when a buffer
   is scheduled, down in its completion handler.

   Completion handlers fire on WHATEVER thread the render path finishes on,
   and frames are admitted from the network queues, so this is exactly the
   written-here/read-there shape the flags above are: its own small lock,
   held just long enough to add or subtract. Not the engineLock — a
   completion handler must never wait behind an engine rebuild.
   */
  private var pendingFrames = 0
  /**
   WHICH QUEUE THAT DEPTH IS COUNTING (defect C, codex cross-family read on
   0ad0149). Bumped by resetPending, and stamped onto every claim.

   The count has two ends and they live on different threads: a claim is
   added when a buffer is scheduled and subtracted in that buffer's
   completion handler. A flush (or any teardown) discards the whole queue
   and zeroes the count — and then the completion handlers of the buffers
   it discarded fire, on the render thread, whenever the render path gets
   to them. Untagged, each of those late subtractions is charged to the
   claims of the NEW queue that has meanwhile been admitted. The depth
   drifts DOWNWARD, reads as a channel with room it does not have, and the
   400 ms guard quietly stops firing — the exact unbounded drift this whole
   seam exists to stop, arriving through the seam's own cleanup.

   So a release only counts against the generation its claim was admitted
   under. A completion from a queue that has since been thrown away is a
   no-op, which is the honest answer: those frames were already discarded
   wholesale by the reset that bumped this number.
   */
  private var pendingGen = 0
  /// How many times the lateness guard has emptied the player from the
  /// front this session, and when it last said so out loud. Observable on
  /// purpose: a bench that cannot name this rate cannot tell a rung at its
  /// ceiling from a rung that is broken. It counts EVICTIONS, not frames —
  /// one flush discards up to a whole maxLeadMs of stale tail, which is a
  /// coarser and more honest unit than "frames refused" ever was.
  private var staleFlushes = 0
  private var lastLateLog = Date.distantPast
  ...
  override func supportedEvents() -> [String]! {
    [Self.peersEvent, Self.speakingEvent, Self.signalEvent, Self.airtimeEvent]
  }

  // ------------------------------------------- this module's airtime seat
  //
  // WHAT MOVED, AND WHY THIS BLOCK IS NOW SHORT (cross-family ARCHITECTURE
  // ruling, 2026-08-27, read of a24b1e2).
  //
  // a24b1e2 put the airtime state HERE: a static Bool for "a session is
  // live", a static epoch beside it, and a snapshot function that read the
  // debt book under one lock order. Every field of it was correct and the
  // shape was still wrong, for one reason the ruling names exactly — a
  // Bool cannot say WHICH session, so a second bridge instance's stop
  // cleared the first one's, and a second start could not be told from an
  // overlap. `sessionLive` is an ANONYMOUS hold, and an anonymous hold is
  // one that anybody may end.
  //
  // The state, the identity and the decision all live in
  // WalkieAirtimeArbiter now (WalkieBleVoice.swift, beside the debt book
  // it subsumes). This module is one of its CLIENTS: it reserves a lease,
  // it hands over the operation handle that can prove its own advertiser
  // down, it forwards the arbiter's state to JS, and it holds nothing that
  // could answer "who owns the air?" on its own. That last property is the
  // point — a module that cannot answer the question cannot answer it
  // WRONG after a reload.

  /// THE LEASE THIS INSTANCE HOLDS, and the whole of S1. Not a Bool: an
  /// exact, opaque identity minted by the arbiter. A second instance of
  /// this class (a JS reload builds one) has its own field, holding nil,
  /// and its stop therefore answers `notOwner` instead of clearing a hold
  /// it never took.
  private var airtimeLease: String?

  /// This bridge's own state sink, by exact token (S3). Registered when JS
  /// starts listening and removed when it stops or the bridge is
  /// invalidated — and removing it touches NO lease, because the hold
  /// belongs to the process and a bridge going away is not a radio going
  /// quiet.
  private var airtimeSink: String?

  /**
   THE RESERVATION, THE SUPPRESSION PROOF, AND ONLY THEN THE ADVERTISER.

   Three steps, in this order, and each one is a gate on the next:

     1. RESERVE. The arbiter mints a lease under its own lock. A refusal
        (another lease standing, or a book that will not come clean) is a
        session with NO RUNG 3, which is the ladder's own fail-soft answer
        — never a failed start, and never a second advertiser.
     2. PROVE THE CREW BEACON QUIET (S2). Asking it to stop is not proof;
        the arbiter drives the sink and reads the effect. Unproven means
        this lease is DEGRADED and no walkie advertiser is minted at all —
        because the whole cost of getting this wrong is that every Android
        in the pod loses this iPhone, and a session with no rung 3 is
        strictly cheaper than that.
     3. MINT, AND HAND OVER THE OPERATION HANDLE. `armStart` retains the
        closure that can prove THIS advertiser down, on the LEASE rather
        than on this instance — which is what lets a duplicate stop find a
        live operation instead of a nilled field.
   */
  private func reserveAirtime(podHash: UInt32, senderHash: UInt32, displayName: String) {
    Self.arbiter.reserve { [weak self] leaseId, why in
      guard let self, let leaseId else {
        NSLog("walkie//airtime-refused why=%@", why)
        return
      }
      DispatchQueue.main.async {
        // The listener is the session's own liveness: a walkie that closed
        // while the reservation was in flight has no rung to mint.
        guard self.listener != nil else {
          Self.arbiter.stop(lease: leaseId) { _, _, _ in }
          return
        }
        self.airtimeLease = leaseId
        Self.arbiter.suppressCrew(lease: leaseId) { [weak self] proven, suppressWhy in
          guard let self else { return }
          guard proven else {
            NSLog("walkie//airtime-degraded why=%@", suppressWhy)
            return
          }
          DispatchQueue.main.async {
            self.startBleVoice(
  ...
  /// carry choppy voice over a GATT pipe; peers arrive under "ble|" keys
  /// and the roster dedupe keeps a podmate who is also on a hi-fi rung on
  /// that rung instead. Every closure hops to main — this file's "main
  /// owns peers" discipline — and every BLE failure contributes no peers.
  ///
  /// REACHED ONLY THROUGH THE ARBITER, and that is the change: this
  /// advertiser exists because a lease authorised it and because the crew
  /// beacon was PROVEN off the air, not because a start ran.
  private func startBleVoice(
    lease leaseId: String,
    podHash: UInt32,
    senderHash: UInt32,
    displayName: String
  ) {
    guard listener != nil, bleVoice == nil else { return }
    ...
    ble.airtimeLease = leaseId
    bleVoice = ble
    // THE OPERATION HANDLE, RETAINED ON THE LEASE. `ble` is captured
    // strongly on purpose: the whole failure this replaces is a stop that
    // nilled the module's field and left the next caller with nothing to
    // interrogate. The lease outlives this instance, so the handle does.
    Self.arbiter.armStart(lease: leaseId, stopper: { settle in
      ble.stop { down, why in settle(down, why) }
    }) { rung, why in
      NSLog("walkie//airtime-rung %@ why=%@", rung.rawValue, why)
    }
    ble.start()
  }

  /// The one arbiter, spelled once.
  private static var arbiter: WalkieAirtimeArbiter { WalkieAirtimeArbiter.shared }

  /// THE SINK REGISTERS WHEN JS IS LISTENING, and its token is exact.
  /// `addSink` REPLAYS the current state on registration, which is what
  /// makes a missed event safe (S3): a subscriber that attached one turn
  /// after a transition is not one transition behind.
  override func startObserving() {
    airtimeSink = Self.arbiter.addSink { [weak self] body in
      self?.sendEvent(withName: Self.airtimeEvent, body: body)
    }
  }

  override func stopObserving() {
    Self.arbiter.removeSink(airtimeSink)
    airtimeSink = nil
  }

  /// THE BRIDGE GOES AWAY; THE RADIO DOES NOT. Invalidate removes exactly
  /// this bridge's own sink and touches NO lease, in every phase — a
  /// reload that handed the advertising slot back by disconnecting would
  /// be the strand and the overlap in one gesture.
  ...
  func start(
    ...
    do {
      ...
      l.start(queue: .main)
      listener = l
      // THE AIRTIME IS RESERVED, NOT ANNOUNCED (ARCHITECTURE ruling,
      // 2026-08-27). This used to set a static Bool and move on — an
      // ANONYMOUS claim that any later stop, from any instance, could
      // clear. It now ASKS, and what comes back is an exact lease this
      // instance holds and nobody else can end.
      //
      // ATOMIC AND ON THIS TURN: the arbiter mints under its own lock and
      // answers here, so there is no window in which this session has
      // asked for the slot and holds no identity for it — which is the
      // window a stop landing between the two would read as `notOwner`.
      //
      // A REFUSAL IS A SESSION WITH NO RUNG 3, never a failed start: the
      // LAN rung below is already coming up and the ladder's own law is
      // that a lower rung's failure contributes no peers and nothing else
      // (docs/WALKIE-LADDER.md §1).
      reserveAirtime(podHash: podHash, senderHash: senderHash, displayName: displayName)
      ...
      // (rung 3's advertiser is minted by reserveAirtime, once the
      // arbiter has granted a lease AND proven the crew beacon quiet.)
      ...
      // THE START'S OWN WRECK OWNS ITS LEASE TOO. A reservation minted a
      // line above this is a hold, and a hold nobody ends is the strand —
      // so the failed start goes through the same barrier a close does,
      // and the arbiter decides whether the slot comes back or moves to
      // the debt phase. The camper's own error is still what surfaces.
      let held = airtimeLease
      airtimeLease = nil
      stopInternal()
      Self.arbiter.stop(lease: held) { outcome, why, _ in
        NSLog("walkie//failed-start airtime=%@ why=%@", outcome.rawValue, why)
      }
  ...
  /**
   File the Bonjour browse and remember it. Its own function because two
   callers need it: start(), and refreshDiscovery() when the browser has
   died — a cancelled or failed NWBrowser has no "ask again", only a
   replacement.
   */
  private func startBrowsing() {
    ...
    b.browseResultsChangedHandler = { [weak self] results, _ in
      ...
      // A ROW THIS BROWSER DID NOT MINT IS NOT A ROW IT MAY RETIRE. The
      // other rungs live in this same table under "aware|" and "ble|"
      // keys and are not Bonjour results at all, so every one of them is
      // carried across the rebuild. This used to say `hasPrefix("aware|")`,
      // which meant every LAN peer arriving or leaving silently emptied
      // rung 3: those rows are minted ONCE, when the GATT pipe proves
      // itself, and nothing re-announces a peer that is still connected —
      // so a lo-fi podmate dropped off this iPhone's channel at the next
      // browse callback and did not come back until the link itself did.
      // The rule is now the key's SHAPE rather than one rung's name, so
      // the next rung to arrive is carried without anyone remembering to.
      for (name, peer) in self.peers where !name.hasPrefix("pp|") {
        next[name] = peer
      }
  ...
  /**
   "LOOK AGAIN" — the walkie panel's refresh control (owner ask,
   2026-08-26: the channel sometimes flaps and autodiscovery is opaque).
   Mirrors WalkieModule.refreshDiscovery.

   IT ASKS THE RADIOS TO LOOK; IT DOES NOT RESTART THE WALKIE. The
   listener, every live connection, the audio engine and the keep-alive
   are untouched — a camper taps this while the channel is misbehaving,
   which is exactly when somebody may be mid-sentence on it.

   The Bonjour browser is replaced ONLY when it is down, and that
   restraint is the load-bearing part. A browse result set IS this
   platform's membership for the LAN rung, and a fresh browser reports an
   empty world until its first results arrive — so rebuilding a HEALTHY
   browser would cancel every LAN peer's connection and re-add them a
   moment later, which is the flap this control exists to answer, caused
   by the control. A browser that has failed or been cancelled has no rows
   left to lose, so there it is replaced.

   Resolves either way: a refresh that could fail would be a control
   showing the camper an error about a condition they cannot act on.
   */
  @objc(refreshDiscovery:rejecter:)
  func refreshDiscovery(
    _ resolve: @escaping RCTPromiseResolveBlock,
    rejecter _: @escaping RCTPromiseRejectBlock
  ) {
    guard listener != nil else {
      resolve(nil) // the walkie is off; there is nothing to look with
      return
    }
    var browserDown = true
    if let b = browser {
      switch b.state {
      case .failed, .cancelled:
        browserDown = true
      default:
        // .setup, .ready and .waiting are all a browser that is still
        // looking (waiting re-arms itself when the interface returns), and
        // a browser that is still looking keeps its rows.
        browserDown = false
      }
    }
    if browserDown {
  ...
  /**
   THE STOP IS A BARRIER (ARCHITECTURE ruling, 2026-08-27).

     "Stop/endSession is a barrier: cancels ambient+sync clients, retires
     services/digest/events, and resolves only after exact owner release
     or debt transfer. Unknown is never clear."

   TWO HALVES, AND THIS IS THE SECOND. The module teardown below retires
   everything this module owns that nobody is sequenced behind — the
   listener, the browser, the aware link, the peer table, the audio graph.
   This promise is the half anybody IS sequenced behind, and it does not
   resolve until the arbiter has reached one of exactly two terminals for
   THIS lease:

     clear     the exact owner's advertiser is proven off the air and the
               process owes nothing. The arbiter has already handed the
               slot back to CrewBeacon, itself, at the effect.
     debt      the proof could not arrive, so the advertiser is on the
               book and the lease is DEMOTED. The owner is released; the
               hold is not.

   …plus the one answer that is about the CALLER rather than the radio:

     notOwner  this instance never held this lease. Nothing was released
               and nothing was proven, and saying so is the whole of S1 —
               a second bridge instance's stop cannot end the first one's
               hold, and before this it could, because the hold was a
               static Bool with no name on it.

   IT RESOLVES, IT DOES NOT REJECT. The old shape rejected with code
   "advertiser" and JS read that one code as the fail-closed fact — which
   worked, and which made every OTHER rejection a generic error that some
   future reader could mistake for a close. A structured outcome cannot be
   mistaken: there is one field, it has four values, and the boundary's
   own contract is that only `clear` may be read as a close (S7).
   */
  @objc(stop:rejecter:)
  func stop(
    _ resolve: @escaping RCTPromiseResolveBlock,
    ...
    // THE IDENTITY OUTLIVES THE ISSUE, and that is the duplicate-stop
    // cure at this level. Clearing the field on the way IN is what made
    // the second tap anonymous: it arrived holding nothing, so it could
    // not be told from a stranger and the arbiter had to answer it
    // `notOwner` — which is safe, and is not the same as being COALESCED
    // onto the terminal the first tap is still waiting for. The field is
    // cleared at the terminal instead, and only when the lease is
    // genuinely gone.
    let held = airtimeLease
    stopInternal()
    Self.arbiter.stop(lease: held) { [weak self] outcome, why, requestId in
      if outcome == .clear {
        // The lease is retired; the id is dead. A debt keeps its name,
        // because this instance's obligation is real and outstanding and
        // a later tap deserves the word `debt` rather than `notOwner`.
        DispatchQueue.main.async { self?.airtimeLease = nil }
      }
      resolve([
        "v": WalkieAirtimeArbiter.wireVersion,
        "outcome": outcome.rawValue,
        "why": why,
        "leaseId": held ?? NSNull(),
        "requestId": requestId,
        // THE STATE THE ANSWER WAS TRUE OF, carried with it, so a caller
        // never has to ask a second question to find out which world its
        // own answer belongs to.
        "state": WalkieAirtimeArbiter.shared.currentState(),
      ])
    }
  }

  /**
   THE STATE, ASKED — the same versioned body the event carries, built by
   the same function, because a subscriber and an asker reading two shapes
   is how every previous round of this lane ended up with two levels that
   had to be kept in agreement.

   WHY THE ASK EXISTS AT ALL. An event is only ever worth as much as the
   listener that happened to be attached when it went past, and three
   windows have no listener: between a stop's answer and JS attaching,
   across a JS reload, and across a background where the bridge has nobody
   to deliver to. A level can be asked; an edge cannot.

   AND WHAT IT IS FOR IS NOT WHAT IT USED TO BE. This is OBSERVABILITY.
   Nothing JS decides from this body can move a radio: the arbiter drives
   the suppression and the resume itself, at the effect, against the lease
   it currently holds. A JS world can be arbitrarily stale here and the
   worst it can do is order its own UX wrong — which is exactly the
   demotion the ruling asks for.

   IT READS, IT DOES NOT PROVE. No radio is touched; proving stays with
   the debt chain and the two effect provers, on their own queues.

   Never rejects. "Who owns the airtime?" has an answer in every state
   this process can be in, including none.
   */
  @objc(airtimeState:rejecter:)
  func airtimeState(
    _ resolve: @escaping RCTPromiseResolveBlock,
    rejecter _: @escaping RCTPromiseRejectBlock
  ) {
  ...
  /// `settled` fires once, on the link queue, when this session's BLE
  /// advertiser is proven off the air — or with `false` when the proof
  /// could not arrive. Callers that are not the JS stop (the start path's
  /// own error teardown) pass nothing and the proof goes nowhere.
  private func stopInternal() {
    ...
    #endif
    awareLink = nil
    // THE ADVERTISER IS NOT TORN DOWN HERE ANY MORE, and dropping the
    // reference is now safe rather than the defect it used to be.
    //
    // THE OLD SHAPE, AND WHY IT COULD NOT BE FIXED IN PLACE (arbiter
    // addendum 1): this function nilled `bleVoice` and then started an
    // ASYNC proof on the object it had just let go of. A second stop
    // arrived, read the cleared field, and had to reconstruct "is
    // something still up?" from a debt book that was still empty because
    // the first proof had not failed yet — so it answered `clear` while
    // stop1's advertiser might still be radiating. No guard on this line
    // could have helped: the question "is a stop already in flight, and
    // whose?" had no home in this module at all.
    //
    // It has one now. The lease holds the operation handle (see
    // startBleVoice), the arbiter holds the lease, and a duplicate stop
    // coalesces onto the SAME terminal instead of answering on its own.
    // This line is once again what it reads as: a module forgetting its
    // own reference to something the process still owns.
    bleVoice = nil
    for (_, p) in peers {
      p.connection?.cancel()
    }
  ...
  /**
   THE LIVE-NOT-LATE GATE (see maxLeadMs). A GENERATION means the frame is
   admitted and its depth CLAIMED under that generation; nil means the
   queue is FULL — and the caller's answer to full is flushStaleTail(),
   never dropping the frame it was handed (see that method for why the tail
   is the wrong thing to keep).

   The generation comes back rather than a bare true because the claim and
   its release are separated by the render path (see pendingGen): the
   caller must carry it into the completion handler, so a completion from a
   queue that has since been flushed cannot pay off a claim from the queue
   that replaced it.

   Claiming before scheduling, not after, is what keeps the counter honest:
   a completion handler can fire on another thread the instant the buffer
   is consumed, and a claim added afterwards would be a subtraction that
   arrived before its addition.

   The depth is measured in frames the player has been given and has not
   yet finished with. The plain scheduleBuffer completion fires when the
   data has been CONSUMED by the render path rather than when the last
   sample leaves the speaker, so this reads a render quantum or so short —
   a handful of milliseconds against a 400 ms bound, and short in the safe
   direction: it makes the guard slightly slower to drop, never quicker.
   */
  private func admitFrames(_ n: Int) -> Int? {
    pendingLock.lock()
    if pendingFrames + n > Self.maxLeadFrames {
      staleFlushes += 1
      let flushes = staleFlushes
      let lead = pendingFrames
      var say = false
      if Date().timeIntervalSince(lastLateLog) > Self.lateLogSeconds {
        lastLateLog = Date()
        say = true
      }
      pendingLock.unlock()
      if say {
        NSLog("walkie//stale-flush count=%d lead=%d", flushes, lead)
      }
      return nil
    }
    pendingFrames += n
  ...
  /// Hands a claim back — from the completion handler on the happy path,
  /// and by hand when a raise means the buffer was never scheduled at all.
  ///
  /// ONLY AGAINST THE GENERATION IT WAS ADMITTED UNDER (see pendingGen). A
  /// teardown or a flush resets the count underneath handlers that are
  /// still in flight, and those handlers land later, on the render thread:
  /// paid against the new generation they would subtract frames that
  /// generation never claimed, and the depth would read as room the
  /// channel does not have. Stale generation, no-op. The clamp stays as a
  /// second floor.
  private func releaseFrames(_ n: Int, gen: Int) {
    pendingLock.lock()
    if gen == pendingGen {
      pendingFrames = max(0, pendingFrames - n)
    }
    pendingLock.unlock()
  }

  /**
   EMPTY THE PLAYER FROM THE FRONT, so the frame about to be scheduled is
   the next thing anybody hears (owner-facing correction, 2026-08-27;
   codex cross-family read).

   The first version of the live-not-late guard refused the frame it was
   HANDED and kept everything already scheduled — it dropped the NEWEST
   audio and preserved the STALE TAIL. Two consequences, both wrong in the
   same direction:
     ...
     - under sustained overload the queue never drains below the bound, so
       the channel sits ~400 ms behind FOREVER. The guard bounded the
       lateness and then held it there.

   A live channel has to sound like NOW. AVAudioPlayerNode offers no
   partial eviction, so the whole queue goes: stop() discards every
   scheduled buffer (and fires their completion handlers), play() re-arms
   the node for the frame about to be scheduled. After an eviction the lead
   is zero and the conversation is live again, at the cost of audio nobody
   could have answered anyway — the same trade the sender's drop-on-busy
   law makes, at the other end of the same pipe.

   UNDER engineLock to read the node, and under ObjCTry to touch it: this
   is reachable from NETWORK INPUT, and an AVFAudio call on a node a
   teardown detached under us RAISES — the class the ObjCTry law exists
   for (CLAUDE.md). The depth is reset either way: on the happy path
   because the queue is empty, and on a raise because the node we were
   counting is not a node any more.
   */
  private func flushStaleTail() {
    engineLock.lock()
    let node = playerNode
    engineLock.unlock()
    guard let p = node else {
      resetPending()
      return
    }
    let exc = ObjCTry.run {
    ...
    resetPending()
    if let raised = exc {
      NSLog("walkie//stale-flush raise=%@", raised.name.rawValue)
    }
  }

  /// The player's queue is gone, so the depth is zero — whatever the
  /// in-flight completion handlers still think. Called wherever the node
  /// is dropped, or a dead engine's backlog would read as depth against a
  /// fresh player and silence a working walkie.
  ///
  /// And it OPENS A NEW GENERATION, which is what makes "whatever the
  /// in-flight handlers still think" true rather than aspirational: every
  /// claim admitted before this line belongs to a queue that no longer
  /// exists, so every completion still to arrive for it is now a no-op
  /// (see pendingGen and releaseFrames). Wrapping is fine and free — the
  /// comparison is equality, never ordering.
  ...
  /// MAIN-CONFINED press counter. Every startTalking and stopTalking bumps
  /// it, and every deferred arc a press leaves behind (the 1 s watchdog,
  /// the settle closure's failure half) checks its captured value first —
  /// a STALE press must reject its own promise and nothing else. Without
  /// this, rapid re-keying let press N's watchdog discard press N+1's
  /// engine and removeTap a node whose engine was already gone, which
  /// RAISES — the measured 2026-08-26 crash (build 36 .ips: SIGABRT in
  /// -[AVAudioNode removeTapOnBus:] on the main queue, repeated PTT).
  ...
  private func ensureEngine() throws -> AVAudioEngine {
    ...
    if let e = engine {
      ...
      if e.isRunning && inputAlive {
        ...
        // Re-assert the session shape on reuse ONLY WHEN IT IS WRONG: a
        // WebRTC call can move category/mode under a cached engine
        // unnoticed (research 2026-08-26), and a category set on a
        // session someone else DEACTIVATED only takes effect at
        // activation — but a re-assert on a session that is already
        // right is not free either. The four-phone bench (2026-08-26
        // evening, the iPhone 16) heard why: a PERIODIC setActive
        // re-triggers the system's audio ducking, and the 1 s throttle
        // this replaced turned that into audible ~1 Hz volume pumping
        // during playback. Category and mode are plain local READS with
        // no server side effects, so steady state now costs two
        // comparisons and asserts nothing; the throttle survives only
        // as a floor UNDER the wrong-shape case so a fighting session
        // (WebRTC mid-teardown) cannot make us duck the world at 50 Hz.
        // Failing softly stays right — the engine IS running; a refused
        // re-assert must not kill working audio.
        // The gate reads all THREE readable wrongs (codex cross-family
        // review, 2026-08-26 night): category/mode, AND categoryOptions —
        // a session that lost .defaultToSpeaker routes playback to the
        // EARPIECE, which is the afternoon's "old PTT played quietly" —
        // AND the health of the LAST assert, because a setActive that
        // failed after its category succeeded must retry (the old shape
        // stamped the throttle first and then read as shape-equal, locked
        // out forever). The one wrong no read exposes — shape-equal but
        // DEACTIVATED — heals through the rebuild lanes instead: call end
        // discards the engine, and an interruption fails isRunning or the
        // liveness probe, and both roads end in the create path's full
        // category+activate.
        let session = AVAudioSession.sharedInstance()
        let shapeWrong = session.category != .playAndRecord
          || session.mode != .default
          || !session.categoryOptions.contains(.defaultToSpeaker)
        if shapeWrong || !sessionAssertHealthy {
          let now = ProcessInfo.processInfo.systemUptime
          if now - lastSessionAssert >= 1.0 {
            lastSessionAssert = now
            NSLog("walkie//session-reassert cat=%@ mode=%@ healthy=%d",
                  session.category.rawValue, session.mode.rawValue,
                  sessionAssertHealthy ? 1 : 0)
            do {
              try session.setCategory(
                .playAndRecord, mode: .default, options: [.defaultToSpeaker]
              )
              try session.setActive(true)
              sessionAssertHealthy = true
            } catch {
              sessionAssertHealthy = false
            }
          }
        }
        return e
        ...
        do {
          ...
          let settle: (Bool, String?) -> Void = { [weak self] ok, why in
            ...
            if ok {
              ...
              // Shared state belongs to the CURRENT press only: a stale
              // press settling late must reject its promise and walk away,
              // never un-key the live press or touch the live tap. And the
              // removeTap runs under the catcher — on a node whose engine
              // was rebuilt underneath it, it RAISES (the build-36 crash).
              if self.pressGen == gen {
                self.setTalking(false)
                _ = ObjCTry.run { self.engine?.inputNode.removeTap(onBus: 0) }
              }
              reject(
                "record",
          ...
          func arm() throws {
            ...
            _ = ObjCTry.run { input.removeTap(onBus: 0) }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
              ...
              // A watchdog outliving its press (release, or a NEWER press
              // took the mic) must not discard the engine the live press
              // is using — that cross-press interference was the measured
              // build-36 crash. It settles its own promise QUIETLY and
              // stops: a stale press has nothing honest to report.
              guard self.pressGen == gen else {
                settle(true, nil)
                return
              }
              ...
              if self.isTalking() {
                // A HELD press with a silent mic is a real failure the
                // camper must hear about — silent keying is a lie.
                settle(false, "no-audio")
              } else {
                // A short TAP the engine could not spin up for before the
                // finger left is NOT a failure to scold (the mini, build
                // 40, 2026-08-26: fresh rebuilds — the corpse-heal working
                // — made first buffers slower than a dab at the button,
                // and every dab earned a "(no-audio)" notification for a
                // press that was already over). The press ended; resolve
                // it quietly. A real mic failure still surfaces on any
                // held press, which is what talking actually is.
                NSLog("walkie//tap-outran-engine")
                settle(true, nil)
              }
            }
          }
  ...
  func stopTalking(
    ...
    setTalking(false)
    _ = ObjCTry.run { engine?.inputNode.removeTap(onBus: 0) }
    // Retire this press's deferred arcs (watchdog, late settle) so none of
    // them mistakes itself for current after the key is released.
  ...
  /**
   THE TWO-THREAD PULSE (docs/VIDEO-CALLS.md §8).

   Two hard freezes on 2026-08-26 ended in a force-quit and left no
   evidence at all: a wedged phone writes no .ips, and a tethered syslog
   shows the same picture — silence — whether the JS thread stopped, the
   main thread stopped, or nothing was happening. Three different bugs,
   one indistinguishable trace.

   So ONE call from JS prints TWO lines from TWO threads: this one, on
   whatever queue the bridge delivered it on (which only exists because JS
   is still running), and `main`, from the main queue (which only lands if
   main is still draining). Read the tether:

     both lines repeating   -> neither thread is wedged; look elsewhere
     `js` alone repeating   -> the MAIN thread is blocked
     neither, app on screen -> the JS thread is blocked

   NO PROMISE ON PURPOSE. A resolver would put the diagnostic back on the
   bridge hop it is measuring, and add a JS continuation to a call whose
   entire job is to cost nothing. JS fires it every 2 s inside the two
   windows a freeze has been seen in, and never otherwise.

   No ObjCTry here: NSLog and DispatchQueue.main.async are C and libdispatch,
   not framework preconditions — there is no ObjC exception to catch.
   */
  @objc(logPulse:)
  func logPulse(_ tag: String) {
    NSLog("walkie//hb %@", tag)
    DispatchQueue.main.async {
      NSLog("walkie//hb main")
    }
  }
  ...
  private func handleFrame(_ d: Data, lane: String) {
    ...
    let claimed = Int(frames)
    // A FULL QUEUE LOSES ITS TAIL, NEVER THIS FRAME (see flushStaleTail).
    // The second admit cannot fail for a real frame — one 20-60 ms buffer
    // is a fraction of the bound and the depth is zero after a flush — but
    // it is asked rather than assumed, because scheduling against a claim
    // that was never granted is how the depth starts lying.
    var admitted = admitFrames(claimed)
    if admitted == nil {
      flushStaleTail()
      admitted = admitFrames(claimed)
    }
    // The generation the claim was granted under travels with it into the
    // completion handler (see pendingGen): the flush above just retired a
    // whole queue's worth of handlers, and every one of them is still
    // going to fire.
    guard let gen = admitted else { return }
    ...
    let exc = ObjCTry.run {
      p.scheduleBuffer(buf, completionHandler: { [weak self] in
        self?.releaseFrames(claimed, gen: gen)
      })
    }
    // A raise means the buffer was NEVER scheduled, so its completion
    // handler will never fire. Hand the claim back here or the depth
    // ratchets upward on every caught raise and the guard eventually
    // drops a channel that is perfectly on time.
    if exc != nil {
      releaseFrames(claimed, gen: gen)
    }
    // The speaking chip fires either way, and that is deliberate: someone
    // IS talking. Emptying the queue is this phone admitting it cannot
    // keep pace, not a reason to pretend the channel is empty.
