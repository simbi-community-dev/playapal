  ...
  private func ensureEngine() throws -> AVAudioEngine {
    ...
    if let e = engine {
      ...
      if e.isRunning {
        ...
        // 2026-08-26). Failing softly is right here — the engine IS
        // running; a refused re-assert must not kill working audio.
        try? AVAudioSession.sharedInstance().setCategory(
          .playAndRecord, mode: .default, options: [.defaultToSpeaker]
        )
        return e
      }
      // The restart failed: the cached engine is DEAD, not paused
      // (measured on the mini 2026-08-25 — a WebRTC call's interruption
      // leaves an engine start() will never revive, and returning it
      // anyway is how PTT read "no-audio" while received frames played
      // nothing). Discard it and rebuild from scratch below; a rebuild
      // failure THROWS, which the talk path turns into an honest reject.
      NSLog("walkie//engine-dead rebuilding")
      e.stop()
      engine = nil
      playerNode = nil
    }
    ...
    AVAudioSession.sharedInstance().requestRecordPermission { [weak self] granted in
      ...
      DispatchQueue.main.async {
        ...
        do {
          ...
          // FORMAT: nil ON PURPOSE (the mini, builds 25 through TF4: every
          // precondition patch — guards, a session bounce — still ended in
          // a raise or a dead pre-read). nil means "the bus's own native
          // format", which cannot mismatch, and the converter is built
          // LAZILY from the first real buffer — no pre-read of
          // input.outputFormat can lie to us. The question CoreAudio kept
          // raising about no longer gets asked.
