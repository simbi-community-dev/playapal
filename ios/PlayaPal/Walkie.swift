...
final class Walkie: RCTEventEmitter {
  ...
  /**
   ...
   land more than this far ahead of what has actually PLAYED is dropped on
   the floor and counted, never queued. Dropping is the rung being honest
   about its bandwidth, exactly like the lo-fi badge; playing ten-second-old
   voice is a lie about liveness, and a walkie that lies about liveness is
   worse than silence on the playa — you answer a question nobody is still
   asking.
  ...
  /// How often a late-drop run is allowed to say so. The log line is for
  ...
  /// How many frames the lateness guard has dropped this session, and when
  /// it last said so out loud. Observable on purpose: a bench that cannot
  /// name the drop rate cannot tell a rung at its ceiling from a rung that
  /// is broken.
  ...
  /**
   THE LIVE-NOT-LATE GATE (see maxLeadMs). True means the frame is admitted
   and its depth CLAIMED; false means drop it and say so.
   ...
   */
  private func admitFrames(_ n: Int) -> Bool {
    pendingLock.lock()
    if pendingFrames + n > Self.maxLeadFrames {
      lateDrops += 1
      let dropped = lateDrops
      ...
      pendingLock.unlock()
      if say {
        NSLog("walkie//late-drop count=%d lead=%d", dropped, lead)
      }
      return false
    }
    pendingFrames += n
  ...
  /// Clamped at zero because a teardown resets the count underneath
  /// handlers that are still in flight; a late handler must not push it
  /// negative and hide the NEXT session's backlog.
  private func releaseFrames(_ n: Int) {
    pendingLock.lock()
  ...
  private func handleFrame(_ d: Data, lane: String) {
    ...
    if admitFrames(claimed) {
      ...
      if exc != nil {
        releaseFrames(claimed)
      }
    }
    // The speaking chip fires either way, and that is deliberate: someone
    // IS talking. Dropping their audio is this phone admitting it cannot
