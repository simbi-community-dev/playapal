  ...
  private func ensureEngine() throws -> AVAudioEngine {
    ...
    if let e = engine {
      ...
      if e.isRunning && inputAlive {
        ...
        // Re-assert the session shape on reuse: a WebRTC call can
        ...
        // audio. Throttled to 1/s: each re-assert is an audio-server
        // XPC round trip, paid per received frame on main. The trade:
        // post-call heal latency is bounded by the window — the mid-call
        // heal never ran on this path anyway (playbackMuted returns
        // first).
