...
if [ "${1:-}" = "--apk" ]; then
  ...
  # had run on fab.)
  (cd android && FAB_ALLOW_LOCAL_BUILD=1 ./gradlew assembleGithubRelease -PallowDebugSigning=true)
  sha256sum android/app/build/outputs/apk/github/release/app-github-arm64-v8a-release.apk
fi
echo 'GATE: PASS'
