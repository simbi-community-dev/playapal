...
def doc_files(pack_dir):
    """The doc files the installer will chunk: .md/.txt by case-insensitive
    suffix (installPack.ts ext()), reserved manifests skipped, deterministic
    lowercased-name order (the doctor's iteration order; embedding keys are
    per-source ordinals, so cross-file order never changes a key)."""
    out = []
    for path in sorted(pathlib.Path(pack_dir).iterdir(), key=lambda p: p.name.lower()):
        if not path.is_file() or path.name.lower() in RESERVED:
            continue
        suffix = path.name.rsplit(".", 1)[-1].lower() if "." in path.name else ""
        if suffix in {"md", "txt"}:
            out.append(path)
    return out
...
def pack_chunks(pack_dir, max_chars=DEFAULT_MAX_CHARS):
    ...
    out = {}
    for path in doc_files(pack_dir):
        per_source_idx = 0
        for heading, content in _doctor.chunk_document(
            _doctor.read_text(path), max_chars
        ):
            out[f"{path.name}:{per_source_idx}"] = {"heading": heading, "content": content}
            per_source_idx += 1
    return out
