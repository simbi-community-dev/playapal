...
a serverless friend finder — in one React Native app that runs entirely on
the phone: no server, no cloud account, no network connection needed.
**Every one of those tools works with no AI model installed.** The Playa
Angel — a small open model fine-tuned on the same guides — is the optional
...
anyone can retype a card by hand — share what you'd write on a note board.
...
public release — then **0.5.1** added share-time consent on friend cards,
