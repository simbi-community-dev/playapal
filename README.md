...
- **Playa Angel Max** is a fine-tune of Google's **Gemma 4 E4B** (Q4_0), so
  it inherits Google's **Gemma Terms of Use** instead — a different regime,
  with its own use restrictions that flow to derivatives.
...
- **Model weights** (downloaded, not part of this repository): two regimes,
  ...
  [Gemma Terms of Use](https://ai.google.dev/gemma/terms), whose use
  ...
  **Playa Angel Light** are LFM2.5 derivatives under the LFM Open License
  ...
  license covers all three; the per-file detail is in [`NOTICE`](NOTICE).
...
mode. The full story is in [CHANGELOG.md](CHANGELOG.md); 1.0 is earned on
playa.
