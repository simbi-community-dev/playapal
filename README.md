...
**A companion for Black Rock City that needs no internet, no cell service,
and no infrastructure of any kind.**

There is no signal on the playa. There is no server behind this app, no
account to make, and nothing to connect to. Playa Pal is one React Native
app that runs entirely on the phone in your hand: the 2026 event guide, a
survival manual you can read straight through, the city map, a whiteout
compass, your camp's board, your friends, and a **pod** of phones that find
each other, carry mail, and talk out loud with no network between them.

Ask it anything and the **Playa Angel** — a small open model fine-tuned on
these same guides, living on the phone — answers from what is stored there
and shows you the rows it read. Everything above worked before she was
...
Every word of the answer is generated on the phone. Nothing you ask leaves
your hand — not to a server, not to us, not ever.
...
home** — see [*Spoken replies*](#spoken-replies); some device voices are
...
full quality **with no Wi-Fi at all** — the linked phones talk straight to
each other. There is no server and no internet anywhere in it: the media
...
**Camp hotspot** under your pod and one phone hosts a no-internet Wi-Fi that
everyone else joins by pointing a camera at the code on screen — an iPhone
offers to join straight from the stock Camera app. Fourteen distinct things
...
- **Seconds** when two phones are near with both apps open. **Minutes**
  ...
  otherwise — and the app tells you which of those you are living in
  rather than implying the fastest.
...
instead of "a message came in". There is no push
...
— a radio quietly eating your battery where you cannot see it is not
something this app will do.

## The city in your pocket

- **What's happening now, and all week.** Browse or search by date, time,
  camp, location and description. Tap the ♡ on any event card — browsing,
  in search results, even on the Angel's answers — and it lands in **♥
...
- **The whole city, drawn offline.** The 🗺 map renders Black Rock City from
  measured geometry — streets, Center Camp, the Man, the Temple, toilet
  banks — with your blue dot, your pins, the active waypoint and your
...
- **Person cards and camp knowledge.** Ask about a person and the Angel
  resolves the identity first, then retrieves the card linked to it —
  never a guess dressed as a fact. In-app notes (memory, event, fix,
...
- **A first hour that explains itself.** First launch is a 30-second
  welcome — your name (optional) and your camp, picked from the real 2026
  placement directory — then a five-card tour. Skip any of it; replay all
  of it from Settings.
...
- **The roster never lists a phone it cannot reach.** A peer appears only
  ...
  link goes quiet reads **(quiet)** — not "lo-fi", which would promise
  live audio over a link that has none — with one sentence saying your
...
- **Failures say their reason.** A call whose invites never left your phone
  ends with "Couldn't get through — their link went quiet", not "No
  answer", and it names the thing that *does* still work: hold the talk
...
- **Damage is named rather than played.** A voice note whose save failed —
  the right size, real bytes, no index, unplayable by anything — is
  refused at the moment of recording. One that arrives damaged from an
  older phone wears "Voice note — won't play" and the reason, and tapping
...
- **Speed is never promised.** Mail says which of seconds, minutes, or
  ...
  lands "when the message reaches it" — the words *instantly*, *right
...
- **The limits are written down twice**, in the two registers that need
  ...
  for a person standing in dust — which is also in the app, at Settings ›
  Help & about › **How Playa Pal works**. A test holds the two lists in
  step.
...
hands it to Android's installer. Nothing happens until you tap it — a phone
with no signal must not spend its battery on a lookup nobody asked for.
...
receiving phone, skip the transfer popup's own "Open" — it sometimes
...
carries no position — where you are goes out only while you have position
...
- **Device proof, rung by rung.** `docs/TEST-MATRIX.md` is the grid that
  ...
  discover a nearby iPhone over Bluetooth, diagnosed and being fixed.
...
- **A podmate reachable only over Bluetooth has no call button.** A video
  call needs the room a shared Wi-Fi gives it — a camp router, or a
  hotspot with no internet behind it, including the Camp hotspot one
  Android in the pod can turn on — or two iPhones that have been linked
  directly to each other, which carry a call with no Wi-Fi at all. Any
...
mode. Then the eighth decimal earned four more: **0.8.2** the pod's own
face — 1:1 video calls, a plain answer to "am I connected", codes scanned
inside the app; **0.8.3** the link that holds in the dust; **0.8.4** the
phones that make their own network — Camp hotspot, iPhone-to-iPhone
pairing, and the iPhone microphone cured at three roots; and **0.8.5**, the
current release, where the talk button proves the microphone is alive and
heals it when it isn't. The full story is in
[CHANGELOG.md](CHANGELOG.md); 1.0 is earned on playa.
