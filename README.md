...
- **Talk live, with or without a network:** hold the walkie button and the pod hears you. Any shared Wi-Fi works, including a hotspot with no internet behind it; where the phones can form their own direct link they use that instead; and with no Wi-Fi at all the voice still carries over Bluetooth, rougher, marked *lo-fi*. Incoming voice is amplified and plays through the loudspeaker, on the volume rocker you already use. The microphone runs only while the button is held.
- **Call one podmate:** a 1:1 video call over the same link the walkie uses — no server and no internet, with the quality climbing on its own to whatever the two phones can carry between them. Hold-to-talk stands down while a call has the microphone.
...
both ways between two Android phones with every network off. Rung 3, the
Bluetooth voice path, and the video call layer are built and unit-covered
but have not yet run between two devices. Rung 4 over Wi-Fi Aware is
...
- **A podmate reachable only over Bluetooth has no call button.** A video
  call needs a shared camp Wi-Fi (a hotspot with no internet behind it
  counts) or two newer phones linking straight to each other; the button
