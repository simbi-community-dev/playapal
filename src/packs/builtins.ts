...
const loreManifest = require('../../assets/packs/playa-lore/pack.json') as PackManifest;
const loreFiles = [
  require('../../assets/packs/playa-lore/lore-1.md.json'),
  require('../../assets/packs/playa-lore/lore-2.md.json'),
  require('../../assets/packs/playa-lore/lore-3.md.json'),
  require('../../assets/packs/playa-lore/lore-4.md.json'),
  require('../../assets/packs/playa-lore/lore-5.md.json'),
  require('../../assets/packs/playa-lore/lore-6.md.json'),
] as { file: string; markdown: string }[];
...
const loreVectors = require('../../assets/packs/playa-lore/embeddings.json');
...
export const BUILTIN_PACKS: BuiltinPack[] = [
  ...
  {
    // PLAYA LORE — our own SUMMARIES of veteran-burner talks and camp
    // histories, each crediting its source video by uploader, title, URL
    // and date and pointing the reader at the original. Bundled because a
    // camper asking "what is the Temple for" or "how did Rangers start"
    // deserves camp culture, not a survival-guide near-miss (owner,
    // 2026-08-25).
    //
    // WHY THIS SHIPS WHEN TRANSCRIPTS DO NOT (docs/LICENSING.md §3): the
    // hard line is on TRANSCRIPT CONTENT — verbatim, personal-local,
    // never redistributed. These files are transformative summaries with
    // per-source attribution, the same posture already blessed for the
    // Burn.Life techniques. The raw transcript pack stays out of the repo.
    //
    // Vectors built with the SAME bge-small-en-v1.5-q8_0 the other packs
    // used, at the same 2,000-char default chunk size — 545 of them. The
    // installer refuses a mismatch as a stale vector build, which is what
    // keeps this honest.
    manifest: loreManifest,
    files: [
      { name: 'pack.json', content: JSON.stringify(loreManifest) },
      ...loreFiles.map(f => ({ name: f.file, content: f.markdown })),
      { name: 'embeddings.json', content: JSON.stringify(loreVectors) },
    ],
  },
];
