...
/**
 * A section's trailing attribution line, e.g.
 * `*Credit: [Burn.Life — Lag Screws 101](…) (Dr. Yes…). Retrieved …*`
 * The curated packs put one at the END of each credited section (the
 * burn.life pattern), so it belongs to the section it closes.
 */
const CREDIT_LINE = /^\*Credit:.*\*$/;

function trailingCredit(body: string): string | null {
  const lines = body.trimEnd().split('\n');
  const last = (lines[lines.length - 1] ?? '').trim();
  return CREDIT_LINE.test(last) ? last : null;
}
 ...
 *
 * CREDIT RIDES WITH CONTENT. A credited section longer than the budget used
 * to split its attribution away from the technique it credits: the retriever
 * would surface the lag-screw chunk and the model would cite it with NO
 * source visible, while the credit sat alone in the next chunk. Measured
 * 2026-08-24 at the guide's real 700-char budget (its "Anchoring: lag screws"
 * section is 1,289 chars) — invisible until then only because the tests
 * chunked at 2,000 and the whole section fit. So every piece of a credited
 * section carries that section's credit line.
 *
 * The budget is reduced by the credit's own length BEFORE packing, because
 * the chunk is the excerpt unit: a chunk that overflows the budget is one
 * the tool payload shows only part of, which is exactly how a fact — or an
 * attribution — goes missing behind a perfect-looking citation.
 */
export function chunkDocument(
  ...
  for (const section of splitSections(text)) {
    ...
    const credit = trailingCredit(body);
    // Floor at 200 so a pathologically long credit cannot starve the body.
    const budget = credit
      ? Math.max(200, maxChars - credit.length - 2)
      : maxChars;
    for (const piece of packParagraphs(body, budget)) {
      const content =
        credit && !piece.includes(credit) ? `${piece}\n\n${credit}` : piece;
      chunks.push({ heading, content, index: chunks.length });
    }
  }
  return chunks;
}
