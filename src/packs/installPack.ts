...
import { VECTOR_DIM } from '../docs/vectorSearch';
...
import { isISODate, weekdayName } from '../events/timeParser';
...
function normalizeEvent(raw: any, file: string, index: number): ParsedEvent {
  ...
  if (!DATE_RE.test(date) || !isISODate(date)) {
...
// VECTOR_DIM lives in src/docs/vectorSearch.ts, which documents WHY it is
// 384 (the shipped bge-small embedder). A second copy here could disagree
// with the search path that consumes these very vectors, and the symptom
// would be silently empty semantic results rather than an error.
