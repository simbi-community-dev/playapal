...
const asCard = (raw: unknown): FriendCard | null => {
...
export function getMyCard(conn: QuickSQLiteConnection): FriendCard {
  ...
  if (raw) {
    const card = asCard(JSON.parse(raw));
    if (card) {
      return card;
    }
  }
...
export function saveMyCard(
  ...
  if (clean(fields.name).length === 0) {
    // A nameless card would be rejected by getMyCard on reload, silently
    // minting a NEW id on the next valid save — friends would see a
    // duplicate person instead of an update (review 2026-08-19).
...
/**
 * Every card this phone holds (mine + friends'), verbatim — the gossip
 * export that lets one person assemble a crew's map and pass it on.
 */
export function exportFriendsBundle(conn: QuickSQLiteConnection): string {
  const me = getMyCard(conn);
  const cards = [...(me.name.length > 0 ? [me] : []), ...listFriends(conn)];
  if (cards.length === 0) {
    throw new Error('No cards to beam yet — fill in yours or import a friend.');
  }
  const bundle: FriendBundle = {
    kind: FRIEND_BUNDLE_KIND,
    format: FRIEND_BUNDLE_FORMAT,
  ...
  try {
    for (const card of bundle.cards) {
      ...
      conn.execute(
        ...
        [card.id, card.seq, card.name, card.camp, card.address, card.note, card.updated_at],
      );
