...
import { getSettingOn } from '../events/engine';
...
const rematNoteProjections = (conn: DbConnection, noteKey: string): void => {
  ...
  if (r.rows && r.rows.length > 0) {
    ...
    const stored = getSettingOn(conn, CAMP_PASSPHRASE_KEY);
...
/**
 * AN ILLUSTRATION IS NOT EVIDENCE (owner field report 2026-08-25). A guide
 * section whose own heading says "[illustrative]" — "Sample entry shape
 * (structure, not a real 2026 event)" — is a writing sample for a human
 * reading the guide, and the Angel stood on it to answer a Burning Man
 * HISTORY question with a made-up event ("Sunrise Yoga & Chai"). The
 * corpus already declares these sections; retrieval simply must not be
 * able to cite one.
 *
 * Applied to the doc chunk table aliased `d`, INSIDE the query for the
 * same reason the event filter is: a post-hoc filter under LIMIT
 * underfills. Filtering at retrieval rather than at chunk time is
 * deliberate — dropping the chunk would renumber every later chunk and
 * silently misalign the pack's precomputed vectors (its stale-build guard
 * caught exactly that). When a pack is next rebuilt WITH its embeddings,
 * excluding the section at chunk time is the cleaner home for this.
 */
export const DOCS_NOT_ILLUSTRATIVE_SQL =
  "d.heading NOT LIKE '%[illustrative]%'";
