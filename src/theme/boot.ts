...
// The saved text size, read at the same moment for the same reason. Unlike
// the palette this one does NOT have to beat the StyleSheet freeze — every
// Text multiplies it in at render (src/components/Text.tsx) — but reading
// it here means the first frame is already the right size, with no
// one-frame jump from default up to the camper's rung.
loadTextScale();
