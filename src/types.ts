...
export interface DateWindow {
  ...
  label: string;
  /** Optional per-day start/end filter for multi-day clock-specific windows. */
  timeStart?: string;
  timeEnd?: string;
  /** Schedule-row date when an after-midnight clock belongs to the prior night. */
  eventDateISO?: string;
}

type EventSearchStrategy =
  | 'fts-phrase'
  | 'fts-and'
  | 'fts-or'
  | 'like-and'
...
interface EventSearchBase {
  window: DateWindow | null;
  /** Canonical semantic terms for this tool execution. Older persisted/test
   * outcomes may omit it; live search_events results always include it. */
  query?: string;
  /** Which query strategy produced the rows (for debugging/telemetry). */
  strategy: EventSearchStrategy;
}

/** Structured result of one search_events execution. The discriminant keeps
 * “tool could not search”, authoritative zero, and relaxed alternatives apart. */
export type EventSearchOutcome =
  ...
  | (EventSearchBase & {
      state: 'empty';
      results: [];
      searchedScope: 'requested-window' | 'all-enabled-events';
    })
  | (EventSearchBase & {
      state: 'matches';
      results: EventRow[];
