...
function armSession(
  ...
  const session = startCrewSession({
    ...
    // THE MESH READINESS BARRIER, THREADED FROM THE OWNER SIDE. The session
    // owns "can the pod see me", and since the mailbox decoupling the honest
    // answer includes "can the pod READ me" — which only the mesh's own
    // publish ack can say. session.ts must not import meshSync to ask (it
    // is the state machine the mesh's radio window hangs off, and the
    // dependency would run backwards through every suite that drives a
    // session), so this file — which already starts the mesh below and
    // wires the native state stream to the session above — hands the
    // barrier in. Minting it is what captures the mesh identity; a mesh
    // that is not running answers "no work" and the recovery settles on its
    // two radio legs alone.
    awaitMeshDigest: awaitMeshDigestReady,
  });
...
async function teardownSession(): Promise<void> {
  // AWAITED, BECAUSE A STOP IS A BARRIER (row 107). This was
  // `stopMeshSync();` — fire and forget — so teardown ran on through
  // masterOff and resolved while the native side had not retired anything
  // yet. On iOS that meant the previous session's services, per-central
  // buffers and open wants were all still live and readable at the moment
  // the UI finished saying "off", and the replacement session could be
  // admitted on top of them. Native completes its retirement before the
  // promise resolves; this is the line that makes the promise mean something.
  await stopMeshSync();
  ...
  // THE SHARING SURFACE ITSELF, and this is the barrier the session's own
  // stop never was. `session.stop()` inside masterOff calls
  // stopAdvertising + stopScan: on Android stopAdvertising closes the GATT
  // server on its way out, but on iOS it stops DISCOVERY only — the service
  // stays published and the payload characteristic keeps its last value, so
  // a central that learned this iPhone's identifier while sharing could
  // reconnect by it after the camper turned sharing off (or removed their
  // last mailbox) and read the payload back. `stopAll` is the verb both
  // modules export for exactly this and it had no production caller at all.
  //
  // The walkie's hold does NOT come here: that suppression is temporary and
  // keeps the mailbox reachable on purpose (holdCrewAdvertising below).
  //
  // A radio that refuses to retire is a degraded rung, not a reason the
  // camper cannot turn sharing off — so the failure is said out loud and the
  // teardown still completes.
  try {
    await stopAllRadio();
  } catch (e) {
    // eslint-disable-next-line no-console
    console.log(`PlayaMesh teardown//stop-all-failed err=${String(e)}`);
  }
}
...
//
// SO THE WALKIE GETS THE AIRTIME WHILE IT IS OPEN — AND THE SEAM MOVED
// NATIVE (ARCHITECTURE ruling, 2026-08-27).
//
// IT USED TO BE HERE, and the reasoning was that rung 3's voice link and
// the crew beacon are separate concerns which only the app knows were
// asked for at once. That was true about the INTENT and wrong about the
// EFFECT. Four rounds of no-gos all landed on the same root: a JS world
// deciding a radio's fate from a snapshot, where the snapshot was true
// when it was built and false when it was read, and where every JS local
// the decision leaned on is reset by a reload the radios do not share.
//
// A process-lifetime arbiter owns the slot now (WalkieBleVoice.swift). It
// hands it out as an exact lease, drives BOTH advertisers' suppression and
// resumption itself at the effect, and is the only thing that may end a
// hold. Both modules are its clients.
//
// SO WHAT ARE THESE TWO FUNCTIONS FOR? The JS-side half of the same
// suppression, and it is a real job rather than a vestige: radio.ts owns
// the cadence, and a tick that called setPayload/startAdvertising into a
// beacon the arbiter is holding down would be this world arguing with the
// process about a radio. `advertisingHeld` stops that argument. What it
// is NOT is proof — the arbiter's own effect read is — and nothing above
// may treat it as any.
//
// The consequence is the demotion the ruling asks for: a JS world can be
// arbitrarily stale here and the worst it costs anybody is a cadence tick.
// It cannot cost an overlap, because no road from a JS read to a radio
// exists any more.
//
...
export function holdCrewAdvertising(): Promise<void> {
  return serialized(async () => {
    ...
    // BOTH HALVES OF THE ONE RADIO, taken together. The advertiser going
    // quiet is the TRANSMIT half (see the essay above); the mesh airtime
    // hold is the RECEIVE half — meshSync's dial queue occupies the same
    // radio the walkie needs for its scan, and on iOS that scan is the
    // only road to discovering an Android peer at all.
    //
    // SET BEFORE THE AWAIT, and deliberately: setCrewAdvertisingHold never
    // throws, but this ordering means the mesh hold cannot be skipped by
    // anything that happens below it. The release clears it the same way,
    // first thing past the same gate, so every path out of that function
    // — early return, refused refresh — hands the clock back.
    setMeshAirtimeHold(true);
    await setCrewAdvertisingHold(true);
  });
}

/**
 * Give the slot back — THIS WORLD'S HALF OF IT.
 *
 * The arbiter has already put the crew beacon back on the air, itself, at
 * the effect, before this runs (S4). What is left is the JS cache and the
 * cadence: clearing the flag alone would leave radio.ts believing it is
 * still suppressed until the next 15 s tick, so the live session is
 * refreshed straight away — and if the session was torn down and rebuilt while the walkie was
...
export function releaseCrewAdvertising(): Promise<void> {
  return serialized(async () => {
    ...
    // The mesh clock comes back FIRST and unconditionally. A hold that
    // leaks is not a slow phone, it is a phone that never syncs pod mail
    // on the fast clock again for the life of the process — so it is
    // cleared before the two things below that can decline: the early
    // return for a phone with no session standing, and the refresh() whose
    // failure this function swallows on purpose.
    setMeshAirtimeHold(false);
...
async function startMailboxPresenceInner(): Promise<void> {
  ...
  // AND THE REASONS ARE READ AGAIN ON THE FAR SIDE OF THE AWAIT (M7, the
  // mesh lane's handoff, and it is not a formality).
  //
  // haveCrewPermissions() is a native round-trip, and on a phone that is
  // being backgrounded it is a round-trip whose ANSWER arrives after the
  // app is gone: the AppState listener above has already set foreground
  // false and queued stopMailboxPresence — behind THIS op, because the
  // flip queue is held by the very call that is suspended here. So the old
  // shape resumed into a world it had stopped being true for, armed the
  // session, put the advert on the air and started meshSync, and only then
  // let the stop it had jumped ahead of tear all of it down again. Same
  // road for the last pod being disbanded mid-await: an advert for nobody,
  // raised and retracted.
  //
  // THE POSTURE READ REFUSES ONLY ON AN EXPLICIT 'background', which is
  // deliberate on both sides. iOS reports 'inactive' while a permission
  // sheet is up and for the moment either side of it, so a stricter test
  // would refuse exactly the grants this path exists to act on; and a
  // harness or headless start with no AppState has no posture to refuse
  // on at all (meshSync's own start reads it this way, same reason). The
  // `foreground` latch is not the reader here on purpose: it is false
  // until installMailboxPresence has run, and a direct startMailboxPresence
  // must not be refused for a lifecycle wire that is not this call's job.
  if (
    active ||
    !crewRadioPresent() ||
    listCrews().length === 0 ||
