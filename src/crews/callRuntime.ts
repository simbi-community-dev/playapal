...
export class CallRuntime {
  ...
  /** Flip goes through the REDUCER, not straight at the track: the model
   * has to learn which lens is live so the self-view can stop mirroring a
   * rear camera (videoCall.ts callSelfMirrored). */
  flipCamera(): void {
    this.dispatch({ type: 'flip-camera' });
  }
