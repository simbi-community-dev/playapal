 ...
 *
 *  - RENDER STALENESS. Liveness EXPIRING is not an event: nothing bumps a
 *    revision when a sighting ages past the live window, so a fold made
 *    while a phone was live can outlive its evidence on screen until the
 *    next presence, message, or crew bump re-renders the section. In the
 *    field any nearby radio traffic bumps presence within seconds; a
 *    genuinely silent mesh can hold a stale fold for minutes. The cure
 *    would be a timer tick in the section; deliberately not added the
