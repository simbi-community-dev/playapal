...
/** One sentence for a refused mic, wherever the refusal surfaces — the
 * pre-flight Android request and the runtime 'permission' reject must say
 * the same thing (field report 2026-08-25), and a shared const is what
 * enforces it. The never-ask-again Settings variant stays its own string. */
const MIC_ASK_COPY =
...
export function WalkiePanel({
  ...
  // "LOOK AGAIN" (owner ask 2026-08-26: the channel flaps and
  // autodiscovery is opaque). The flag is an ACKNOWLEDGEMENT OF THE TAP,
  // not a claim about anybody: docs/WALKIE-LADDER.md §5a forbids
  // "connecting…" because such a word promises a peer, and this one only
  // says what this phone is doing — it asked, a moment ago. It clears on a
  // timer rather than on the native promise because both natives resolve
  // in milliseconds, and a label that blinks is a label nobody saw.
  ...
  const lookAgain = useCallback(() => {
    setLooking(true);
    announce('Asking every radio to look again');
    void refreshWalkieDiscovery();
    if (lookTimer.current) {
      clearTimeout(lookTimer.current);
    }
    lookTimer.current = setTimeout(() => setLooking(false), 2000);
  }, []);
  ...
  // Computed once per render — it renders twice below (gate + line).
  const capLine = walkieCapCopy(peers.length);

  return (
    <View style={styles.wrap}>
      {/* THE RADIO SWITCH, and it now LOOKS like one (UX review
          2026-08-26). It used to be a line of bold text — a header, to any
          eye that has ever seen one — that was secretly the on/off for a
          live radio, with its whole state carried in a trailing clause a
          camper had to READ to learn the mic was hot. Two costumes, one
          control: the named anti-pattern of that review.

          It is a Switch now, wearing the same control and the same sage
          track as the two other on/off questions a pod asks ("Share my
          position with…", "Host it on this phone") — because it is the
          same KIND of question, and nobody should have to learn a third
          gesture for it. The state is said in WORDS under the title rather
          than appended to it, so on/off is legible three ways: the switch,
          the sentence, and the strip's live dot above.

          IT REPORTS THE RADIO, NEVER THE STAGE. `open` is only whether the
          stage is showing; binding the switch to that would render the
          walkie as OFF while the mic was still live, which is the worst
          state a walkie has. The stage's own Hide row below closes the
          VIEW alone — a genuinely different act, and now it looks it. */}
      <View style={styles.radioRow}>
        <View style={styles.radioBody}>
          <Text style={styles.radioTitle}>🎙 Walkie</Text>
          <Text style={on ? styles.radioOn : styles.radioOff}>
            {on
              ? 'On — your voice goes out when you hold the button, and this pod comes through. It keeps running while you look at other screens.'
              : 'Off — nothing is on the air. Turn it on to talk, and to take a call.'}
          </Text>
        </View>
        {/* The Switch names ITS OWN setting (a11y review 2026-08-24): an
            unlabeled switch reads as "switch, off". */}
        <Switch
          value={on}
          onValueChange={toggleWalkie}
          accessibilityLabel="Walkie — live talk with this pod"
      ...
      {open ? (
        <View>
          ...
          {walkieRefreshPresent() ? (
            // IT LIVES WITH THE CHANNEL LINE, not with the setup gestures
            // at the foot of the stage, because it is about THIS list: the
            // camper is reading for a name that should be there and is
            // not, and the answer belongs next to the question. It is a
            // quiet verb — the same weight as Show/Hide — precisely so it
            // never competes with the button below it, which is the only
            // control on this stage that puts a voice on the air.
            <View style={styles.lookRow}>
              <Pressable
                onPress={lookAgain}
                accessibilityRole="button"
                accessibilityLabel="Look again for podmates"
                style={styles.stageTap}>
                <Text style={styles.stageVerb}>
                  {looking ? '🔄 Looking…' : '🔄 Look again'}
                </Text>
              </Pressable>
              <InfoTap topic="looking again" text={WALKIE_REFRESH_COPY} />
            </View>
          ) : null}
          <Pressable
            onPressIn={pressIn}
...
const styles = StyleSheet.create({
  ...
  // The radio switch row — the shareRow shape from the pod's other on/off
  // question, so the two read as the same kind of control.
  radioRow: {
    alignItems: 'center',
    flexDirection: 'row',
    minHeight: tap.minHeight,
  },
  radioBody: { flex: 1, marginRight: spacing.md },
  radioTitle: { color: colors.night, fontSize: type.body, fontWeight: '700' },
  // ON wears the status gold the rest of the app gives to "something is
  // happening that you should know about"; OFF stays metadata-faded. An
  // open mic must never be the quieter of the two states.
  radioOn: { color: colors.gold, fontSize: type.small, marginTop: 2 },
  radioOff: { color: colors.faded, fontSize: type.small, marginTop: 2 },
  ...
  // The verb and its ? sit on one line: the explanation is teaching text
  // about a control, so it belongs beside the control, not under it.
  lookRow: { alignItems: 'center', flexDirection: 'row', gap: spacing.sm },
