 ...
 * Display reads resolve newest-per-author (dedupe by from_hash, keep the
 * newest created_min). The one deliberate history read is historicalSelfNames:
 * delayed mail can still contain the name this exact stable self card used
 * when the sender composed it. Those names are mention targets only — never
 * roster rows or sender display — and every retained row is revalidated.
...
/** A retained row earns identity only when its body names the same stable
 * card its envelope hashes. Shared by current display and the one historical
 * self projection so neither path can accidentally relax the roster gate. */
const validMemberBody = (rec: CrewRecord): MemberBody | null => {
  const body = decodeMemberBody(rec.body);
  return body && hash32(body.cardId) === rec.from_hash ? body : null;
};
...
export function announcedMembers(crewCode: string): AnnouncedMember[] {
  ...
  for (const rec of recordsOfKind(POD_MEMBER_KIND, [code])) {
    ...
    const body = validMemberBody(rec);
    if (!body) {
      continue;
    }
...
/**
 * Superseded names retained for this phone's exact stable self card.
 *
 * This is intentionally NOT announcedMembers(): current roster and sender
 * display stay newest-only. Mention arrival alone needs history because a
 * message composed before a rename can arrive afterward. Every candidate is
 * still a pod-member row for this normalized pod, still passes the normal
 * cardId/from_hash gate, and must name my exact card id — a hash collision or
 * a body forged under somebody else's envelope cannot become my alias. A
 * retained spelling also yields to any DIFFERENT current from_hash that owns
 * it; the self hash is exempt because its newest announcement can itself be
 * stale while the local card has already moved on.
 */
export function historicalSelfNames(
  crewCode: string,
  myCardId: string,
  currentName: string,
  ...
  const out: string[] = [];
  for (const rec of recordsOfKind(POD_MEMBER_KIND, [code])) {
    if (rec.from_hash !== mine) {
      continue;
    }
    const body = validMemberBody(rec);
    if (!body || body.cardId !== myCardId) {
      continue;
    }
    const folded = fold(body.name);
    if (
      folded === current ||
      ownedByAnother.has(folded) ||
      seen.has(folded)
    ) {
      continue;
    }
