...
export interface BoardBody {
  ...
  refId: string;
  /** The WRITER whose item this replies to, '' for an item or a legacy
   * body (integration contract: replies are qualified by (writer, id), so
   * two writers' same-id items can never cross-thread). Tolerated absent
   * on decode — a pre-format-3 phone's gossip still installs. */
  refWriterId: string;
  createdAt: string;
  done: boolean;
}
...
export function decodeBoardBody(body: string): BoardBody | null {
  ...
  return {
    ...
    refWriterId:
      typeof r.refWriterId === 'string'
        ? r.refWriterId.trim().slice(0, 32)
        : '',
...
const contentSignature = (b: BoardBody): string =>
  // Mirrors campBoard.postSignature field-for-field (one canonical
  // signature): authorName is mutable wire content too — a playa-name
  // change must republish old posts, or every carried copy keeps the old
  // name until its text happens to change.
  [
    b.type,
    b.text,
    b.refId,
  ...
  for (const code of codes) {
    ...
    for (const post of fresh) {
      const body: BoardBody = {
        ...
        text: post.text,
        refId: post.ref_id ?? '',
        refWriterId: post.ref_writer_id ?? '',
  ...
  for (const rec of recordsOfKind(BOARD_POST_KIND, crewCodes)) {
    ...
    incoming.push({
      ...
      post: {
        ...
        ref_writer_id: body.refWriterId.length > 0 ? body.refWriterId : null,
