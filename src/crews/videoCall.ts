...
export interface CallModel {
  ...
  micMuted: boolean;
  /** Which lens is capturing. Every call opens on the front camera
   * (callRuntime's getUserMedia asks facingMode 'user'), and Flip toggles
   * this — it is not a cosmetic flag: the SELF-VIEW's mirroring is read
   * off it (callSelfMirrored), and a stale answer here is the owner's
   * "pan right and it goes left". */
  frontCamera: boolean;
...
/**
 * THE MIRRORING RULE, in one place (owner report 2026-08-26, the first
 * real iPhone↔iPhone call: "the front camera is flipped so when you pan
 * right it goes left").
 *
 * A SELF-VIEW is a mirror and a REMOTE view is a window. Every calling app
 * on earth does this and campers have thirty years of habit behind it: you
 * expect your own face to move the way it moves in a bathroom mirror, and
 * you expect the person you called to look like a person, not like their
 * own reflection (mirrored remote video puts their lettering backwards).
 *
 * So: the remote RTCView is NEVER mirrored, and the self-view is mirrored
 * exactly while the FRONT lens is capturing. The rear lens is a window
 * too — it is pointed at the world, not at you — and mirroring it is the
 * disorientation the owner reported: pan right, the picture goes left.
 * The old panel hardcoded `mirror` on the self-view, so Flip left it
 * mirrored, and every rear-camera moment lied about which way the world
 * was moving.
 */
...
/**
 * The call-duration read-out, mm:ss until the hour and h:mm:ss after it.
 * Small, top centre, and honest about a call that has only just started
 * (0:00 for the first second — never a blank that reads as "not
 * connected"). Negative and nonsense clocks floor at zero rather than
 * rendering "-1:59" at somebody in the dust.
 */
export function callDurationCopy(ms: number): string {
  const total = Math.max(0, Math.floor((isFinite(ms) ? ms : 0) / 1000));
  const s = total % 60;
  ...
  switch (m.phase) {
    ...
    case 'live': {
      switch (e.type) {
        ...
        case 'flip-camera': {
          // The FLAG moves in the reducer and the LENS moves in the
          // runtime, from one event — so the self-view's mirroring
          // (callSelfMirrored) and the camera actually capturing can
          // never disagree. A runtime-only flip is exactly how the panel
          // came to mirror a rear-facing lens.
          const next = { ...m, frontCamera: !m.frontCamera };
          return { model: next, effects: [{ do: 'flip-camera' }] };
        }
        case 'toggle-mic': {
