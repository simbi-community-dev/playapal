 ...
 * central by, which on iOS is not the identifier we scan them under — so
 * this is a "somebody pulled" cue, never an address to dial.
 */
export function onSyncServed(cb: (s: { peerId: string }) => void): () => void {
  const sub = getEmitter().addListener(
    'CrewSyncServed',
    (e: { peerId?: string }) => {
