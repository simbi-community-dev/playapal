...
export function startWalkieSession(id: WalkieSessionId): Promise<void> {
  return enqueue(() => doStartWalkieSession(id));
}
...
async function doStopWalkieSession(): Promise<void> {
  if (!state.session && !runtime && !offPeers && !offCall && !walkieOn()) {
    // A queued duplicate — a second off tap, or the channel watcher filing
    // a stop behind one already running — reaches here after the real stop
    // finished and finds nothing standing. One stop is one stop. The
    // listeners are part of "standing": a claim already cleared by the
    // public verb must not skip the detach that still owes.
    return;
  }
  const rt = runtime;
  runtime = null;
