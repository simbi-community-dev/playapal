...
/**
 * Does this message name ME? `podNames` is the pod's whole CURRENT '@'
 * vocabulary as this phone knows it. `selfNames` is a separate target set:
 * current self-card name first, then validated retained names for that exact
 * stable card. Folding both into the matching pool preserves longest-first
 * across the whole vocabulary without promoting an old self name into the
 * current roster or sender display.
 */
export function mentionsMe(
  ...
  const mine = new Set(selfNames.map(n => fold(n.trim())).filter(Boolean));
  if (mine.size === 0) {
  ...
  return mentionedNames(text, [...podNames, ...selfNames]).some(n =>
    mine.has(fold(n.trim())),
  );
}
