 ...
 *
 * SCOPED BY CREW WHEN THE CALLER HAS A SCOPE, and that is not a nicety.
 * A want list is a stranger's write: whoever holds a connection to this
 * phone's GATT server names ids, and this read is what turns those names
 * into rows. Unscoped, "which ids may I ask for" is answered by the id
 * space alone — every row this phone carries for EVERY pod it is in,
 * including one the asking peer was never in. The digest a peer was
 * offered is already crew-scoped (syncDigest), so an honest want can only
 * ever name ids inside the scope; a want that names anything else was not
 * built from an offer we made.
 *
 * `crewCodes` is optional ONLY because the codec suites drive this store
 * directly in a single-crew world. The mesh always passes the scope it is
 * serving under — syncLink.serveMessages threads it from meshSync's own
 * crewCodes getter — and the arm that pins that is what keeps the
 * production path from quietly losing it.
 */
export function messagesByIds(
  ...
  const conn = getDb();
  const codes = crewCodes === undefined ? null : normalizeCodes(crewCodes);
  if (codes !== null && codes.length === 0) {
    // A scope was asked for and it is EMPTY: this phone is serving no pod
    // right now, so it serves no rows. Not the same as "no scope given".
    return [];
  }
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const res =
      codes === null
        ? conn.execute(
            `SELECT * FROM crew_messages WHERE id IN (${placeholders(
              slice.length,
            )})`,
            slice,
          )
        : conn.execute(
            `SELECT * FROM crew_messages
              WHERE id IN (${placeholders(slice.length)})
                AND crew_code IN (${placeholders(codes.length)})`,
            [...slice, ...codes],
          );
...
export function recordWants(ids: string[], nowMin: number): void {
  ...
  const conn = getDb();
  for (const id of ids) {
    stampWant(conn, id, nowMin);
  }
}

/**
 * One id's stamp, and the ONLY place a want row is written.
 *
 * Returns the exact row it wrote and the exact row it replaced — the two
 * halves an attempt needs to undo itself without guessing. Everything
 * else in this section (recordWants, the attempt below) goes through
 * here, so "what does a stamp look like" has one answer.
 */
function stampWant(conn: Conn, id: string, nowMin: number): {
  wrote: WantStamp;
  prior: WantStamp | null;
} {
  const res = conn.execute(
    'SELECT asked_min, tries, retry_min FROM crew_sync_wants WHERE id = ?',
    [id],
  );
  const row = rowsOf(res)[0];
  const prior: WantStamp | null = row
    ? {
        asked_min: Number(row.asked_min),
        tries: Number(row.tries),
        retry_min: Number(row.retry_min),
      }
    : null;
  const tries = prior ? prior.tries + 1 : 0;
  const wrote: WantStamp = {
    asked_min: nowMin,
    tries,
    retry_min: nowMin + backoffFor(tries),
  };
  conn.execute(
...
// ------------------------------------------------------- the want ATTEMPT
//
// THE STAMP NEEDS AN OWNER, AND THE OWNER NEEDS A PREIMAGE.
//
// recordWants stamps every id BEFORE the fetch goes out, deliberately: a
// peer that walks away mid-transfer must still count as a try. That makes
// the stamp a debt — wantsFrom filters a backed-off id straight out of the
// next want list — and a debt nobody answers for SUPPRESSES the id for the
// whole retry window. A session that dies mid-exchange is exactly that
// case: a dead pod's back-off standing over the pod that replaced it.
//
// THE FIRST CURE WAS NOT A ROLLBACK. It called forgiveWants, which is a
// COMMUTATION, not an undo: the row keeps its bumped tries, is re-armed at
// the two-minute base step rather than at whatever it had before, and —
// the part that makes it not a rollback at all — forgiveWants is a no-op
// once tries passes FORGIVE_TRIES_CEILING, so an id with real refusal
// history kept a back-off it had not earned this pass, up to six hours.
//
// SO AN ATTEMPT CARRIES ITS OWN PREIMAGE, and the undo is an exact CAS.
// Opening one reads each id's row, writes the stamp, and keeps BOTH — the
// row that was there and the row it wrote — inside ONE transaction, so a
// DB that fails halfway leaves no stamps at all and no half-attempt in
// this map to roll back later. Exactly one terminal ever runs per attempt:
//
//   COMMIT   — the exchange landed. clearWants for what actually arrived;
//              the misses keep the back-off the ledger exists to charge.
//   FORGIVE  — a LIVE transport failure. The strike stands, the sentence
//              is commuted (forgiveWants, unchanged).
//   ROLL BACK— the session died. Each id is restored to its exact prior
//              row, and ONLY IF the row is still byte-identical to what
//              this attempt wrote: a later attempt that re-stamped the
//              same id owns it now, and a rollback that clobbered that
//              would erase a live pass's debt.
//
// A terminal on an attempt that is already closed does nothing, so no
// terminal can run twice and a stale attempt can never speak for a live
// one.

/** One want row, as this ledger writes and restores it. */
export type WantStamp = {
  asked_min: number;
  tries: number;
  retry_min: number;
};
...
/**
 * A tokened want stamp with its preimage. Opaque to the conductor: it
 * carries the ids the pass asked for and the two rows per id that make an
 * exact undo possible.
 */
export type WantAttempt = {
  token: number;
  ids: string[];
  wrote: Map<string, WantStamp>;
  prior: Map<string, WantStamp | null>;
};

let wantAttemptSeq = 0;
/** The attempts whose terminal has NOT run yet — the checkable half of
 * "its own stamp". An attempt leaves this map at its one terminal. */
const openWantAttempts = new Set<number>();

/**
 * Stamp these ids as one owned attempt. Throws only if the DB does, and a
 * throw leaves NOTHING behind: the transaction rolls the partial stamps
 * back and the token was never registered, so there is no attempt for a
 * later terminal to act on.
 */
export function openWantAttempt(ids: string[], nowMin: number): WantAttempt {
  wantAttemptSeq += 1;
  const attempt: WantAttempt = {
    token: wantAttemptSeq,
    ids: [...ids],
    wrote: new Map(),
    prior: new Map(),
  };
  if (attempt.ids.length > 0) {
    ...
    inTransaction(conn, () => {
      for (const id of attempt.ids) {
        const { wrote, prior } = stampWant(conn, id, nowMin);
        attempt.wrote.set(id, wrote);
        attempt.prior.set(id, prior);
      }
    });
  }
  // Registered only after the write COMMITTED: a half-written attempt is
  // not an attempt, and a token in this map for stamps that never landed
  // is the map leak the transaction exists to refuse.
  openWantAttempts.add(attempt.token);
  return attempt;
}

/** The exchange stayed live to the end: ack what landed, and leave every
 * id that did not arrive holding the back-off it earned. */
export function commitWantAttempt(
  attempt: WantAttempt,
  landedIds: string[],
): void {
  if (!openWantAttempts.delete(attempt.token)) {
    return;
  }
  clearWants(landedIds);
}

/** A LIVE transport failure: the connection died before the peer said
 * anything about any id, so the strike stands and the sentence is
 * commuted. */
export function forgiveWantAttempt(
  attempt: WantAttempt,
  nowMin: number,
): void {
  if (!openWantAttempts.delete(attempt.token)) {
  ...
  forgiveWants(attempt.ids, nowMin);
}

/**
 * The session died: put every id back exactly as this attempt found it.
 *
 * The CAS is the whole guarantee. An id whose row no longer matches what
 * this attempt wrote has been re-stamped by a LATER attempt — a live pass
 * that owns the debt now — and is left untouched. Returns how many rows
 * were actually restored, so an arm can tell a real rollback from a
 * no-op.
 */
export function rollBackWantAttempt(attempt: WantAttempt): number {
  if (!openWantAttempts.delete(attempt.token)) {
    return 0;
  }
  if (attempt.ids.length === 0) {
  ...
  return inTransaction(conn, () => {
    let restored = 0;
    for (const id of attempt.ids) {
      const wrote = attempt.wrote.get(id);
      if (wrote === undefined) {
        continue;
      }
      const before = attempt.prior.get(id) ?? null;
      const cas = [id, wrote.asked_min, wrote.tries, wrote.retry_min];
      // THE COMPARE, stated as its own read rather than inferred from an
      // UPDATE's row count: the engines behind this Conn do not agree on
      // what an execute() reports, and "how many rows changed" is the one
      // number this decision turns on.
      const still = conn.execute(
        `SELECT id FROM crew_sync_wants
          WHERE id = ? AND asked_min = ? AND tries = ? AND retry_min = ?`,
        cas,
      );
      if (rowsOf(still).length === 0) {
        continue; // a newer attempt owns this id — it is not ours to undo
      }
      if (before === null) {
        conn.execute(
          `DELETE FROM crew_sync_wants
            WHERE id = ? AND asked_min = ? AND tries = ? AND retry_min = ?`,
          cas,
        );
      } else {
        conn.execute(
          `UPDATE crew_sync_wants
              SET asked_min = ?, tries = ?, retry_min = ?
            WHERE id = ? AND asked_min = ? AND tries = ? AND retry_min = ?`,
          [before.asked_min, before.tries, before.retry_min, ...cas],
        );
      }
      restored += 1;
    }
