 ...
 *
 * NAMING SOMEONE (owner ask, 2026-08-26: "if kupo is in a pod with me, and
 * i type @kupo in chat, it would send her phone a real buzz"). Typing '@'
 * offers the pod's roster as chips above the input; tapping one inserts
 * the name as PLAIN TEXT, so the wire format is untouched and a phone
 * running an older build renders the sentence exactly as typed. The buzz
 * itself is minted on the RECIPIENT'S phone when the message lands
 * (src/crews/pocketAlerts.ts) — real, and on the radio's schedule, which
 * is what the caption under the chips says out loud.
 *
...
import { applyMention, mentionSuggestions } from './mentions';
...
import { PIN_SLACK_PT, colors, radius, spacing, tap, type } from '../theme';
...
// PIN_SLACK_PT is shared from the theme so every auto-scrolling list
// agrees about what "at the bottom" means.
...
export function PodMessages({
  ...
  /**
   * THE POD'S '@' VOCABULARY — who this phone can offer while a thumb
   * types a mention (src/crews/mentions.ts). Narrower than `names` above
   * on purpose: that map names anyone whose card is on this phone, so a
   * friend from another camp would be offered as a podmate and mentioning
   * them would buzz nobody. This is the ROSTER — announced members of THIS
   * pod, plus picked cards — under podRoster's own rule (card spelling
   * wins) and minus me, because nobody @s themselves.
   */
  const podNames = useMemo(() => {
    const roster = announcedNames(crew.code);
    const picked = new Set(crew.memberIds);
    for (const f of friends) {
      if (picked.has(f.id) || roster.has(hash32(f.id))) {
        roster.set(hash32(f.id), f.name);
      }
    }
    roster.delete(hash32(me.id));
    return [...roster.values()];
    // msgRev again: an announcement arriving over the mesh is what makes a
    // brand-new podmate mentionable.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [crew.code, crew.memberIds, friends, me.id, msgRev]);

  // Thread reads ride the SAME invalidation signal as the name maps above:
  // every writer bumps msgRev (compose, markRead, prune, radio accepts).
  // Unmemoized, three synchronous SQL queries — two SELECT * whose rows
  // carry voice-note bodies up to 256 KiB — plus the sort re-ran on every
  // keystroke of the draft box.
  const { thread, unread } = useMemo(() => {
    const codes = [crew.code];
    const heard = inbox(codes, me.id);
    const mine = myOutbox(codes, me.id);
    return {
      thread: [...heard, ...mine].sort(oldestFirst),
      unread: unreadCount(codes, me.id),
    };
    // msgRev is the store's revision signal, same as the memos above.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  /**
   * WHO THE '@' IS OFFERING RIGHT NOW. Derived from the draft, never
   * stored: there is no "mention mode" to get stuck in, so a draft the
   * camper edits, clears or sends can never leave a stale row hanging over
   * the composer.
   *
   * A typed name is PLAIN TEXT on the wire (mentions.ts) — this row is a
   * spelling aid, not a token editor, so nothing here changes what gets
   * sent versus what a camper types by hand.
   */
  const suggestions = useMemo(
    () => mentionSuggestions(draft, podNames),
    [draft, podNames],
  );
      ...
      {open ? (
        <>
          ...
          {suggestions.length > 0 ? (
            // THE '@' ROW (owner ask, 2026-08-26: "if kupo is in a pod with
            // me, and i type @kupo in chat, it would send her phone a real
            // buzz"). House idiom, not a library: a scrolling row of chips
            // above the input, each at the 44pt tap floor, tapping one
            // completes the name as plain text.
            //
            // The caption states the mesh truth at the moment it is being
            // relied on — the buzz is REAL, its timing belongs to the
            // radio. Nowhere in this app may a mention imply a phone can be
            // reached from across camp on demand.
            <View>
              <ScrollView
                ...
                {suggestions.map(n => (
                  <Pressable
                    ...
                    onPress={() => setDraft(d => applyMention(d, n))}
                    accessibilityRole="button"
                    accessibilityLabel={`Mention ${n} — buzzes their phone when the message reaches them`}>
                    <Text style={styles.mentionChipText}>@{n}</Text>
                  </Pressable>
                ))}
              </ScrollView>
              <Text style={styles.mentionHint}>
                Naming someone buzzes their phone — when the message reaches
                it.
              </Text>
            </View>
          ) : null}
          ...
          <View style={styles.footerInfo}>
            <InfoTap
              ...
              text={
                ...
                'phone at camp keeps the mailbox. Put an @ before a ' +
                'podmate’s name and their phone buzzes for it — at the ' +
                'moment the message reaches them, not before.'
              }
            />
          </View>
        </>
...
const styles = StyleSheet.create({
  ...
  // The '@' row sits directly on the card, above the composer — no
  // surface of its own, so the chips are the only thing that reads as
  // tappable. maxHeight keeps a long roster from growing the card.
  mentionRow: { marginTop: spacing.sm },
  mentionChip: {
    ...tap, // gloves and dust, same 44pt floor as every other target here
    alignItems: 'center',
    backgroundColor: colors.field,
    borderRadius: radius.chip,
    justifyContent: 'center',
