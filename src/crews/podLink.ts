...
/** Same ceiling the friend and beam decoders use — a hostile fragment never
 * gets to allocate against the phone before it is rejected. */
...
/** Pod names are clamped at 40 in crew.ts; the same clamp here is what
 * guarantees a name can never be the reason an invite overflows one QR. */
const NAME_MAX = 40;
