...
export function CrewSection({
  ...
  // THE MINI-BAR'S TAP HAS TO LAND ON THE RIGHT POD. The walkie session
  // names the pod whose channel is open, and the camper can be looking at a
  // different pod's card when they tap "tap to open" from another tab. So
  // asking for the stage also brings this card to that pod — otherwise the
  // tap opens a card whose walkie is honestly OFF, which reads as the app
  // losing the channel it just promised was running.
        ...
        {picking ? (
          ...
          <View>
                    ...
                    if (!down) {
                      return (
                        <Text style={styles.shareHint}>
                          ...
                          share while the app is open. Messages and voice
            ...
            {/* key={crew.id}: switching pods REMOUNTS both strips, so the
                thread, composer draft, panel-local talk state and unread
                state can never bleed across pods (composed review, Aug 24).
                It no longer closes the WALKIE — the channel is owned above
                this card now (walkieSession.ts), and hanging it up because
                someone glanced at their other pod was the same defect as
                hanging it up because they glanced at the map. The panel
                takes crew.id so every "walkie is on" claim on this card is
                about THIS pod's channel and not some other pod's. */}
        ...
        <Pressable
          ...
          <View
            ...
            <Text style={styles.swapHint}>
              They point a camera at this — theirs or the Scan button below —
              and your card lands on their phone. No signal, no Bluetooth, no
              server: you can see whose card you are taking.
            </Text>
            <View style={styles.inviteRow}>
              <Pressable
                onPress={scanTheirs}
            ...
            <Text style={styles.swapHint}>
              Not together? "Send my card" works over any messenger — and
              opens a web page with your card for anyone who doesn't have the
              app.
            </Text>
            <Pressable
              onPress={() => setSwapWith(null)}
