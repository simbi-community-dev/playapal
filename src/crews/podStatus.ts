 ...
 *
 * AND A CANCELLED CHECK HAS NOTHING TO SAY — null, not a sentence.
 *
 * checkPodUpdates answers `cancelled` when the pod it was asked about
 * stopped existing while its dials were on the radio (a background bounce
 * under the camper's own thumb is the ordinary way there). Its `inRange` and
 * `moved` are both 0 and mean NOTHING — they are not a measurement of an
 * empty pod, they are the absence of a measurement — so the zero-in-range
 * branch below would turn "we could not ask" into "Nobody in range right
 * now", which is a claim about the pod, stated to someone standing next to
 * their podmate. The honest surface for a superseded gesture is no surface
 * at all: null, and PodLinks renders nothing.
 */
