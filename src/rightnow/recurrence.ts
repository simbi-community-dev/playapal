...
export function recurrenceCounts(rows: EventRow[]): Map<string, number> {
  ...
  for (const ev of rows) {
    // A camper-authored note is one human statement, never a series:
    // recurrence inference reads pack listings only (CAMP-NOTES ruling E).
    if ((ev as { source_kind?: string }).source_kind === 'camp_note') {
      continue;
    }
