...
const LIMITATION_COPY: Record<LimitationTopic, { title: string; body: string }> = {
  ...
  'video-call-link': {
    ...
    body:
      ...
      'more room than that. Put both phones on any shared Wi-Fi — a hotspot ' +
      'with no internet behind it counts — and the call carries. With no ' +
      'Wi-Fi anywhere, a call rings and then says it could not get through: ' +
      'that is the current honest limit (field-measured), and the walkie ' +
      'and voice notes are the tools that work there. A podmate you can ' +
  ...
  // Truth: docs/WALKIE-LADDER.md §11 "What I could not measure" as of
  // 2026-08-25 — the direct link IS measured both ways on two phones with
  // every network off; voice over Bluetooth and the call layer are not.
  'device-proof': {
    title: 'Some of the newest pieces have not met two phones yet',
    body:
      'Live voice between two phones over their own direct link is measured ' +
      'and real — both directions, every network switched off. Voice over ' +
      'Bluetooth and the video call are built and tested, but no two phones ' +
      'have yet carried a real conversation on them in dust. If one of those ' +
      'two does not work at camp, that is the new part; messages, positions ' +
...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'Better links do more. Live voice works over Bluetooth too, rougher, and ' +
        'the app marks it lo-fi so your ears are not surprised. On a shared ' +
        'camp Wi-Fi, or between two newer phones that link straight to each ' +
        'other with no router at all, voice is clean and a video call becomes ' +
        'possible.',
