...
const LIMITATION_COPY: Record<LimitationTopic, { title: string; body: string }> = {
  ...
  'video-call-link': {
    title: 'Video calls need a full-quality link',
    body:
      'The walkie carries voice with no network at all; a video call needs ' +
      'more room than that. Two ways to have it: put both phones on any ' +
      'shared Wi-Fi and the call carries — an iPhone and an Android ' +
      'included, measured between the two, and a hotspot with no internet ' +
      'behind it counts — or, for two iPhones that have done Link iPhones ' +
      'directly, the call carries with no Wi-Fi at all (field-measured, ' +
      'full quality, the linked phones talk straight to each other). ' +
      'Otherwise, with no Wi-Fi anywhere, a call rings and then says it ' +
      'could not get through (field-measured) — so one Android in the pod ' +
      'can make the Wi-Fi itself: turn on Camp hotspot under your pod and ' +
      'the others join by pointing a camera at the code it shows. Nothing ' +
      'there touches the internet. A podmate you can ' +
