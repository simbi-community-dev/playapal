...
export function OnboardingFlow({ onDone }: Props) {
  ...
  if (step === 'name') {
    return (
      <View style={styles.container}>
        ...
        {dots}
        <Pressable style={styles.primary} onPress={() => setStep('camp')}>
          <Text style={styles.primaryText}>Next</Text>
        </Pressable>
        <Pressable
          style={styles.skip}
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.skip} onPress={() => finish({ name })}>
        <Text style={styles.skipText}>Skip</Text>
      </Pressable>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  campRowTextActive: { color: colors.cream, fontWeight: '700' },
  dotRow: {
    flexDirection: 'row',
  ...
  primaryText: { color: colors.cream, fontSize: type.body, fontWeight: '800' },
