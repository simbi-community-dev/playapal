...
// Stated once in src/util/base64.ts, which explains why this tree has two
// alphabets and why merging them corrupts one side or the other.
import { B64_STANDARD as B64_ALPHABET } from '../util/base64';
...
/** Every note kind, in display order. EXPORTED so the compose sheet offers
 * exactly the kinds this module accepts — a second list is how a kind gets
 * added here and silently stays unofferable there. */
export const KINDS: NoteKind[] = ['memory', 'event', 'fix', 'resource', 'art'];
...
/**
 * The WIRE read of the same rows — kind AS STORED, never coerced (review
 * batch 4.3). fromRow's display coercion maps an unknown future kind to
 * 'memory' for screens; sealing through THAT read re-canonicalizes the
 * bytes, so a downgrade-and-restore cycle resealed the same seq with
 * changed content (a self-fork), and a future build's note came back to
 * it permanently flattened. What the sender sealed is what this phone
 * reseals: the WireNote lesson, applied to our own storage.
 */
export function ownWireNoteRows(
  conn: QuickSQLiteConnection,
  campId: string,
  writerId: string,
): WireNote[] {
  ...
  return ((res.rows?._array ?? []) as NoteRow[]).map(r => ({
    ...fromRow(r),
    kind: r.kind,
  }));
}
...
export function parseWireNotes(raw: unknown, writerId: string, authorName: string): WireNote[] {
  ...
  return raw.map((r: any): WireNote => {
    ...
    // The id keeps its ORIGIN-writer prefix for life — incarnation rotation
    // deliberately does not rewrite note ids (campBoard rotation: supersedes
    // chains reference them), so the sealing writer and the id prefix
    // legitimately diverge after a restore-clone rotation. Authenticity is
    // the ENVELOPE's job (the HMAC binds every note to the sealing writer);
    // this check only refuses ids that are not note ids at all.
    if (!/^[^:\s]{1,32}:.+$/.test(id) || id.length > 64) {
      throw new CampNoteError('note id is not a writer-prefixed note id');
    }
...
// One bulk read per rebuild, not one point query per note (the set-based
// pattern rematerializeAllBoards already uses): a writer can carry up to
// MAX_NOTES_PER_WRITER point queries otherwise, inside one UI-blocking
// transaction. NOT facts/hiddenItems' hiddenKeys — importing it here would
// close a module cycle (hiddenItems already imports rematerializeNotes).
const hiddenNoteIds = (conn: QuickSQLiteConnection): Set<string> => {
  const r = conn.execute(
    "SELECT key FROM hidden_items WHERE kind = 'camp_note'",
  );
  return new Set(
    ((r.rows?._array ?? []) as { key: string }[]).map(row => row.key),
  );
};
...
/**
 * A FORK's notes, projected read-only under the fork pack (review batch
 * 4.1). camp_forks.envelope_json is the fork's full sealed content, and
 * until this pass only its POSTS had a read consumer — a notes-only fork
 * installed a "(conflicted copy)" pack whose entire divergence was
 * invisible. Notes never enter camp_notes (the canonical store keys on
 * writer_id and would collide with the accepted variant) and never enter
 * events — a conflicted snapshot is not calendar authority, and a
 * duplicate structured row would poison the deterministic event
 * pipeline's counts and ambiguity. Searchable text under the fork pack is
 * the resolution surface. The note-id hide key still applies — one key
 * across every surface, and both copies of a note share their origin id.
 * Caller has already swept this pack's doc_chunks; supersession is
 * resolved WITHIN the fork's own list (a snapshot's internal structure),
 * never across copies.
 */
export function projectForkNotes(
  conn: QuickSQLiteConnection,
  packId: string,
  notes: readonly WireNote[],
): void {
  const superseded = new Set(notes.map(n => n.supersedes).filter(Boolean));
  const hidden = hiddenNoteIds(conn);
  for (const w of notes) {
    if (superseded.has(w.id) || hidden.has(w.id)) {
      continue;
    }
    const n: CampNote = { ...w, kind: asKind(w.kind) };
    const when =
      n.kind === 'event' && n.when_date
        ? ` on ${n.when_date}${n.time_start ? ` at ${n.time_start}` : ''}`
        : '';
    const heading = `${n.title || `${kindWord[n.kind]} from ${n.author_name}`} (conflicted copy)`;
    const body =
      `${kindWord[n.kind]}${when}${n.where_addr ? ` at ${n.where_addr}` : ''}` +
      `${n.year ? ` (about ${n.year})` : ''}: ${n.text} ` +
      `(recorded by ${n.author_name}, ${n.created_at.slice(0, 10)}; from a ` +
      `conflicted copy of their board — camp-passphrase verified, not authenticated)`;
    conn.execute(
      'INSERT INTO doc_chunks (pack_id, source_file, heading, content, note_key) VALUES (?, ?, ?, ?, ?)',
      [packId, 'camp-notes', heading, body, n.id],
    );
