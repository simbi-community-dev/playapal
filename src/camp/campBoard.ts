...
function refreshOwnPackRowOnly(conn: QuickSQLiteConnection): void {
  ...
  upsertBoardPackRow(conn, {
    ...
    name: `Camp board — ${displayName(identity.authorName)} (this phone)`,
...
export function installCampBundle(
  ...
  const installFork = (v: VerifiedEnvelope): void => {
    ...
    upsertBoardPackRow(conn, {
      ...
      description: `Two versions of ${author}'s board carry revision ${v.env.seq} with different contents. Both are shown; remove the stale one under Camp > Campmates' boards once resolved.`,
      version: v.env.seq,
    });
    conn.execute(
