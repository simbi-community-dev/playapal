...
export function installCampBundle(
  ...
  {
    // Export-closure: this phone re-beams own + every stored campmate, so
    // stored foreign writers must stay under MAX_BEAM_ENVELOPES - 1 or the
    // next beam this phone makes would be rejected by every receiver.
...
export function upsertCampNote(
  ...
  // N+1 at authoring keeps every beam this phone makes importable.
  if (!input.id) {
