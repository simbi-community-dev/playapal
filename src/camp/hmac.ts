/**
 * Self-contained SHA-256 + HMAC-SHA256 (hex) for the camp-data beam seal.
 *
 * Why hand-rolled: the app ships NO crypto dependency (react-native-quick-
 * crypto is not installed; node_modules only carries transitive sha.js,
 * which is not ours to depend on), Hermes has no WebCrypto, and the threat
 * model is doc 30 Axis 4A — accidental cross-camp imports and casual
 * tampering, not nation-states. ~100 lines of the textbook FIPS 180-4
 * algorithm, pinned by the RFC 4231 test vectors in __tests__/campHmac.
 * Pure functions, no state, no I/O.
 */

/* eslint-disable no-bitwise */

// FIPS 180-4 round constants (first 32 bits of the fractional parts of the
// cube roots of the first 64 primes).
const K = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
  0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
  0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
  0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
  0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
  0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

/**
 * UTF-8 encode without relying on TextEncoder (Hermes coverage varies).
 *
 * DO NOT "FIX" THE LONE-SURROGATE BEHAVIOUR HERE. This encoder emits CESU-8
 * (ED A0 80) for a lone surrogate rather than the U+FFFD the platform would
 * produce. That IS a defect — its siblings in friendLink.ts and callSignal.ts
 * were corrected, because their bytes are a wire format that gets re-decoded
 * and our own strict decoder refused them.
 *
 * These bytes are different: they are the INPUT TO A DIGEST. sha256Hex over
 * this encoder is what derives CAMP IDENTITY — campIdFor / keyIdFor in
 * campBoard.ts hash the normalized passphrase, and normalizePassphrase does
 * not strip surrogates. Change the bytes and a camper whose passphrase carries
 * one truncated emoji derives a DIFFERENT camp id on the new version: their
 * phone and their campmates' phones compute different identities from the same
 * typed words, the camp silently splits in two, and nothing throws.
 *
 * __tests__/campIdentityFrozen.test.ts pins the derived ids and fails naming
 * that consequence. If camp identity must ever move, the change is a
 * version-gated migration and that suite is its receipt — not a table to
 * update.
 */
export function utf8Bytes(s: string): Uint8Array {
  const out: number[] = [];
  for (let i = 0; i < s.length; i++) {
    let cp = s.codePointAt(i)!;
    if (cp > 0xffff) {
      i++; // consumed a surrogate pair
    }
    if (cp < 0x80) {
      out.push(cp);
    } else if (cp < 0x800) {
      out.push(0xc0 | (cp >> 6), 0x80 | (cp & 0x3f));
    } else if (cp < 0x10000) {
      out.push(0xe0 | (cp >> 12), 0x80 | ((cp >> 6) & 0x3f), 0x80 | (cp & 0x3f));
    } else {
      out.push(
        0xf0 | (cp >> 18),
        0x80 | ((cp >> 12) & 0x3f),
        0x80 | ((cp >> 6) & 0x3f),
        0x80 | (cp & 0x3f),
      );
    }
  }
  return Uint8Array.from(out);
}

const rotr = (x: number, n: number): number => (x >>> n) | (x << (32 - n));

export function sha256(data: Uint8Array): Uint8Array {
  const H = new Uint32Array([
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c,
    0x1f83d9ab, 0x5be0cd19,
  ]);
  const len = data.length;
  // Pad to a 64-byte boundary with room for the 0x80 marker + 64-bit length.
  const padded = new Uint8Array((((len + 8) >> 6) + 1) << 6);
  padded.set(data);
  padded[len] = 0x80;
  const dv = new DataView(padded.buffer);
  dv.setUint32(padded.length - 8, Math.floor(len / 0x20000000));
  dv.setUint32(padded.length - 4, (len << 3) >>> 0);

  const w = new Uint32Array(64);
  for (let off = 0; off < padded.length; off += 64) {
    for (let i = 0; i < 16; i++) {
      w[i] = dv.getUint32(off + i * 4);
    }
    for (let i = 16; i < 64; i++) {
      const s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >>> 3);
      const s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >>> 10);
      w[i] = (w[i - 16] + s0 + w[i - 7] + s1) >>> 0;
    }
    let a = H[0], b = H[1], c = H[2], d = H[3];
    let e = H[4], f = H[5], g = H[6], h = H[7];
    for (let i = 0; i < 64; i++) {
      const t1 =
        (h +
          (rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)) +
          ((e & f) ^ (~e & g)) +
          K[i] +
          w[i]) >>>
        0;
      const t2 =
        ((rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)) +
          ((a & b) ^ (a & c) ^ (b & c))) >>>
        0;
      h = g; g = f; f = e; e = (d + t1) >>> 0;
      d = c; c = b; b = a; a = (t1 + t2) >>> 0;
    }
    H[0] = (H[0] + a) >>> 0; H[1] = (H[1] + b) >>> 0;
    H[2] = (H[2] + c) >>> 0; H[3] = (H[3] + d) >>> 0;
    H[4] = (H[4] + e) >>> 0; H[5] = (H[5] + f) >>> 0;
    H[6] = (H[6] + g) >>> 0; H[7] = (H[7] + h) >>> 0;
  }

  const out = new Uint8Array(32);
  for (let i = 0; i < 8; i++) {
    out[i * 4] = H[i] >>> 24;
    out[i * 4 + 1] = (H[i] >>> 16) & 0xff;
    out[i * 4 + 2] = (H[i] >>> 8) & 0xff;
    out[i * 4 + 3] = H[i] & 0xff;
  }
  return out;
}

const concat = (a: Uint8Array, b: Uint8Array): Uint8Array => {
  const out = new Uint8Array(a.length + b.length);
  out.set(a);
  out.set(b, a.length);
  return out;
};

const toHex = (bytes: Uint8Array): string =>
  Array.from(bytes, x => x.toString(16).padStart(2, '0')).join('');

export function sha256Hex(message: string): string {
  return toHex(sha256(utf8Bytes(message)));
}

/** HMAC-SHA256 over UTF-8 strings, lowercase hex output (RFC 2104 / 4231). */
export function hmacSha256Hex(key: string, message: string): string {
  let k = utf8Bytes(key);
  if (k.length > 64) {
    k = sha256(k);
  }
  const ipad = new Uint8Array(64).fill(0x36);
  const opad = new Uint8Array(64).fill(0x5c);
  for (let i = 0; i < k.length; i++) {
    ipad[i] ^= k[i];
    opad[i] ^= k[i];
  }
  const inner = sha256(concat(ipad, utf8Bytes(message)));
  return toHex(sha256(concat(opad, inner)));
}

/** Constant-time-ish hex comparison (cheap; the threat is casual anyway). */
export function digestsEqual(a: string, b: string): boolean {
  if (a.length !== b.length) {
    return false;
  }
  let diff = 0;
  for (let i = 0; i < a.length; i++) {
    diff |= a.charCodeAt(i) ^ b.charCodeAt(i);
  }
  return diff === 0;
}
