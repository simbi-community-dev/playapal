  ...
  try {
    const res = conn.execute(
      `SELECT m.chunk_id, vec_distance_cosine(v.embedding, ?) AS distance
       ...
       WHERE m.model = ?` + (packId ? ' AND d.pack_id = ?' : '') +
        ' ORDER BY distance LIMIT ?',
      packId
        ? [JSON.stringify(queryVector), EMBEDDER_MODEL_ID, packId, limit]
        : [JSON.stringify(queryVector), EMBEDDER_MODEL_ID, limit],
    );
...
export async function searchDocsSemantic(
  ...
  const vectorIds = vectorRankedIds(conn, qv, packId, limit);
  ...
  const fused = rowsByIds(conn, fusedIds);
...
export function searchDocs(args: SearchDocsArgs, limit = 5): DocSearchOutcome {
  ...
  const conn = getDb();
  const pinned = aboutPin(conn, terms, packId, limit);
  if (pinned) {
    return visibleOutcome(conn, pinned);
  }
  ...
  const found = collectLadder<DocStrategy, DocRow>(
    conn,
    docRungs(terms, packId, packIdPrefix, limit, expand, prefix),
    limit,
  );
  ...
  if (shrunk) {
    const rescue = collectLadder<'fts-prefix' | 'like-prefix', DocRow>(
      ...
      [
        ...
        { q: buildDocsLikeQuery({ terms: shrunk, mode: 'or', packId, packIdPrefix, limit }), strategy: 'like-prefix' as const },
      ],
      limit,
    );
