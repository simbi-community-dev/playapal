...
import { distributionChannel } from '../config/distribution';
...
export type UpdateReason =
  ...
  | 'ios'
  /** This copy came from the Play Store, whose own policy forbids an app
   * that installs its own APK — so the Store is the update channel here.
   * Permanent for this install, and not a fault. */
  | 'store-build'
...
export function updateReasonCopy(reason: UpdateReason): string {
  switch (reason) {
    ...
    case 'store-build':
      return 'This copy came from the Play Store, so its updates come from there too — open Playa Pal in the Store to get the newest build.';
    case 'offline':
...
export async function checkLatestRelease(): Promise<CheckResult> {
  // THE STORE BUILD NEVER REACHES GITHUB. UpdateRow does not render at all
  // on that flavor, so nothing should arrive here — which is exactly why the
  // refusal belongs here too. A row hidden by a component is one edit away
  // from being visible again; a network call that refuses itself is not, and
  // "the app phones GitHub for an APK" is the sentence a Play policy review
  // would be reading.
  if (distributionChannel() === 'play') {
    return { ok: false, reason: 'store-build' };
  }
...
export async function downloadAndInstall(): Promise<DownloadResult> {
  // Belt to the manifest's braces: the play flavor has no
  // REQUEST_INSTALL_PACKAGES, so the native half could not open the
  // installer even if it were asked. It is not asked.
  if (distributionChannel() === 'play') {
    return { ok: false, reason: 'store-build' };
  }
  if (!native) {
