...
import { fixNotesForSubject } from '../camp/campNotes';
import { getCampIdentity } from '../camp/campBoard';
...
export interface ToolResult {
  json: string;
  cards: ChatCard[];
  /** Authoritative metadata for one search_events execution, including an
   * explicit zero result and whether the requested window was relaxed. */
...
export interface ToolOutcome extends ToolResult {
  ...
  shrink: (limit: number) => Promise<ToolResult>;
  ...
  switch (call.function.name) {
    case 'search_events': {
      const run = async (limit: number) => {
        ...
        const eventSearch: EventSearchOutcome = outcome.state === 'matches'
          ? { ...outcome, results: outcome.results.slice(0, limit) }
          : outcome;
        return {
          json: toolResultJson(eventSearch),
          cards: eventSearch.results.map(event => ({ kind: 'event' as const, event })),
          eventSearch,
        };
      };
    ...
    case 'lookup_facts': {
      ...
      const run = async (limit: number) => {
        ...
        // Ruling C (CAMP-NOTES-DESIGN): corrections attach by TYPED subject
        // and splice DIRECTLY after the base evidence — never by ranking
        // luck. The person card's own passage resolves to its graph id via
        // person_card_chunks (the deterministic identity index), and every
        // visible fix note for that person rides the tool result labeled as
        // what it is: a camp note, passphrase-verified, not the base pack.
        let json = absent ? noCoverageJson(absent) : factsResultJson(outcome);
        if (person) {
          try {
            const conn = getDb();
            const passage = outcome.results.find(
              r => r.pack_id === person.pack_id && r.heading.includes(person.name),
            ) ?? outcome.results[0];
            const pidRes = conn.execute(
              'SELECT person_id FROM person_card_chunks WHERE pack_id = ? AND chunk_id = ?',
              [passage.pack_id, passage.id],
            );
            const personId =
              pidRes.rows && pidRes.rows.length > 0
                ? String(pidRes.rows.item(0).person_id)
                : null;
            if (personId) {
              const fixes = fixNotesForSubject(
                conn,
                getCampIdentity(conn).campId,
                'person',
                `${passage.pack_id}|${personId}`,
              );
              if (fixes.length > 0) {
                const base = JSON.parse(json);
                base.corrections = fixes.map(f => ({
                  camp_note: f.text,
                  recorded_by: f.author_name,
                  recorded_at: f.created_at.slice(0, 10),
                  trust: 'camp-passphrase verified, not authenticated',
                }));
                base.corrections_note =
                  'campers recorded these fixes about this person; prefer them where they contradict the base pack, and say who recorded them';
                json = JSON.stringify(base);
              }
            }
          } catch {
            // Adjacency is additive: a splice failure must never break the
            // base lookup.
          }
        }
        return {
