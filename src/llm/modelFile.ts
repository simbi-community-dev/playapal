...
export async function pickModel(): Promise<string | null> {
  ...
  return copy.localUri.replace(/^file:\/\//, '');
}

/** Persist only a model path whose native context published successfully.
 * Returns the previously remembered path so a successful replacement can
 * reclaim its now-unreachable managed copy. */
export function rememberModel(path: string): string | null {
  const previous = getSetting(SETTING_KEY);
  setSetting(SETTING_KEY, path);
  return previous;
}

/** Remove a no-longer-resident copy created in the app documents directory. */
export async function discardUnpublishedModel(path: string): Promise<void> {
  const managedPrefix = `${DocumentDirectoryPath}/`;
  if (
    path === DEV_MODEL_PATH ||
    !path.startsWith(managedPrefix) ||
    getSetting(SETTING_KEY) === path
  ) {
    return;
  }
  try {
    ...
    console.warn('[model] failed to remove unpublished model copy:', error);
  }
}
