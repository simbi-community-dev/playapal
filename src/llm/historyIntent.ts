 ...
 */
const CLAUSE_SPLIT =
  /\s*(?:,|;|\band\b|\bthen\b)\s+(?=(?:who|whom|what|which|when|where|how|did|does|do|has|have|had|is|are|was|were|show|list|find)\b)/i;
...
export function historyToolArgs(
  ...
  const whole = historyToolArgsOneClause(text, anchor);
  if (whole) {
    return whole;
  }
  ...
  for (let i = 0; i < clauses.length; i++) {
    // A pronoun in clause i resolves to the nearest earlier clause's topic
    // when the session has no anchor; the session anchor still wins when it
    // exists (a follow-up turn about the person the app last resolved).
    let localAnchor = anchor;
    if (!localAnchor) {
      for (let j = i - 1; j >= 0 && !localAnchor; j--) {
        localAnchor = clauseTopic(clauses[j]);
      }
    }
    const args = historyToolArgsOneClause(clauses[i], localAnchor);
    if (args && !('entity' in args && PRONOUN.test(args.entity))) {
      return args;
    }
  }
  return null;
