/**
 * Persona layer: three switchable system prompts on the ONE resident model
 * (owner ruling 2026-08-13: personas are system prompts + per-persona saved
 * sessions on the same loaded GGUF — never separate model files).
 *
 * Every prompt = persona + mandate + TOOL NUDGE. The tool-nudge paragraph is
...
// Pixel 7 bench: prefill costs ~1s per 13 prompt tokens, so every persona
