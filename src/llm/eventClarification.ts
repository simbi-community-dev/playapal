...
function splitEventCoordination(
  ...
  const coordinators = [...clause.matchAll(EVENT_COORDINATOR)];
  if (coordinators.length === 0) {
    // No coordinator, no split — and no catalog span scan for a clause
    // that was never going to be cut (the provider walks every enabled
    // title, per call, on every chat turn).
    return [clause];
  }
  const protectedSpans = titleSpans?.(clause) ?? [];
  const boundary = coordinators.find(match => {
