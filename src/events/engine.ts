...
/**
 * SETTINGS READ/WRITE AGAINST AN EXPLICIT CONNECTION.
 *
 * This SQL existed THREE times: src/events/db.ts (getSetting/setSetting, on
 * the app connection) and again as a private kvGet/kvSet in campBoard.ts and
 * friendCard.ts, which need to run inside a caller's transaction and so cannot
 * use db.ts's getDb(). Three statements of one statement, and the ON CONFLICT
 * clause is exactly the kind of thing that gets fixed in one of them.
 *
 * It lives HERE rather than in db.ts because db.ts imports campBoard, so
 * campBoard importing db.ts would close a cycle. engine.ts is below both, and
 * both already import DbConnection from it.
 */
export function getSettingOn(conn: DbConnection, key: string): string | null {
...
export function setSettingOn(conn: DbConnection, key: string, value: string): void {
  conn.execute(
