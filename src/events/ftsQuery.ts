...
export function buildFtsQuery(opts: QueryOpts): BuiltQuery | null {
  ...
  // EXACT-TITLE MATCH OUTRANKS EVERYTHING: a camper who types the full name
  // of a thing ("Man burn") means THAT thing, and BM25's length-normalized
  // score can put "After Man Burn Party!" above the two-word "Man Burn"
  // milestone once the pack is rich enough (surfaced by the 2026 API
  // refresh, 5,796 rows). The tiebreak is the raw query as a whole title,
  // case-insensitive; everything else keeps its BM25 order.
  const rawQuery = opts.terms.join(' ').toLowerCase();
  sql +=
    ' ORDER BY (lower(e.title) = ?) DESC, ' +
    'bm25(f.events_fts, 10.0, 4.0, 6.0, 6.0) LIMIT ?';
  params.push(rawQuery, opts.limit ?? TOP_N);
  return { sql, params };
}
