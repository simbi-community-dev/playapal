import { DOCS_NOT_ILLUSTRATIVE_SQL, EVENTS_NOT_HIDDEN_SQL } from '../facts/hiddenItems';
...
export function eventTermProbes(terms: readonly string[]): string[] {
  return [...new Set(terms.map(term =>
    term.length > 4 ? term.toLowerCase().slice(0, 4) : term.toLowerCase(),
  ))];
}
...
export interface EventTimeFilter {
  startISO: string;
  endISO: string;
  /** Repeated clock over a multi-day window, such as "this week at 21:00". */
  dailyTime?: string;
  /** Repeated local-time interval over a multi-day window. */
  dailyStart?: string;
  dailyEnd?: string;
}

function eventScope(
  opts: Pick<QueryOpts, 'dateRange' | 'timeFilter'>,
  params: (string | number)[],
): string {
  let sql = '';
  if (opts.dateRange) {
  ...
  const { startISO, endISO, dailyTime, dailyStart, dailyEnd } = opts.timeFilter;
  sql += " AND e.time_start IS NOT NULL AND e.time_start <> ''";
  if (dailyTime || (dailyStart && dailyEnd)) {
    // The date range widens one day so an overnight event can overlap the first
    // requested morning/clock. Keep that widening as a candidate superset, but
    // reject prior-day ordinary events (and trailing-day late events) by their
    // actual absolute interval before applying the repeated local-time test.
    sql +=
      " AND datetime(e.date || 'T' || e.time_start) <= datetime(?)" +
      " AND datetime((CASE WHEN e.time_end IS NOT NULL AND e.time_end <> ''" +
      " AND e.time_end < e.time_start THEN date(e.date, '+1 day') ELSE e.date END)" +
      " || 'T' || COALESCE(NULLIF(e.time_end, ''), e.time_start)) >= datetime(?)";
    params.push(endISO, startISO);
    if (dailyTime) {
      sql +=
        " AND ((e.time_end IS NOT NULL AND e.time_end <> '' AND e.time_end < e.time_start" +
        ' AND (? >= e.time_start OR ? <= e.time_end))' +
        " OR ((e.time_end IS NULL OR e.time_end = '' OR e.time_end >= e.time_start)" +
        " AND ? >= e.time_start AND ? <= COALESCE(NULLIF(e.time_end, ''), e.time_start)))";
      params.push(dailyTime, dailyTime, dailyTime, dailyTime);
      return sql;
    }
    const dailyCrossesMidnight = dailyEnd! < dailyStart!;
    sql += dailyCrossesMidnight
      ? " AND ((e.time_end IS NOT NULL AND e.time_end <> '' AND e.time_end < e.time_start)" +
        " OR ? >= e.time_start OR ? <= COALESCE(NULLIF(e.time_end, ''), e.time_start))"
      : " AND ((e.time_end IS NOT NULL AND e.time_end <> '' AND e.time_end < e.time_start" +
        " AND (? >= e.time_start OR ? <= e.time_end))" +
        " OR ((e.time_end IS NULL OR e.time_end = '' OR e.time_end >= e.time_start)" +
        " AND ? >= e.time_start AND ? <= COALESCE(NULLIF(e.time_end, ''), e.time_start)))";
    params.push(
      dailyEnd!,
      dailyStart!,
      ...(dailyCrossesMidnight ? [] : [dailyEnd!, dailyStart!]),
    );
    return sql;
  }
  sql +=
...
interface QueryOpts {
  terms: string[];
  mode: FtsMode;
  /** Require this many distinct 4-character term probes before ORDER/LIMIT. */
  minimumTermMatches?: number;
  /** Inclusive ISO date range over event start dates. */
...
function eventRelevanceScope(opts: QueryOpts, params: (string | number)[]): string {
  const minimum = opts.minimumTermMatches;
  const probes = eventTermProbes(opts.terms);
  if (!minimum || probes.length < minimum) {
    return '';
  }
  const haystack =
    "lower(COALESCE(e.title, '') || ' ' || COALESCE(e.desc, '') || ' ' || " +
    "COALESCE(e.camp, '') || ' ' || COALESCE(e.location, ''))";
  const hits = probes.map(probe => {
    params.push(`%${escapeLike(probe)}%`);
    return `(CASE WHEN ${haystack} LIKE ? ESCAPE '\\' THEN 1 ELSE 0 END)`;
  });
  params.push(minimum);
  return ` AND (${hits.join(' + ')}) >= ?`;
}
...
export function buildFtsQuery(opts: QueryOpts): BuiltQuery | null {
  ...
  sql += eventScope(opts, params);
  sql += eventRelevanceScope(opts, params);
  // The raw term join is the deterministic title tiebreak before BM25 (Man
  // Burn before similarly-worded siblings).
  sql += ' ORDER BY CASE WHEN lower(trim(e.title)) = lower(trim(?)) THEN 0 ELSE 1 END,';
  params.push(opts.terms.join(' '));
  sql += ' bm25(f.events_fts, 10.0, 4.0, 6.0, 6.0) LIMIT ?';
 ...
 */
export function buildEventBrowseQuery(
  opts: Pick<QueryOpts, 'dateRange' | 'timeFilter' | 'limit'>,
): BuiltQuery {
...
function eventLikeGroups(
  terms: readonly string[],
  params: (string | number)[],
  expand: (term: string) => string[] = termVariants,
): string[] {
  return terms.map(term => {
    const alts = [...new Set(expand(term))].flatMap(value => {
...
/** Exact references bypass the fuzzy ladder entirely. The caller supplies the
 * stored spellings for every punctuation-equivalent title, so each recurring
 * occurrence remains eligible without FTS/LIKE tokenization becoming an
 * identity boundary. */
export function buildExactEventTitlesQuery(
  opts: Pick<QueryOpts, 'dateRange' | 'timeFilter' | 'limit'> & {
  ...
  const params: (string | number)[] = [...opts.titles];
  const groups = eventLikeGroups(opts.terms ?? [], params);
  const semanticScope = groups.length > 0
    ? `(${groups.join(' AND ')}) AND `
    : '';
  const scope = eventScope(opts, params);
  params.push(opts.limit ?? TOP_N);
  return {
    sql:
      'SELECT e.* FROM events e ' +
      'JOIN packs p ON p.id = e.pack_id AND p.enabled = 1 WHERE ' +
      `e.title IN (${opts.titles.map(() => '?').join(', ')}) AND ` +
      semanticScope + EVENTS_NOT_HIDDEN_SQL + scope +
      ' ORDER BY ROW_NUMBER() OVER (' +
      'PARTITION BY e.title ORDER BY e.date, e.time_start, e.id' +
      '), e.date, e.time_start LIMIT ?',
    params,
  };
}
...
export interface DocsScopeOpts {
...
/** Shared pack-scope clause for every docs retrieval arm. */
export function docsPackScope(
  opts: DocsScopeOpts,
  params: (string | number)[],
): string {
  // EVERY doc path appends this scope, which makes it the one place an
  // illustrative section can be excluded from all of them at once — the
  // same single-leak-point discipline EVENTS_NOT_HIDDEN_SQL follows.
  const base = ` AND ${DOCS_NOT_ILLUSTRATIVE_SQL}`;
  const prefix = opts.packIdPrefix
    ? `${escapeLike(opts.packIdPrefix)}%`
    : null;
  if (opts.packId && prefix) {
    params.push(opts.packId, prefix);
    return base + " AND (d.pack_id = ? OR d.pack_id LIKE ? ESCAPE '\\')";
  }
  if (opts.packId) {
    params.push(opts.packId);
    return base + ' AND d.pack_id = ?';
  }
  if (prefix) {
    params.push(prefix);
    return base + " AND d.pack_id LIKE ? ESCAPE '\\'";
  }
  return base;
}
