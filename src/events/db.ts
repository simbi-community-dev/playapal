...
export function setPackEnabledFull(
  ...
  const settingIn = (key: string): string | null => {
...
export function removePack(packId: string): string | null {
  ...
  const notesBacked = notesCandidate
    ? Boolean(
        conn.execute(
          'SELECT 1 FROM camp_writers WHERE camp_id = ? AND writer_id = ? LIMIT 1',
          [notesCandidate[1], notesCandidate[2]],
        ).rows?.length ||
          (conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'camp_notes'",
          ).rows?.length &&
            conn.execute(
              'SELECT 1 FROM camp_notes WHERE camp_id = ? AND writer_id = ? LIMIT 1',
              [notesCandidate[1], notesCandidate[2]],
            ).rows?.length),
      )
    : false;
  const notesWriter = notesBacked ? notesCandidate : null;
  ...
  inTransaction(conn, () => {
    ...
    const writerMatch = boardWriter ?? notesWriter;
    if (writerMatch) {
    ...
    if (board || notesWriter) {
      // Deleting a replier removes text embedded in surviving root-author
      // chunks; rebuild those chunks from the remaining enabled source rows.
      rematerializeAllBoards(conn);
    }
  });
  refreshFactGraphSafe(conn);
