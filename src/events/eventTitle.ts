...
// The lead list never changes at runtime — spread and filter it once, not
// once per catalog group per scan.
const TITLE_LEAD_LIST = [...TITLE_LEADS].filter(Boolean);

/** The list-lead verdict depends only on the HAYSTACK, yet
 * eventTitleReferenceSpans computes it per call — and exactRequestedTitles
 * calls once per catalog group (3,000+ per scan) with the SAME prepared
 * object, so object identity is an exact cache key. String-typed callers
 * build a fresh prepared object and pay one compute, as before. */
const titleListLeadCache = new WeakMap<PreparedEventTitleText, boolean>();

function titleListLeadFor(haystack: PreparedEventTitleText): boolean {
  const hit = titleListLeadCache.get(haystack);
  if (hit !== undefined) {
    return hit;
  }
  const listText = haystack.normalized
    ...
    .trim();
  const lead = TITLE_LEAD_LIST.some(
    value => listText === value || listText.startsWith(`${value} `),
  );
  titleListLeadCache.set(haystack, lead);
  return lead;
}
...
export function eventTitleReferenceSpans(
  ...
  const haystack = preparedText(text);
  const needle = preparedTitle(title);
  const titleListLead = titleListLeadFor(haystack);
