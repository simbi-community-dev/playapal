...
import { getSettingOn } from './engine';
...
function titleCatalogUnsafe(): TitleCatalogGroup[] {
  ...
  const revision = String(
    getSettingOn(conn, 'event_title_catalog_revision') ?? '0',
  );
...
/** One-entry memo: runTurn consults the exact-title arm four-plus times
 * per conversational turn with the SAME text against the SAME catalog,
 * and each miss is a full scan of every enabled title group. Keyed on the
 * catalog groups ARRAY IDENTITY — titleCatalogUnsafe returns the same
 * array object until the event_title_catalog_revision triggers bump, and
 * the fail-soft [] path builds a fresh array so it can never poison the
 * memo (it just never hits). Module-private on purpose: exporting it
 * would trip exportsHaveCallers. */
let lastExactTitles: {
  text: string;
  semantic: string | undefined;
  groups: readonly TitleCatalogGroup[];
  result: ExactTitleReferences;
} | null = null;
...
function exactRequestedTitles(
  ...
  if (
    lastExactTitles &&
    lastExactTitles.text === text &&
    lastExactTitles.semantic === semanticText &&
    lastExactTitles.groups === catalog
  ) {
    return lastExactTitles.result;
  }
  ...
  const result: ExactTitleReferences = {
    groups,
    rawSpans: selected.map(match => match.span),
  };
  lastExactTitles = { text, semantic: semanticText, groups: catalog, result };
  return result;
}
...
export function enabledTitleSpans(
  ...
  // Same one-entry memo shape as exactRequestedTitles, same key rules.
  const catalog = titleCatalog();
  if (
    lastTitleSpans &&
    lastTitleSpans.text === text &&
    lastTitleSpans.groups === catalog
  ) {
    return lastTitleSpans.result;
  }
  const haystack = prepareEventTitleText(text);
  const result = catalog.flatMap(group => eventTitleSpans(haystack, group));
  lastTitleSpans = { text, groups: catalog, result };
  return result;
}
