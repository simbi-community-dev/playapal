...
export const BASE_TABLES_SQL: string[] = [
  ...
  `INSERT OR IGNORE INTO settings (key, value)
     VALUES ('event_title_catalog_revision', '0')`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_events_insert
     AFTER INSERT ON events BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_events_delete
     AFTER DELETE ON events BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_events_update
     AFTER UPDATE OF id, pack_id, title ON events BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_packs_insert
     AFTER INSERT ON packs BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_packs_delete
     AFTER DELETE ON packs BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_packs_update
     AFTER UPDATE OF id, enabled ON packs BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_hidden_insert
     AFTER INSERT ON hidden_items WHEN NEW.kind = 'event' BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_hidden_delete
     AFTER DELETE ON hidden_items WHEN OLD.kind = 'event' BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  `CREATE TRIGGER IF NOT EXISTS event_title_catalog_hidden_update
     AFTER UPDATE OF kind, key ON hidden_items
     WHEN OLD.kind = 'event' OR NEW.kind = 'event' BEGIN
       UPDATE settings SET value = CAST(value AS INTEGER) + 1
       WHERE key = 'event_title_catalog_revision';
     END`,
  ...
  // Field conversation log (src/log/chatLog.ts): session_id groups exactly one
  // conversation and rotates at each reset; turns, tool round-trips, pack
  // installs, and failures remain LOCAL-ONLY
  ...
  // shared state. Each writer's posts live under its camp-scoped pack
  // (pack_id = camp-board-<camp_id>-<writer_id>); replies qualify their target
  // by (ref_writer_id, ref_id). Imports are per-writer snapshot replication
  // guarded by the camp_writers high-water mark below. `done` is
  ...
  // Retrieval does not read this table directly: enabled, active-camp open
  // roots and replies materialize into doc_chunks under the root pack_id.
...
export const ADDITIVE_COLUMNS: { table: string; column: string; ddl: string }[] = [
  ...
  // 2026-08-25 (codex Angel batch): format-3 replies carry the writer they
  // answer (ref_writer_id survives incarnation rotation), and fork packs
  // keep their full authenticated envelope for durable re-verification.
  { table: 'camp_posts', column: 'ref_writer_id', ddl: 'ALTER TABLE camp_posts ADD COLUMN ref_writer_id TEXT' },
  // 2026-08-25 (composed-tree review, batch 5): the gossip door's revision
  // memory. Election was within-batch only, so a LATE relay of an older
  // copy re-installed stale text tomorrow. NULL on beam-installed rows by
  // design: a sealed snapshot is the writer's authoritative reset and any
  // gossip beats it forward from there.
  { table: 'camp_posts', column: 'gossip_seq', ddl: 'ALTER TABLE camp_posts ADD COLUMN gossip_seq INTEGER' },
  { table: 'camp_posts', column: 'gossip_min', ddl: 'ALTER TABLE camp_posts ADD COLUMN gossip_min INTEGER' },
  { table: 'camp_posts', column: 'gossip_rid', ddl: 'ALTER TABLE camp_posts ADD COLUMN gossip_rid TEXT' },
  { table: 'camp_forks', column: 'envelope_json', ddl: "ALTER TABLE camp_forks ADD COLUMN envelope_json TEXT NOT NULL DEFAULT ''" },
];
