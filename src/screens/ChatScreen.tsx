...
interface Props {
  ...
  /** Whether the Angel wakes with the app on this phone; null until the
   * startup probe has answered (src/llm/angelRest.ts). */
  angel?: AngelPosture | null;
  /** A wake or a rest in flight. */
  angelBusy?: boolean;
  onAngelChange?: (awake: boolean) => void;
  /** The screen remains mounted across tabs so its visible transcript stays
   * aligned with the app-resident LlamaSession conversation. */
  active: boolean;
...
/** Points from the bottom of the transcript that still count as "at the
 * newest" — momentum and rounding leave slack, and a list that refuses to
 * re-pin two points short reads as broken. */
...
export function ChatScreen({
  ...
  /**
   * The resting card, or null when she is awake (or when this screen was
   * mounted without the switch, as the standalone suites do). Built ONCE:
   * the strip below and the input's placeholder both turn on "is she
   * resting", and two copies of that condition would drift apart.
   */
  const restCard =
    angel && !angel.awake && onAngelChange ? (
  ...
  /**
   * Is the reader at the newest? The list follows a streaming answer only
   * while they are — scrolling UP to re-read something earlier used to be
   * undone within a frame, because every token grows the content and the
   * old handler scrolled to the end unconditionally. Same rule the pod's
   * answering machine follows (src/crews/PodMessages.tsx): follow the pinned
   * reader, never yank the one who went looking. A ref, not state: it is
   * written on every scroll frame and the render does not use it.
   */
  ...
  React.useLayoutEffect(() => {
    // Invalidate every turn started in the previous visibility session. Bumping
    // on BOTH edges means a turn hidden mid-decode cannot resume speech when the
    // retained screen becomes active again.
    if (activeRef.current !== active) {
      speechGenerationRef.current += 1;
    }
    activeRef.current = active;
    if (active) {
      // Packs can change while this retained screen is hidden.
      setPacks(listPacks());
      return;
    }
  ...
  const send = useCallback(async () => {
    ...
    if (text.length === 0 || busy || !session.isReady || !activeRef.current) {
      return;
    }
    const speechGeneration = speechGenerationRef.current;
    const canSpeak = () =>
      activeRef.current && speechGenerationRef.current === speechGeneration;
    setInput('');
    setBusy(true);
    // Asking is catching up: whatever they were re-reading, the answer to the
    // question they just typed is the thing to follow (see followingRef).
  ...
  const switchPersona = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setMessages([]);
        followingRef.current = true; // an empty transcript is the newest
      } catch (error) {
        console.warn('[chat] persona switch failed:', error);
      } finally {
        setBusy(false);
      }
    },
      ...
      { text: 'Keep', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            ...
            setBusy(true);
            try {
              await session.newConversation(onStatus);
              setMessages([]);
              // An empty transcript IS the newest; leaving the pin off would
              // make the first answer of the new chat refuse to follow.
              followingRef.current = true;
            } catch (error) {
              console.warn('[chat] new conversation failed:', error);
            } finally {
              setBusy(false);
            }
          })();
  ...
  // Keyboard insets are supplied by the App root's useKeyboardInset — the
  // overlay that mounts this screen applies them inline (absolute children
  // ignore the root's padding). KeyboardAvoidingView could not do this
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ONE strip, never both: a resting Angel's card REPLACES the model
          status bar, whose idle wording ("No model yet") would otherwise sit
          directly above "she is resting" and contradict it — the model is on
          the phone; she is simply not holding it. */}
      {restCard ?? <ModelStatusBar status={status} onPickModel={onPickModel} />}
        ...
        <TextInput
          ...
          placeholder={
            status.state === 'ready'
              ...
              : restCard
              ? // She is on the phone and CHOOSING not to run: "load a model"
                // would send the camper looking for a file they already have.
                `The ${personaLabel} is resting — wake her above`
...
const styles = StyleSheet.create({
  ...
  personaTextActive: { color: colors.onAccent, fontWeight: '700' },
  knowledgeRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    alignItems: 'center',
  ...
  sendText: { color: colors.onAccent, fontSize: 18, fontWeight: '700' },
});
