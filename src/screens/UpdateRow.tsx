 ...
 * stopped being one.
 *
 * IT IS NOT IN THE PLAY BUILD AT ALL. Google Play's Device and Network
 * Abuse policy forbids an app that downloads and installs its own APK, so
 * the Store flavor both loses the permission (the manifest overlay in
 * android/app/src/play/) and loses this row (the channel flag read by
 * src/config/distribution.ts). A Store camper is not missing anything: the
 * Store is their update channel, and it is a better one than this.
 *
...
export function UpdateRow({
  ...
  // Nothing, not even a quiet line: a Store build has a working update
  // channel and does not need a paragraph about the one it cannot have.
  if (channel === 'play') {
    return null;
  }
