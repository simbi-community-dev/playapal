...
export function SettingsScreen({
  ...
  // Pocket alerts (src/crews/pocketAlerts.ts): the buzz for pod mail and
  // calls is asked in context when a pod feature first arms, and a decline
  // is permanently respected there — THIS row is the one deliberate way
  // back. Read once at mount (Settings re-mounts per tab visit, the model
  // roster's own pattern) and re-read after the tap answers.
  const [alertsChoice, setAlertsChoice] = useState<'' | 'granted' | 'denied'>(
    () => pocketAlertsChoice(),
  );
  const reAskAlerts = useCallback(async () => {
    setAlertsChoice((await reAskPocketAlerts()) ? 'granted' : 'denied');
  }, []);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {privacyOpen ? (
        <>
          {/* Pocket alerts: what the phone may say while it is in a pocket
              is a privacy-adjacent choice, so the re-ask row lives here.
              The copy names the current state honestly; if the OS itself
              has stopped showing its dialog (Android's two-strike rule,
              iOS's ask-once), the tap answers from the OS without one and
              the row keeps telling the truth. */}
          <Text style={styles.subTitle} accessibilityRole="header">
            Pocket alerts
          </Text>
          <View style={styles.card}>
            <Pressable
              style={styles.row}
              onPress={() => {
                // A build without the native notifier has nothing to ask
                // FOR — the row states that instead of running a dead ask.
                if (pocketAlertsPresent()) {
                  void reAskAlerts();
                }
              }}
              accessibilityRole="button"
              accessibilityLabel="Buzz for pod messages and calls">
              <View style={styles.rowBody}>
                <Text style={styles.rowTitle}>
                  Buzz for pod messages and calls
                </Text>
                <Text style={styles.rowDesc}>
                  {!pocketAlertsPresent()
                    ? 'This build cannot post notifications — pocket alerts arrive with a newer install.'
                    : alertsChoice === 'granted'
                    ? 'On — a pocketed phone buzzes when pod mail arrives or a podmate calls. Each kind can be tuned in system settings.'
                    : alertsChoice === 'denied'
                    ? 'Off — you said no and the app has not asked since. Tap to be asked again. If the phone itself blocks the ask, allow notifications for Playa Pal in system settings.'
                    : 'Not set — the app asks the first time you share with a pod or open the walkie. Tap to answer now instead.'}
                </Text>
              </View>
              <Text style={styles.rowChevron}>›</Text>
            </Pressable>
                ...
                <Pressable
                  ...
                  onPress={() => {
                    NativeModules.MicProbe.run()
                      ...
                      .catch((e: any) =>
                        Alert.alert('Microphone check', String(e?.message ?? e)),
                      );
                  }}
                  accessibilityRole="button"
                  accessibilityLabel="Run the microphone check">
                  <View style={styles.rowBody}>
                    <Text style={styles.rowTitle}>Run the microphone check</Text>
                    <Text style={styles.rowDesc}>
                      Tries every way of listening this phone supports and
                      shows what each one said — takes about eight seconds,
                      best with the walkie off. Screenshot the result if
                      live talk is not working.
                    </Text>
                  </View>
                  <Text style={styles.rowChevron}>›</Text>
                </Pressable>
