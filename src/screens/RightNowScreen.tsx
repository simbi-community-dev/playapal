...
export function RightNowScreen({ onAskAngel, onOpenCompass }: Props) {
  ...
  // a11yLabel: the ♥ chips would otherwise announce as bare glyphs; every
  // chip also speaks its selected state (a11y review 2026-08-24).
  const whenChip = (
    key: string,
    label: string,
    selected: boolean,
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        {whenChip(
          ...
          daySel === 'faves',
          () => setDaySel('faves'),
          favCount > 0 ? `Faves, ${favCount} saved` : 'Faves',
        )}
        ...
        <View style={styles.todRow}>
          {TOD_SEGMENTS.map(seg => (
            <Pressable
              ...
              accessibilityState={{ selected: todSel === seg.key }}
      ...
      <ScrollView style={styles.results}>
        ...
        {empty ? (
          <Text style={styles.empty}>
            {!hasEnabledEventPack
              // The path names the real route (Settings regroup 2026-08-24:
              // Public packs now lives inside the Offline content group).
              ? 'No event pack is enabled — turn one on in Settings › Offline content › Public packs.'
              : daySel === 'now'
                ? beforeGates
        ...
        {freeText.trim().length > 3 ? (
          <Pressable
            style={styles.askAngel}
            onPress={() => onAskAngel(freeText)}
            accessibilityRole="button"
            accessibilityLabel={`Ask the Angel: ${freeText.trim()}`}>
            <Text style={styles.askAngelText}>Ask the Angel: "{freeText.trim()}"</Text>
          </Pressable>
        ) : null}
        <Pressable
          style={styles.takeMeHome}
...
const styles = StyleSheet.create({
  ...
  // onAccent: scheme-aware ink on clay/plum fills (a11y review 2026-08-24).
  ...
  askAngelText: { color: colors.onAccent, fontSize: type.small, fontWeight: '600' },
  anchorNote: {
    color: colors.faded,
    fontSize: type.tiny,
    textAlign: 'center',
  ...
  takeMeHomeText: { color: colors.onAccent, fontSize: type.body, fontWeight: '800' },
});
