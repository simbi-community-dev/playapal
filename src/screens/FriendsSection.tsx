...
export function FriendsSection({
  ...
  return (
    <>
      ...
      {/* THE TUFTE PASS (owner ask 2026-08-26). How cards move, and what
          the consent pick does and does not promise, is a section-level
          lesson: the same sentences on every phone, every day. It rides the
          heading. The Alert at the moment of sharing still says the
          consent part out loud, which is where it decides anything. */}
      <View style={styles.headingRow}>
        <Text style={styles.sectionTitle} accessibilityRole="header">
          Friends on playa
        </Text>
        <InfoTap
          topic="how cards travel"
          text={
            'Cards travel phone-to-phone only — a QR your camera scans, or ' +
            'a file over AirDrop / Quick Share. No server ever sees who ' +
            'camps where. When you share, you pick "just for them" or "pass ' +
            'it on"; only "pass it on" cards ride "Beam friends". The pick ' +
            'is honored by the app, not enforced by cryptography — anyone ' +
            "can retype a card by hand, so share what you'd write on a note " +
            'board.'
          }
        />
      </View>
      <View style={styles.card}>
        ...
        {editOpen ? (
          <View>
            ...
            {/* The ? sits beside the radio group, not inside it: a
                non-radio child would join the group a screen reader counts
                through. The Alert at share time says this same sentence at
                the moment it decides something — here it is a reminder. */}
            <View style={styles.infoRow}>
              <View
                style={[styles.scopeRow, styles.infoFlex]}
              ...
              <InfoTap
                topic="who a card reaches"
                text={
                  '"Just for them" cards never ride "Beam friends" — only ' +
                  'the person you hand one to gets it.'
                }
              />
            </View>
            <Pressable
...
const styles = StyleSheet.create({
  ...
  // THE TUFTE PASS (owner ask 2026-08-26): both of this file's explainer
  // paragraphs moved behind a ?, and `hint`/`scopeHint` went with them —
  // a style with no user is the same dead weight as an export with no
  // caller. What is left is one layout: the thing, then its glyph.
