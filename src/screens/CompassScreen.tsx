    ...
    <View style={styles.container}>
      ...
      {managePins ? (
        <ScrollView style={styles.pinStage} contentContainerStyle={styles.pinList}>
          {/* THE TUFTE PASS (owner ask 2026-08-26). "Home first, then
              newest" explains the ORDER of the rows below it — it is about
              what is on screen right now, so it stays. What the taps DO is
              the same lesson every time. */}
          <View style={styles.pinListHintRow}>
            <Text style={styles.pinListHintFlex}>Home first, then newest.</Text>
            <InfoTap
              topic="the pin list"
              text={
                'Tap a pin to aim at it; Remove forgets it — you can always ' +
                'drop it again.'
              }
            />
          </View>
        ...
        <View style={styles.stage}>
          ...
          />
          {/* Four gestures, a promise and a pointer to the other view — a
              manual page laid over a live map. The map is the data; this is
              one glyph of ink under it, and the gestures are all
              discoverable by trying. */}
          <View style={styles.mapInfo}>
            <InfoTap
              topic="the map"
              text={
                'Tap a pin, friend, or landmark to select it; tap open ' +
                'ground to aim there; hold, then drag to place a spot ' +
                'exactly. All offline. Switch to "Arrow view" for the ' +
                'walking arrow.'
              }
            />
          </View>
...
const styles = StyleSheet.create({
  ...
  // THE TUFTE PASS's two layouts here: the surviving ordering clause with
  // its ? beside it (the hint's own bottom margin moves onto the row), and
  // the map's lone glyph, centred by the stage it sits in.
  pinListHintRow: {
    alignItems: 'center',
    flexDirection: 'row',
    marginBottom: spacing.md,
  },
  pinListHintFlex: { color: colors.faded, flex: 1, fontSize: type.small },
  mapInfo: { marginTop: spacing.lg },
  pinRow: {
    flexDirection: 'row',
    alignItems: 'center',
