...
jest.mock('../src/crews/radio', () => ({
  ...
  haveCrewPermissions: async () => {
    // A ROUND-TRIP AN ARM CAN HOLD OPEN. On a phone this is a native call
    // that can outlive the reason it was made — the OS backgrounds the app,
    // or the camper leaves their last pod, while the answer is in the air.
    // The M7 arms below are the only ones that use it.
    if (mockPermGate) {
      await mockPermGate;
    }
    return mockGranted;
...
// ---------------------------------------------------------------------------
// M7 — THE PERMISSION AWAIT IS A SUSPENSION WITH NO BOUND.
//
// The mesh lane traced this and handed it here rather than reaching into
// share.ts from a mesh commit, and it deliberately shipped NO arm: an arm
// without the fix is a red suite, and a red suite is not a handoff. These are
// that arm, written on the side that owns the code.
//
// haveCrewPermissions() is a native round-trip. Everything after it arms a
// radio: a session, an advertisement, meshSync. Nothing between the two can
// stop it — the flip queue is held by the very call that is suspended, so the
// stop the lifecycle files lands BEHIND the arm it was meant to prevent and
// can only tear down what should never have come up.
// ---------------------------------------------------------------------------

describe('the reasons to be on the air are read again after the permission await', () => {
  it('backgrounded while the permission answer was in the air: nothing arms', async () => {
    // Mutation: delete the post-await re-read in startMailboxPresenceInner.
    // The radio comes up for a backgrounded app — an advertisement and a
    // mesh session raised for a phone in a pocket, then retracted by the
    // stop that was queued behind it. THE ORDER IS THE WHOLE DEFECT: the
    // stop cannot run first, because this call is holding the queue.
    let open!: () => void;
    mockPermGate = new Promise<void>(r => {
      open = r;
    });
    const arming = startMailboxPresence();
    await flush();
    // Nothing has happened yet: the call is suspended inside the ask.
    expect(calls).toEqual([]);

    // The app goes to the background WHILE the answer is in the air. The
    // posture is read from AppState rather than from the `foreground` latch
    // on purpose — see the production comment — so this is the whole event
    // as share.ts sees it.
    ...
    expect(mailboxPresenceOn()).toBe(false);
    expect(calls).toEqual([]);
    expect(meshStarts).toHaveLength(0);
  });

  it('the last pod was disbanded while the permission answer was in the air: nothing arms', async () => {
    // The same seam, the other reason. Mutation: re-read only the posture
    // and this one lives — an advertisement for a pod that no longer exists,
  ...
  it('…and an ordinary grant still arms, so the re-read is a gate and not a wall', async () => {
    // THE NON-VACUITY HALF. A gate that refused everything would pass both
    // arms above and take the mailbox off the air for every camper.
    // Mutation: invert the posture test (refuse unless 'background') and
    // this dies while the two above stay green — which is exactly the shape
    // a one-sided arm cannot catch.
    let open!: () => void;
    mockPermGate = new Promise<void>(r => {
      open = r;
    });
