...
const KNOWN_UNCALLED: Record<string, string> = {
  ...
  walkieOn: 'walkie liveness accessor; the panel tracks its own open state',
  ...
  encodeFriendSchemeLink:
    'DEFECT: the friend QR renders the https carrier while this docstring claims it is the one encoded. Owner call on the tradeoff (site/f.html is a real web fallback), then one of the two must change.',
  encodeBeamLink:
