...
describe('CrewSection — no crew yet', () => {
  test('one quiet card: the invite copy and a Start button, nothing else', () => {
    ...
    expect(t).toContain(
      'Your pod is the people whose phones stay in touch — for a night, a day, or the whole camp all week. One glance: which way, how far.',
    );
...
describe('CrewSection — with a crew', () => {
  ...
  test('presenceFor flips a row live and the compass uses THOSE coords', () => {
    ...
    const presenceFor = (cardId: string) =>
      cardId === 'aaaa1111'
        ? { lat: 40.7901, lon: -119.21, atMs: Date.now(), live: true }
        : null;
  ...
  test('"We\'re together — swap cards" rides the existing card share lane', () => {
    mockFriends = [alex()];
    saveCrew(newCrew('Dawn patrol', ['aaaa1111']));
    ...
    expect(shareSpy).toHaveBeenCalledWith(
      { title: 'Playa Pal friend card', message: mockCardJson },
