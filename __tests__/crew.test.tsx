...
jest.mock('../src/events/db', () => ({
  ...
  getDb: () => ({
    execute: (sql: string, params: unknown[] = []) => {
      ...
      // The unread COUNT, so the pane strip's badge and its "land on the
      // pane with news" rule can be driven from a test (2026-08-26). One
      // number, set per test; the real query runs on the shipped DDL in
      // podMessages.test.tsx.
      const rows = /COUNT\(\*\) AS n/.test(sql)
        ? [{ n: mockUnread }]
...
/** Mail waiting in the shown pod, as the COUNT query would answer it. */
let mockUnread = 0;
...
import { composeRecord } from '../src/crews/messages';
...
describe('CrewSection — no crew yet', () => {
  test('one quiet card: the invite copy and a Start button, nothing else', () => {
    ...
    const t = textOf(root);
    // No standalone "Pods" title any more (owner, Aug 27: it broke the
    // page differently from Camp, which has none) — with no pod yet, the
    // lede below IS the card's identity.
...
/**
 * THE PANE STRIP (owner ask 2026-08-26, after a professional UX designer
 * read this page as "horrifying … extremely confused. The visual cues are
 * not easy at all, and the various sections are not very organized").
 *
 * Four concerns that used to be one endless scroll are four panes behind a
 * strip that never scrolls away — the Camp tab's idiom, applied here
 * because consistency IS the ask. What these tests hold is the part that
 * is easy to lose in a later refactor and expensive to lose in the field:
 * the strip is reachable whatever the mailbox is doing, and switching a
 * chip costs a camper nothing.
 */
describe('CrewSection — four panes, one pod card', () => {
  /** One node per chip. A Pressable surfaces as composite + host with the
   * same props, so the raw find returns each chip several times; the label
   * is what identifies a destination (and what a screen reader navigates
   * by), so it is what de-duplicates them. */
  const tabs = (root: any) => {
    const seen = new Set<string>();
    return root.root
      .findAll((n: any) => n.props?.accessibilityRole === 'tab')
      .filter((n: any) => {
  ...
  const tabNamed = (root: any, label: string) =>
    tabs(root).find((n: any) =>
      String(n.props.accessibilityLabel).startsWith(label),
    );

  const selected = (root: any) =>
    tabs(root)
      .filter((n: any) => n.props.accessibilityState?.selected)
      .map((n: any) => n.props.accessibilityLabel);

  /** Every pane's ScrollView, shown or hidden — the strips are mocked to
   * null in this suite, so these four are the only scrolls in the tree. */
  const panes = (root: any) =>
    root.root.findAllByType(require('react-native').ScrollView);

  const visiblePanes = (root: any) =>
    panes(root).filter(
      (s: any) =>
        require('react-native').StyleSheet.flatten(s.props.style)?.display !==
        'none',
    );

  test('the strip has the pod\'s four concerns, and sits OUTSIDE every scroll', () => {
    // MUTATION THIS DIES ON: putting the strip inside a pane's ScrollView.
    // The answering machine has no length limit, so a strip that scrolls
    // with it is a strip a camper cannot reach on a busy day — which is
    // the exact defect the four panes exist to undo.
    saveCrew(newCrew('Dawn patrol', ['aaaa1111']));
    const root = renderSection();
    expect(tabs(root).map((n: any) => n.props.accessibilityLabel)).toEqual([
      'Who is in this pod',
      'The answering machine',
      'Live talk and calls',
      'Invite people, and camp gear',
    ]);
    // …and each one is READABLE, not only spoken.
    const strip = textOf(root);
    for (const word of ['People', 'Mail', 'Walkie', 'Setup']) {
      expect(strip).toContain(word);
    }
    for (const t of tabs(root)) {
      let node: any = t.parent;
      while (node) {
        expect(node.type).not.toBe(require('react-native').ScrollView);
        node = node.parent;
      }
    }
    // Exactly one pane is showing at a time — a strip whose chips all
    // resolve to the same scroll is a strip that does nothing.
    expect(visiblePanes(root)).toHaveLength(1);
    expect(panes(root)).toHaveLength(4);
  });

  test('the first pane is the one with NEWS: mail when mail is waiting', () => {
    // MUTATION: hard-code the default to 'mail'. A camper with an empty
    // mailbox then lands on a blank screen — the worst first impression a
    // busy surface can make. Mutation B: hard-code 'people', and the tab
    // badge's promise ("25 waiting") lands them somewhere else entirely.
    saveCrew(newCrew('Dawn patrol', ['aaaa1111']));
    expect(selected(renderSection())).toEqual(['Who is in this pod']);
    mockUnread = 3;
    expect(selected(renderSection())).toEqual(['The answering machine, 3 waiting']);
  });

  test('the default re-reads on RE-ENTRY, and a chosen pane outranks it', () => {
    // MUTATION: read the default once at mount and never again. The card
    // is kept ALIVE across tab switches (App.tsx), so "once at mount"
    // means once per APP RUN — a camper whose first-ever visit had no
    // mail lands on People forever after, even when they tap the tab
    // BECAUSE its badge says twelve waiting. Counter-mutation: re-read
    // unconditionally, and a camper mid-Setup with a QR on screen gets
    // yanked to Mail by a tab switch — the camper's own hand outranks
    // the news.
    saveCrew(newCrew('Dawn patrol', ['aaaa1111']));
    let root: any;
    TestRenderer.act(() => {
      root = TestRenderer.create(
        <CrewSection onOpenCompass={() => {}} active={true} />,
      );
    });
    liveRoots.push(root);
    expect(selected(root)).toEqual(['Who is in this pod']);

    // The camper walks to another tab; mail arrives while they are away.
    TestRenderer.act(() => {
      root.update(<CrewSection onOpenCompass={() => {}} active={false} />);
    });
    mockUnread = 12;
    // Arrival bumps the store's own revision — composeRecord is an
    // exported door onto that signal (the mocked db swallows its INSERT).
    TestRenderer.act(() => {
      composeRecord('text', 'dawn', 'me', 'hi', '', null, 0);
    });
    TestRenderer.act(() => {
      root.update(<CrewSection onOpenCompass={() => {}} active={true} />);
    });
    expect(selected(root)).toEqual(['The answering machine, 12 waiting']);

    // Now the camper CHOOSES Setup, walks away, more mail arrives — and
    // re-entry leaves them exactly where their thumb put them.
    TestRenderer.act(() =>
      tabNamed(root, 'Invite people, and camp gear')!.props.onPress(),
    );
    TestRenderer.act(() => {
      root.update(<CrewSection onOpenCompass={() => {}} active={false} />);
    });
    mockUnread = 20;
    TestRenderer.act(() => {
      composeRecord('text', 'dawn', 'me', 'hi again', '', null, 0);
    });
    TestRenderer.act(() => {
      root.update(<CrewSection onOpenCompass={() => {}} active={true} />);
    });
    expect(selected(root)).toEqual(['Invite people, and camp gear']);
  });

  test('a chip switches the pane, and every other pane stays MOUNTED', () => {
    // MUTATION: render the chosen pane and unmount the rest. It looks
    // identical and quietly throws away a half-typed message, the walkie
    // stage's scroll position and the roster's live subscriptions every
    // time a thumb lands on a chip. The proof is that a sentence which
    // belongs to a HIDDEN pane is still in the tree.
    mockFriends = [alex()];
    saveCrew(newCrew('Dawn patrol', ['aaaa1111']));
    const root = renderSection();
    expect(selected(root)).toEqual(['Who is in this pod']);
    // People is showing: the roster's own honesty floor is on screen…
    expect(textOf(root)).toContain('Where people said they\'d be');
    // …and Setup's join code is mounted behind it, not thrown away.
    expect(textOf(root)).toContain('Share code');

    TestRenderer.act(() => tabNamed(root, 'Invite people')!.props.onPress());
    expect(selected(root)).toEqual(['Invite people, and camp gear']);
    expect(visiblePanes(root)).toHaveLength(1);
    // Both sentences are STILL there — one visible, one mounted-and-hidden.
    expect(textOf(root)).toContain('Share code');
    expect(textOf(root)).toContain('Where people said they\'d be');
  });

  test('the strip carries state as ink: a mail count, and nothing when zero', () => {
    // MUTATION: render the badge unconditionally (a "0" chip is furniture
    // pretending to be news), or read a number this screen derived rather
    // than the store's own count the tab badge uses — two counts on one
    // phone that can disagree.
    /** Everything written ON the Mail chip — the word, and the mark. */
    const mailChip = (root: any) =>
      tabNamed(root, 'The answering machine')!
        .findAllByType(require('react-native').Text)
        .map((t: any) => flatText(t.props.children))
        .join('|');
    ...
    expect(tabNamed(quiet, 'The answering machine')!.props.accessibilityLabel).toBe(
      'The answering machine',
    );
    expect(mailChip(quiet)).toBe('Mail');
    mockUnread = 25;
    const busy = renderSection();
    expect(tabNamed(busy, 'The answering machine')!.props.accessibilityLabel).toBe(
      'The answering machine, 25 waiting',
    );
    expect(mailChip(busy)).toBe('Mail|25');
  });
});

/**
 * WHERE A WALKIE-STAGE REQUEST LANDS. Read as source: driving it needs a
 * live session and a native radio, which is walkieRingAnywhere.test.tsx's
 * harness, not this suite's settings Map.
 */
describe('the mini-bar tap lands on the right pod AND the right pane', () => {
  const src = require('fs').readFileSync(
    'src/crews/CrewSection.tsx',
    'utf8',
  ) as string;
  const routing = src.slice(
    src.indexOf('const walkieStageAsked'),
    src.indexOf('// -- end walkie-stage routing'),
  );

  test('asking for the stage brings the card to that pod and that pane', () => {
    // MUTATION: drop setPane('walkie'). The camper taps "tap to open" on
    // the mini-bar from the camp board, lands on Pods — and lands on Mail,
    // with the walkie they were just promised one more tap away and no
    // sign of where it went.
    expect(routing).toContain('setActivePodId(walkiePodId)');
    expect(routing).toContain("setPane('walkie')");
  });

  test('the strip says when the radio is hot, and only for THIS pod', () => {
    // MUTATION: light the dot from walkieOn() instead of walkieOnFor(crew.id).
    // The session can be holding ANOTHER pod's channel — a live dot on this
    // pod's Walkie chip would then promise an open mic on a channel nobody
    // is on, which is the staleness class the whole walkie lane guards.
    expect(src).toContain('walkieOnFor(crew.id)');
    expect(src).toContain('on the air');
  });

  test('the session drives the pane, and the pane never drives the session', () => {
    // MUTATION: add an effect binding setWalkiePanelOpen to the pane.
    // Tapping the Walkie chip while ANOTHER pod's channel is open would
    // then drag People, Mail and Setup onto that other pod too — a pod
    // switch nobody asked for, from a tap that only meant "show me the
    // walkie".
    expect(src).not.toMatch(/setWalkiePanelOpen/);
  });
});
