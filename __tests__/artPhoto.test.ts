...
describe('board posts are bounded at authoring (B13 reachability: the one uncapped writer surface)', () => {
  ...
  test('post N+1 refuses; an EDIT of an existing post still saves', () => {
    ...
    // THE BYPASS ARM (survived MUT6 in the definitive batch until added):
    // a NOVEL id at the cap is still a CREATE — INSERT OR REPLACE would
    // happily mint it if the gate keyed on "id supplied" instead of
    // "row exists".
    expect(() =>
