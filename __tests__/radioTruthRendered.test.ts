...
/**
 * THE SAME LIE, ONE LAYER OVER (mailbox decoupling, 2026-08-25). A session
 * used to mean a share, so Settings asked sessionActive() and was right.
 * Now the app holds a session whenever it is open with a pod — carrying pod
 * mail, no position in it — and that same question would put "Sharing is ON
 * right now — your pod can see your position" at the top of Settings for a
 * camper who has shared nothing. The row must ask about the POSITION.
 */
describe('a session is not a share', () => {
  test('the master off-switch is gated on a position being on the air', () => {
    // Mutation: gate on sessionActive() (the old question) — the kill
    // switch appears, and claims to be needed, for a phone that is only
    // carrying messages.
    const src = codeOnly(RADIO_SETTINGS);
    expect(src).toMatch(/const crewSharingOn = sharingCrewId\(\) !== null;/);
    expect(src).not.toMatch(/sessionActive\(\)/);
  });

  test('the copy scopes itself to position, not to silence', () => {
    // Mutation: keep "ends every session" — the tap does not do that any
    // more (mail keeps moving while the app is open), and a promise of
    // silence the phone does not keep is the same class of lie.
    const src = readRadioSrc(RADIO_SETTINGS);
    expect(src).toMatch(/turns position sharing off everywhere/);
    expect(src).toMatch(/pod messages keep moving while the app is open/);
    expect(src).not.toMatch(/One tap here ends every session/);
  });

  test('the pod card says the switch is only about position', () => {
    // Mutation: leave the old hint alone — the sentence a camper reads
    // beside the switch still implies mail rides on it.
    const src = readRadioSrc(RADIO_CREW);
    expect(src).toMatch(/this switch is only about your position/);
  });
});
