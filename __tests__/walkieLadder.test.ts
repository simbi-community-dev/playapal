...
describe('rung 3 on iOS mirrors the Kotlin spec', () => {
  ...
  test('a peer is listed only after PROOF: connect + write budget + identity read (§5)', () => {
    ...
    // The budget is read into a local now (2026-08-27) so the refusal can
    // SAY the number it refused over — the gate itself is unchanged, and
    // both halves are asserted so neither can drift out from under it.
    expect(bleSwift).toMatch(
      /let budget = peripheral\.maximumWriteValueLength\(for: \.withoutResponse\)/,
    );
    // Spelled WalkieBleVoice.* rather than Self.* since 2026-08-27: the
    // gate moved INTO the per-dial generation, which is a different type,
    // and the constants stay on the coordinator so both halves of the rung
    // read one number.
    expect(bleSwift).toMatch(/budget >= WalkieBleVoice\.minVoiceWrite/);
    const ident = bleSwift.slice(bleSwift.indexOf('private func handleIdent'));
    expect(ident.slice(0, 700)).toContain('be32(b, 2) == podHash');
    // The proof must PRECEDE the listing, and only bookkeeping's worth of
    // source may stand between them (2026-08-27: back to the pre-watchdog
    // 300 — the liveness stamp that widened this window to 500 left with
    // the iOS watchdog's revert, and the gap it occupied is gone with it).
    // What this enforces is exactly the bound and the order, no more: a
    // call to onPeer that is not preceded by the link being marked ready
    // fails, and so does anything real growing into the gap.
    //
    // `gen.markReady()` rather than `peer.ready = true` since the
    // generation substrate: readiness is now a property of the LINK and
    // the durable peer reads it through the link it currently holds, so
    // there is no flag on the entry for a dead link to leave behind.
  ...
  test('one receive path on iOS too — BLE frames run the same gates', () => {
    ...
    // ...and closing the walkie tears the rung down with it — now through
    // the OPERATION HANDLE the airtime arbiter retains on the lease
    // (ARCHITECTURE ruling, 2026-08-27). It used to hang off this module's
    // own field, which a stop cleared on its way in, so a second stop
    // found nothing to interrogate; the lease outlives the instance and
    // the handle goes with it. walkieBleLiveness.test.ts holds the whole
    // arm set. The anchor moves with the handle: one on `stop()` would
    // report the cured file as a rung that no longer closes at all.
    expect(swift).toMatch(
      /Self\.arbiter\.armStart\(lease: leaseId, stopper: \{ settle in\n\s*ble\.stop \{ down, why in settle\(down, why\) \}/,
    );
    expect(bleSwift).toMatch(
      /func stop\(_ proven: \(\(Bool, String\) -> Void\)\? = nil\) \{\s*\n\s*queue\.async \{ \[self\] in\s*\n\s*stopped = true/,
    );
  });
