...
describe('the mini-bar is the visible proof of a hot radio', () => {
  test('it shows exactly when the walkie is on AND the stage is closed', async () => {
    ...
    // "Open" is intent AND visibility now (codex seam, 2026-08-27): the
    // stage the camper wants open only counts as open while it is on
    // screen. Wanted-but-hidden keeps the bar — a hot radio with nothing
    // on screen admitting it is the lie the bar exists to prevent.
    setWalkieStageVisible(true);
    expect(walkieMiniBarShown(walkieSessionState())).toBe(false); // on, open, visible
    setWalkieStageVisible(false);
    expect(walkieMiniBarShown(walkieSessionState())).toBe(true); // on, wanted, hidden
    setWalkieStageVisible(true);
    setWalkiePanelOpen(false);
...
/**
 * ...AND THE PANE RESHAPE DID NOT PUT IT BACK IN A BOX (2026-08-26).
 *
 * The Pods tab became four chip-switched panes that day. A pane is a
 * `display:'none'` subtree — mounted, but invisible — so a ring rendered
 * inside one would be INVISIBLE on three chips out of four, which is the
 * 2026-08-25 defect wearing a new costume: the camper reading their mail
 * would never see the phone ringing, and the caller would hear out the
 * 30 s no-answer. Source pins, because "it is not inside that subtree" is a
 * claim about where a mount point IS, and no render can prove an absence.
 */
describe('the ring and the mini-bar live above every tab AND every pane', () => {
  const readSource = (p: string): string =>
    require('fs').readFileSync(p, 'utf8') as string;

  test('WalkieDeck mounts once, in App.tsx, outside the tab body', () => {
    // Mutation: move <WalkieDeck/> inside the {tab === ...} body, or into
    // PodScreen, or into one of CrewSection's panes.
    const app = readSource('App.tsx');
    expect(app).toMatch(/<WalkieDeck\b/);
    // It sits between the tab BODY and the tab BAR — after </View> closes
    // the body, before the bar opens.
    expect(app.indexOf('<WalkieDeck')).toBeGreaterThan(
      app.indexOf('<View style={styles.body}>'),
    );
    expect(app.indexOf('<WalkieDeck')).toBeLessThan(
      app.indexOf('<View style={styles.tabBar}'),
    );
  });

  test('neither the Pods screen nor the pod card renders a call surface', () => {
    // Mutation: any of these three files mounting the deck, the call panel
    // or the ring — a second copy is also a second ring on one phone.
    for (const f of [
      'src/screens/PodScreen.tsx',
      'src/crews/CrewSection.tsx',
      'src/crews/WalkiePanel.tsx',
    ]) {
      const src = readSource(f);
      expect(src).not.toMatch(/<WalkieDeck\b/);
      expect(src).not.toMatch(/<VideoCallPanel\b/);
    }
  });
});
