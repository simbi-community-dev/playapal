...
describe('release ABI split', () => {
  ...
  test('is gated on a release APK task being requested (debug keeps every ABI)', () => {
    ...
    // The matcher grew \w* when the github/play flavor split renamed every
    // task (assembleGithubRelease, bundlePlayRelease): the gate must see a
    // FLAVORED release task, or the signing refusal and the ABI split both
    // sleep through a real release build.
    expect(gradle).toMatch(/\(assemble\|install\|package\)\\w\*Release/);
  });
