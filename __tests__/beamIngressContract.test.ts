...
describe('Android beam ingress filters (contract §1)', () => {
  ...
  test('the friend-card deep links are preserved byte-for-byte', () => {
    expect(manifest).toMatch(
      /<data android:scheme="https" android:host="playapal\.lol" android:pathPrefix="\/f" \/>/,
    );
    expect(manifest).toMatch(
      /<data android:scheme="https" android:host="www\.playapal\.lol" android:pathPrefix="\/f" \/>/,
    );
    expect(manifest).toMatch(
  ...
  test('the beam deep links get sibling filters (§1 / §5)', () => {
    expect(manifest).toMatch(/android:pathPrefix="\/b"/);
    expect(manifest).toMatch(
