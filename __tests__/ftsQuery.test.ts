...
describe('eventTermProbes', () => {
  test('dedupes the same four-character relevance probe', () => {
    expect(eventTermProbes(['packing', 'packed', 'moop'])).toEqual(['pack', 'moop']);
  });
});
...
describe('sanitizeKeywords', () => {
  ...
  test('applies caller exclusions before the six-term cap', () => {
    const shell = new Set(['please', 'search', 'guide', 'events', 'classes', 'include']);
    expect(sanitizeKeywords(
      'please search guide events classes include sunrise yoga',
      { exclude: shell },
    )).toEqual(['sunrise', 'yoga']);
  });
...
describe('buildFtsQuery', () => {
  test('joins the FTS table to events and ranks by BM25', () => {
    ...
    // Fuzzy searches retain the shipped raw-term title tiebreak: "Man burn"
    // ranks the two-word milestone above "THE MAN BURNS".
    expect(q!.sql).toContain(
      'CASE WHEN lower(trim(e.title)) = lower(trim(?)) THEN 0 ELSE 1 END',
    );
    expect(q!.sql).toContain('bm25(f.events_fts');
    // "welding" carries its sibling root so corpus "welder(s)"/"welded" meet it.
    expect(q!.params).toEqual(['("welding"* OR "weld"*)', 'welding', TOP_N]);
    const titled = buildExactEventTitlesQuery({
      titles: ['Welding Workshop'],
    });
    expect(titled!.sql).toContain('e.title IN (?)');
    expect(titled!.params).toEqual(['Welding Workshop', TOP_N]);
  });

  test('exact-title qualifiers filter searchable columns before the bound', () => {
    const q = buildExactEventTitlesQuery({
      titles: ['Morning Coffee'],
      terms: ['morning', 'coffee', 'kids', 'center', 'camp'],
      dateRange: { startDate: '2026-09-01', endDate: '2026-09-07' },
      limit: 5,
    });
    expect(q).not.toBeNull();
    expect(q!.sql.indexOf('e.title IN (?)')).toBeLessThan(
      q!.sql.indexOf("e.title LIKE ? ESCAPE '\\'"),
    );
    expect(q!.sql.indexOf("e.location LIKE ? ESCAPE '\\'"))
      .toBeLessThan(q!.sql.indexOf('LIMIT ?'));
    expect(q!.sql).toContain(') AND (');
    expect(q!.params[0]).toBe('Morning Coffee');
    expect(q!.params.slice(-3)).toEqual(['2026-09-01', '2026-09-07', 5]);
  });
  ...
  test('multi-term relevance is filtered in SQL before the bounded limit', () => {
    const q = buildFtsQuery({
      terms: ['moop', 'trash', 'leave'],
      mode: 'or',
      minimumTermMatches: 2,
      limit: 5,
    });
    expect(q!.sql.indexOf('CASE WHEN')).toBeLessThan(q!.sql.indexOf('LIMIT'));
    expect(q!.params).toEqual([
      '"moop"* OR "trash"* OR "leave"*',
      '%moop%',
      '%tras%',
      '%leav%',
      2,
  ...
  test('time range is filtered in SQL before the bounded limit', () => {
    const q = buildFtsQuery({
      terms: ['yoga'],
      mode: 'and',
    ...
    expect(q!.sql).toContain("datetime(e.date || 'T' || e.time_start)");
    expect(q!.params).toEqual([
      '"yoga"*',
      '2026-09-02',
      '2026-09-03',
  ...
  test('browse query uses the same temporal owner predicate', () => {
    const q = buildEventBrowseQuery({
      dateRange: { startDate: '2026-09-01', endDate: '2026-09-07' },
      timeFilter: {
        startISO: '2026-09-01T21:00',
        endISO: '2026-09-07T21:00',
        dailyTime: '21:00',
      },
    });
    expect(q.sql).toContain('e.time_end < e.time_start');
    expect(q.params).toEqual([
      '2026-09-01',
      '2026-09-07',
      '2026-09-07T21:00',
  ...
  test('daily daypart ranges use absolute and repeated-time predicates before limiting', () => {
    const q = buildEventBrowseQuery({
      dateRange: { startDate: '2026-08-31', endDate: '2026-09-07' },
      timeFilter: {
...
describe('bounded SQL relevance floor', () => {
  test('finds a valid multi-term row after more than 100 one-term decoys', () => {
    const { DatabaseSync } = require('node:sqlite');
    const db = new DatabaseSync(':memory:');
    db.exec(`
      CREATE TABLE packs (id TEXT PRIMARY KEY, enabled INTEGER NOT NULL);
      CREATE TABLE hidden_items (kind TEXT NOT NULL, key TEXT NOT NULL, ts TEXT NOT NULL DEFAULT '');
      CREATE TABLE events (
      ...
      INSERT INTO packs VALUES ('test', 1);
    `);
    const insert = db.prepare(
      'INSERT INTO events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    );
    for (let id = 1; id <= 105; id += 1) {
      insert.run(id, 'test', `Alpha decoy ${id}`, '', '', '', '2026-09-01', '10:00', '');
    }
    insert.run(
      999,
      'test',
    ...
    expect(db.prepare(q.sql).all(...q.params).map((row: { id: number }) => row.id))
      .toEqual([999]);
  });
});
...
describe('an illustration is not evidence', () => {
  // The guide's own "Sample entry shape (structure, not a real 2026
  // event) [illustrative]" section put a MADE-UP event into the Angel's
  // answer to a Burning Man history question (owner field report
  // 2026-08-25). Every doc path appends docsPackScope, so excluding it
  // there is the one place it can be excluded from all of them.
  test.each([
    ['unscoped', {}],
    ['pack-scoped', { packId: 'survival-guide' }],
    ['prefix-scoped', { packIdPrefix: 'camp-' }],
    ['both', { packId: 'survival-guide', packIdPrefix: 'camp-' }],
  ])('docsPackScope excludes illustrative headings: %s', (_label, opts) => {
    const params: (string | number)[] = [];
    expect(docsPackScope(opts as any, params)).toContain('[illustrative]');
  });
});
