...
/**
 * A FUNCTION BODY, SIGNATURE TO ITS MATCHING BRACE — and asserted
 * non-empty at every call site.
 *
 * The reader this replaces was `swift.slice(swift.indexOf('private func
 * stopInternal()'))`, anchored on an arity that no longer exists: the
 * teardown carries the advertiser's proof completion now. `indexOf`
 * answered -1, `slice(-1)` handed back the file's LAST CHARACTER, and the
 * `toContain`s below then failed with "expected sweepTimer?.cancel(),
 * received a newline" — a suite that had stopped reading, reporting as a
 * file that had changed. Anchoring on the OPEN PAREN and counting braces
 * fixes the first half; the length assertion is what makes the second half
 * impossible, because a reader that finds nothing must SAY so.
 */
const swiftFuncBody = (src: string, signature: string): string => {
...
describe('MUTATION 4 — the probe cadence is bounded', () => {
  ...
  test('the keep-alive lives and dies with the walkie SESSION', () => {
    ...
    const swiftStop = swiftFuncBody(swift, 'private func stopInternal(');
    expect(swiftStop.length).toBeGreaterThan(0);
