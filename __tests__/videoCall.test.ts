...
describe('the mirror convention (owner 2026-08-26: "pan right and it goes left")', () => {
  const live = {
    ...idleCall,
    phase: 'live' as const,
    peerHash: 7,
  ...
  test('a call opens on the front lens, and the self-view is a mirror', () => {
    // getUserMedia asks facingMode 'user', so anything but true here is the
    // model disagreeing with the camera the runtime actually opened.
    expect(idleCall.frontCamera).toBe(true);
    expect(callSelfMirrored(idleCall)).toBe(true);
  });

  test('flip-camera moves the FLAG and the LENS from one event', () => {
    // Mutation: drop the flip-camera effect and the lens never switches
    // while the panel unmirrors; drop the flag and the lens switches while
    // the panel keeps mirroring — which is the bug as reported.
    const a = reduceCall(live, { type: 'flip-camera' });
    expect(a.model.frontCamera).toBe(false);
    expect(a.effects).toEqual([{ do: 'flip-camera' }]);
    expect(callSelfMirrored(a.model)).toBe(false);
    const b = reduceCall(a.model, { type: 'flip-camera' });
    expect(b.model.frontCamera).toBe(true);
    expect(callSelfMirrored(b.model)).toBe(true);
  });

  test('a flip outside a live call changes nothing', () => {
    // Mutation: let 'ringing' flip — the flag drifts away from the camera
    // that has not been opened yet, and the first frame is mirrored wrong.
    const ringing = { ...idleCall, phase: 'ringing' as const };
    expect(reduceCall(ringing, { type: 'flip-camera' }).model).toEqual(ringing);
    expect(reduceCall(idleCall, { type: 'flip-camera' }).effects).toEqual([]);
  });

  test('the next call starts front-facing again', () => {
    // Mutation: carry frontCamera across the hangup — call two opens on the
    // front camera (getUserMedia says so) while the model still says rear.
    const flipped = reduceCall(live, { type: 'flip-camera' }).model;
    const after = reduceCall(flipped, { type: 'hangup' }).model;
    expect(after.frontCamera).toBe(true);
  });
});

describe('the call duration read-out', () => {
  test('mm:ss below the hour, h:mm:ss above it, zero-floored', () => {
    // Mutation: drop the padStart and 1:05 renders as "1:5"; drop the
    // Math.max and a clock skew renders "-1:59" at somebody in the dust.
    expect(callDurationCopy(0)).toBe('0:00');
    expect(callDurationCopy(7_400)).toBe('0:07');
    expect(callDurationCopy(65_000)).toBe('1:05');
    expect(callDurationCopy(59 * 60_000 + 59_000)).toBe('59:59');
    expect(callDurationCopy(3_600_000)).toBe('1:00:00');
    expect(callDurationCopy(3_725_000)).toBe('1:02:05');
    expect(callDurationCopy(-5_000)).toBe('0:00');
    expect(callDurationCopy(NaN)).toBe('0:00');
  });
});
