...
function scopedPackFiles(
  ...
  return [
    {
      name: 'pack.json',
      content: JSON.stringify({ id, name, version: 1 }),
    },
    {
      name: 'about-testcamp.md',
      content: `# Testcamp\n\nTestcamp is the ${marker} identity card.`,
    },
    {
      name: 'semantic.md',
      content: `# Hidden topic\n\nThis is the ${marker} vector candidate.`,
    },
    {
      name: 'embeddings.json',
      content: JSON.stringify({
        ...
        dim: 384,
        vectors: {
          'about-testcamp.md:0': basisVec(50),
          'semantic.md:0': semanticVector,
        },
      }),
    },
  ];
...
function scopedConn() {
  ...
  const query = basisVec(30);
  const inPrefix = [...query];
  inPrefix[31] = 0.8;
  installPackFromFiles(
    conn,
    scopedPackFiles('testcamp', 'Testcamp', query, 'OUT-OF-PREFIX'),
  );
  // The import door now REFUSES the camp namespace (an imported pack there
  // would be swept by camp maintenance), and production camp-board packs
  // are minted by the camp path, not importPack. The harness mirrors that:
  // install under a neutral id through the real pipeline, then rename the
  // rows into the namespace the prefix-scoping under test actually reads.
  installPackFromFiles(
    conn,
    scopedPackFiles(
      'zz-scope-probe',
  ...
  for (const sql of [
    "UPDATE packs SET id = 'camp-board-testcamp' WHERE id = 'zz-scope-probe'",
    "UPDATE doc_chunks SET pack_id = 'camp-board-testcamp' WHERE pack_id = 'zz-scope-probe'",
    "UPDATE doc_vectors SET pack_id = 'camp-board-testcamp' WHERE pack_id = 'zz-scope-probe'",
  ]) {
    try {
      conn.execute(sql);
    } catch {
      // a table absent from this harness simply has nothing to rename
    }
  }
  return conn;
}
...
describe('install-time vector handling', () => {
  ...
  it('fails loudly on a stale vector build (orphan key)', () => {
    ...
    expect(() => installPackFromFiles(conn, files)).toThrow(
      /stale vector build/,
    );
  });
});
...
describe('pack prefix scope coherence', () => {
  it('keyword-only about pin never selects a stronger identity match outside the prefix', () => {
    const saved = mockConn;
    mockConn = scopedConn();
    try {
      const out = searchDocs(
        { query: 'testcamp', pack_prefix: 'camp-board-' },
        2,
      );
      expect(out.strategy).toBe('about-pin');
      expect(out.results.map(r => r.pack_id)).toEqual([
        'camp-board-testcamp',
      ]);
      expect(out.results[0].content).toContain('IN-PREFIX');
  ...
  it('keyword LIKE fallback excludes matching rows outside the prefix', () => {
    const saved = mockConn;
    mockConn = scopedConn();
    try {
      const out = searchDocs(
        { query: 'vector candidate', pack_prefix: 'camp-board-' },
        2,
      );
      expect(out.strategy).toBe('like-and');
      expect(out.results.map(r => r.pack_id)).toEqual([
        'camp-board-testcamp',
      ]);
      expect(out.results[0].content).toContain('IN-PREFIX');
  ...
  it('semantic fusion never admits a stronger vector candidate outside the prefix', async () => {
    const saved = mockConn;
    mockConn = scopedConn();
    try {
      setQueryEmbedder(async () => basisVec(30));
      const out = await searchDocsSemantic(
        { query: 'unshared paraphrase', pack_prefix: 'camp-board-' },
        2,
      );
      expect(out.results.map(r => r.pack_id)).toEqual([
        'camp-board-testcamp',
