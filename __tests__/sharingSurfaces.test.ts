...
describe('the Share section says who each path is for', () => {
  ...
  test('…and answered ON the row, not behind a tap', () => {
    // THE TUFTE PASS ALMOST TOOK THIS (2026-08-26). The pass moves standing
    // explanation behind a circled ?, and the first cut swept "that part
    // needs signal" in with it — where the assertion above still passed,
    // because the sentence was still in the file. It was not on the row.
    //
    // Mutation: move either phrase inside an <InfoTap>. This section's whole
    // premise is that signal is the one thing a camper cannot find out by
    // trying, so a caveat one tap away is a caveat discovered by being
    // failed by it — at the gate, with no signal to fix it with.
    const behindATap = [...share.matchAll(/<InfoTap\b[\s\S]*?\/>/g)].map(m => m[0]);
    for (const phrase of ['needs signal', 'No signal, ever']) {
