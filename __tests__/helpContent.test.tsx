...
describe('the limitation set covers what a camper must be told', () => {
  ...
  /**
   * THE CONNECTIVITY MATRIX (owner, 2026-08-26: "i swore i did successfully
   * call the iphone with the p9 via video chat at some point, when they were
   * on the same billwi home wifi network, right? ... let's make sure that's
   * clear in help and tooltips etc"). He had, and Help was underselling its
   * own app in three separate places at once.
   *
   * Three rows, three DIFFERENT answers, and the reason this needs pins is
   * that the old copy blurred them into one paragraph a camper could not act
   * on. Each test below dies on the specific way that sentence used to be
   * wrong — every one of these was a live claim in this file yesterday.
   */
  const matrix = () => everyPhrase().join('\n');

  test('MATRIX · voice: any pair close by, rougher only when nothing carries it', () => {
    // Mutation: restore "Live voice works over Bluetooth too, rougher" as
    // the whole story — a camper with two Android phones and no network is
    // told to expect lo-fi when the app already gives them full quality.
    const all = matrix();
    expect(all).toMatch(/Live voice always works/i);
    expect(all).toMatch(/two Android phones[\s\S]*with every network switched off/i);
  });

  test('MATRIX · video: any pair on any shared Wi-Fi, cross-platform included', () => {
    // Mutation: leave the video card saying only "put both phones on any
    // shared Wi-Fi". True, and it leaves the owner's own measured
    // Pixel-to-iPhone call unstated, so an iPhone owner reads the app as
    // Android-only and never tries.
    const video = HELP_LIMITATIONS.find(l => l.topic === 'video-call-link');
    expect(video!.body).toMatch(/an iPhone and an Android included/i);
    // …and the hotspot escape from "no Wi-Fi anywhere" is named, not merely
    // the limit. A limitation card that only names a wall is half a card.
    expect(video!.body).toMatch(/Camp hotspot/);
  });

  test('MATRIX · the iPhone-to-iPhone correction is stated, not implied', () => {
    // Mutation: say "two newer phones link straight to each other" and let
    // an iPhone pair believe they have the direct link. They do not — the
    // iOS side is inert until its pairing surface exists — so two iPhones
    // with no Wi-Fi silently get lo-fi and no call button, unexplained.
    expect(matrix()).toMatch(
      /iPhones do not yet make the direct link between themselves/i,
    );
  });

  test('MATRIX · messages are the row with no conditions at all', () => {
    // Mutation: hedge mail the way voice and video are hedged. Mail is the
    // one thing that always works between any two phones, and a camper who
    // doubts it stops leaving notes — the app's most reliable feature,
    // talked out of use by its own manual.
    expect(matrix()).toMatch(/Messages and voice notes always get through/i);
  });

  test('MATRIX · the proof card claims only what two phones have done', () => {
    // Mutation, in EITHER direction: leave "the video call ... has not yet
    // run between two devices" standing after the owner measured it (stale
    // in the modest direction), or sweep the whole card away and claim
    // everything is proven (stale in the dangerous one). Voice over
    // Bluetooth is the piece still waiting, and it is named.
    const proof = HELP_LIMITATIONS.find(l => l.topic === 'device-proof');
    expect(proof!.body).toMatch(/video call has carried a real conversation/i);
    expect(proof!.body).toMatch(/voice over Bluetooth/i);
    expect(proof!.body).not.toMatch(/the video call[\s\S]{0,80}not/i);
  });
