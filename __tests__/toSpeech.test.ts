...
describe('eventToSpeech', () => {
  it('speaks "Title, day, time, at location" with & spoken as and', () => {
    expect(eventToSpeech(YOGA)).toBe(
      'Sunrise Yoga, Tuesday, 6:00 AM to 7:30 AM, at 7:30 and G.',
    );
  });

  it('omits the end time when open-ended', () => {
    expect(eventToSpeech({ ...YOGA, time_end: '' })).toBe(
      'Sunrise Yoga, Tuesday, 6:00 AM, at 7:30 and G.',
    );
  });
  ...
  it('handles named locations', () => {
    ...
    ).toBe('Sunrise Yoga, Tuesday, 6:00 AM to 7:30 AM, at Center Camp.');
  });
});
...
describe('speechForAssistantMessage', () => {
  it('speaks prose then each event card', () => {
    expect(
      ...
      'Two yoga sessions tomorrow: Sunrise Yoga, Tuesday, 6:00 AM to 7:30 AM, at 7:30 and G. Dusk Flow, Tuesday, 6:00 PM, at 7:30 and G.',
    );
  });
  ...
  it('adds terminal punctuation to bare prose so TTS pauses before cards', () => {
    expect(
      ...
      'Here is one. Sunrise Yoga, Tuesday, 6:00 AM to 7:30 AM, at 7:30 and G.',
    );
  });
});
