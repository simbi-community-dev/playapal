...
class WalkieModule(private val ctx: ReactApplicationContext) :
  ...
  companion object {
    ...
    /**
     * LIVE-NOT-LATE (field-measured 2026-08-26, three phones on the BLE
     * lo-fi rung): P7 keyed, an iPhone lit "someone is talking" at once and
     * then emitted the AUDIO TEN SECONDS LATE. The transport was fine. The
     * receiver was the bug — when the pipe cannot hold real-time pace, a
     * receiver that schedules EVERY frame it gets simply walks its own
     * playback further and further into the past, and the drift is
     * monotonic: it never catches up on its own, because nothing in a
     * stream of 20 ms frames ever arrives early.
     *
     * So this is the deepest backlog a receiver will hold. A frame that
     * would land more than this far ahead of what has actually PLAYED
     * means the queue is FULL — and a full queue is emptied from the
     * front, so the newest frame is the one that plays and the stale tail
     * is what goes (see makeRoom for which end goes and why). Evicting is
     * the rung being honest about its bandwidth, exactly like the lo-fi
     * badge; playing ten-second-old voice is a lie about liveness, and a
     * walkie that lies about liveness is worse than silence on the playa —
     * you answer a question nobody is still asking.
     *
     * 400 ms is chosen as "a beat behind, not a conversation behind": it
     * absorbs a Wi-Fi retry burst or a 60 ms GATT lane hiccup without
     * ever letting a listener fall out of turn-taking. Mirrored in
     * ios/PlayaPal/Walkie.swift (maxLeadMs) and pinned by a test that
     * reads both files — one number, two languages.
     */
    const val MAX_LEAD_MS = 400

    /** The bound in the unit the track actually counts in. DERIVED, never
     * a second literal: a bound that can drift from its own milliseconds
     * is two bounds. */
     ...
     * readable, and 50 frames a second would drown the buffer it is
     * printed into. */
    const val LATE_LOG_MS = 3_000L
  ...
  /**
   * Frames handed to the CURRENT track since it was built. Against the
   * track's own playback head this is the only queue-depth reading Android
   * offers, and the subtraction is the whole LIVE-NOT-LATE measurement:
   * written minus played = what is still waiting to be heard.
   *
   * Atomic for the same reason seqCounter is — three receive threads reach
   * handleFrame ("walkie-rx", one "walkie-rx-aware" per datapath, and the
   * BLE GATT server callback) and a lost update here would under-count the
   * backlog, which is the failure direction that lets drift back in.
   */
  private val framesWritten = java.util.concurrent.atomic.AtomicInteger(0)

  /** How many times the lateness guard has emptied the track from the
   ...
   * one flush discards up to a whole MAX_LEAD_MS of stale tail, which is
   * a coarser and more honest unit than "frames refused" ever was. */
  private val staleFlushes = java.util.concurrent.atomic.AtomicInteger(0)
  @Volatile private var lastLateLog = 0L
  ...
  /**
   * File one mDNS discovery and remember its listener.
   *
   * A FRESH LISTENER EVERY TIME, deliberately: NsdManager keys its
   * bookkeeping on the listener OBJECT and refuses one that is already in
   * use, so re-filing discovery after a stop means a new object, never the
   * one we just retired. That is the only reason this is a function rather
   * than the block it used to be — refreshDiscovery needs to file a
   * second one.
   */
   ...
   *
   * WHAT IT IS AND IS NOT. It asks every radio to look for podmates right
   * now. It does NOT restart the walkie: the socket, the receive threads,
   * the audio track, the keep-alive and every live peer row survive it
   * untouched, because a camper taps this while the channel is misbehaving
   * and somebody may be mid-sentence on it. Idempotent, safe while
   * talking, and safe when nothing is wrong — each rung fills only its own
   * holes (WalkieBleLink.refresh, WalkieAwareLink.refresh).
   *
   * mDNS is re-filed rather than nudged because there IS no nudge: a
   * DiscoveryListener that has stopped hearing has no API that says "ask
   * again". Re-filing does not cost the channel — peers are removed only
   * by onServiceLost, which a stop does not emit, and a re-resolved peer
   * lands under the same service-name key. The service REGISTRATION is
   * deliberately left alone: it is how everyone else finds us, and
   * re-registering would rename this phone's instance and drop it out of
   * their lists to cure our own.
   *
   ...
   */
  @ReactMethod
  fun refreshDiscovery(promise: Promise) {
    if (socket == null) {
      promise.resolve(null) // the walkie is off; there is nothing to look with
      return
    }
    Log.i(TAG, "walkie//look-again peers=" + peers.size)
    try {
      val manager = nsd
      if (manager != null) {
        try {
          discListener?.let { manager.stopServiceDiscovery(it) }
        } catch (_: Exception) {}
        discListener = null
        startDiscovery(manager)
      }
    } catch (_: Exception) {
      // One rung's refusal is not the control's failure — the others below
      // still looked, which is what the camper asked for.
    }
    try {
      bleLink?.refresh()
    } catch (_: Exception) {}
  ...
  private fun stopInternal() {
    ...
    roster = emptyList()
    lastSeq.clear()
    // Under the track monitor: an RX thread mid-write holds the same lock
    // (writeTrack), so the release can never land under a write in flight.
    synchronized(this) {
      try {
        track?.release()
      } catch (_: Exception) {}
      track = null
      // The backlog died with the track. Left standing, it would read as
      // depth against the NEXT session's playback head, which starts at
      // zero — a stale count that silences a fresh walkie. INSIDE the
      // monitor with the release, for writeTrack's reason (defect E): a
      // reset that lands outside it can be overtaken by an RX thread's
      // accounting and resurrect the depth it just discarded.
      framesWritten.set(0)
    }
  }
  ...
  fun startTalking(promise: Promise) {
    ...
    recordThread = Thread({
      ...
      try {
        ...
        var bleFill = 0
        // One reusable UDP packet for the same reason — at the 9-peer cap
        // a fresh packet per peer per 20 ms frame is 450 allocs/s.
        val udpPkt = DatagramPacket(buf, HEADER)
        while (talking) {
          var got = 0
          ...
          for (p in targets) {
            ...
            udpPkt.setData(buf, 0, HEADER + got)
            udpPkt.address = host
            udpPkt.port = p.port
            try {
              (p.socket ?: s).send(udpPkt)
            } catch (_: Exception) {
  ...
  private fun receiveLoop(s: DatagramSocket) {
    val buf = ByteArray(HEADER + FRAME_BYTES)
    // One packet for the life of the loop — same rule as the tx thread's
    // bleSamples: this thread must not churn the GC at 50 Hz. receive()
    // shrinks length to the datagram's; resetting restores capacity.
  ...
  private fun handleFrame(buf: ByteArray, n: Int, srcSocket: DatagramSocket?, pkt: DatagramPacket?) {
    ...
    // ONE track, and the lateness guard reads its playback head — so it is
    // fetched once here rather than at each write site.
    val t = ensureTrack()
    if ((head and 0x0F) == CODEC_ADPCM8K) {
      ...
      if (pcm8.isNotEmpty()) {
        ...
        // THE FRAME COUNT IS HANDED TO writeTrack, NOT SPENT HERE. Deciding
        // out here — with the monitor open between the decision and the
        // write — is defect A's real face: two receive threads read the
        // same lead and both admit. See writeTrack.
        writeTrack(t, out, 0, out.size, frames)
      }
    } else {
      val frames = (n - HEADER) / 2
      ...
      // Admission, eviction, write and accounting are ONE transaction, and
      // it is inside writeTrack. This caller no longer gets to ask first.
      writeTrack(t, buf, HEADER, n - HEADER, frames)
    }
    // The speaking chip fires either way, and that is deliberate: someone
    // IS talking. Emptying the track is this phone admitting it cannot
  ...
  /**
   * THE LIVE-NOT-LATE GATE (see MAX_LEAD_MS). True means WRITE this frame.
   *
   * AudioTrack offers no queue-depth getter either, so the depth is a
   * subtraction: every frame we hand it is counted, the playback head says
   * how many have actually left the speaker, and the difference is voice
   * still waiting its turn. Add the frame being offered and the answer is
   * exactly the question the policy asks — how far ahead of what has been
   * HEARD would this land?
   *
   * Both sides are 32-bit and wrap together, so the difference stays
   * correct across a wrap (2^31 frames is 37 hours of continuous audio at
   * 16 kHz; the wrap is theory, the arithmetic is free anyway).
   *
   * CALLED ONLY FROM writeTrack, UNDER writeTrack'S MONITOR (defect A's
   * real face, codex→opus cross-family re-read of 09ab350). This used to
   * be a call-site gate, placed before the pre-amp loop so a frame nobody
   * would play cost the receive thread nothing to prepare. That saving was
   * paid for with the one thing this seam cannot spend: the interval
   * between deciding and writing. Three threads reach handleFrame — one
   * "walkie-rx", one "walkie-rx-aware" per datapath, and the BLE GATT
   * callback thread — and with the decision out there, two of them read
   * the SAME lead of 5440, both computed 5440 + 960 <= 6400 and both were
   * admitted; the writes then landed one after the other for a lead of
   * 7360, 460 ms, with no eviction and nothing to notice it. A locked
   * re-measurement does not help, because it only ever ran for the callers
   * the stale outer measurement REJECTED. So the measurement, the
   * eviction, the write and the accounting are now one transaction, and
   * the pre-amp work is done unconditionally — a few hundred multiplies
   * per 20 ms frame is the price of a decision that cannot go stale.
   *
   * WHICH END OF A FULL QUEUE GOES (owner-facing correction, 2026-08-27;
   * codex cross-family read). The first version of this guard refused the
   * frame it was HANDED and kept everything already queued — it dropped
   * the NEWEST audio and preserved the STALE TAIL. Two consequences, both
   * wrong in the same direction:
   *
   *   - a new PTT burst arriving into a full queue loses its START, so
   *     the sentence a camper actually keyed reaches the speaker with its
   *     first words missing, behind audio nobody is waiting for;
   *   - under sustained overload the queue never drains below the bound,
   *     so the channel sits ~400 ms behind FOREVER. The guard bounded the
   *     lateness and then held it there.
   *
   * A live channel has to sound like NOW. So a full queue is emptied from
   * the front — the stale tail is evicted and the newest frame is the one
   * that plays. The bound stops being a floor the channel rests on and
   * goes back to being a ceiling it touches: after an eviction the lead is
   * zero and the conversation is live again, at the cost of the audio
   * nobody could have answered anyway. That is the same trade the sender's
   * drop-on-busy law makes, applied at the other end of the same pipe.
   */
  @Synchronized
  private fun makeRoom(t: AudioTrack, frames: Int): Boolean {
    val lead = framesWritten.get() - t.playbackHeadPosition
    if (lead + frames <= MAX_LEAD_FRAMES) {
      return true
    }
    val flushes = staleFlushes.incrementAndGet()
    val now = System.currentTimeMillis()
    if (now - lastLateLog > LATE_LOG_MS) {
      lastLateLog = now
      Log.i(TAG, "walkie//stale-flush count=" + flushes + " lead=" + lead)
    }
    // No re-measurement follows, and that is the fix rather than a saving:
    // this measurement was taken under the monitor the flush and the write
    // are about to happen under, so there is no interval for it to go
    // stale in. The earlier shape measured twice because the first
    // measurement was unlocked, and a second look that only runs for the
    // callers the FIRST look rejected cannot see the ones it admitted.
    return flushStale(t)
  }

  /**
   * Empty the track from the front, so the frame about to be written is
   * the next thing anybody hears.
   *
   * AudioTrack has no partial eviction: `flush()` discards everything
   * queued and is a NO-OP on a track that is playing, so the sequence is
   * pause -> flush -> play. It also resets the playback head to zero,
   * which is why the written count follows it in the same breath — the
   * subtraction in makeRoom is only honest while both sides count from
   * one origin (the same rule ensureTrack states for a fresh track).
   *
   * UNDER THE WRITE'S OWN MONITOR, for the reason writeTrack states: a
   * flush racing stopInternal's release would be operating on freed native
   * memory, and there is no exception for that — the process just dies. A
   * track that is no longer ours makes no room and the frame is honestly
   * dropped, exactly as writeTrack's -1 already means.
   *
   * NO RE-MEASUREMENT LIVES HERE ANY MORE (defect A, twice). The first
   * version of this routine asked whether the EXISTING lead cleared the
   * ceiling — a different question than the one that got it called, which
   * let a queue sitting at exactly MAX_LEAD_FRAMES admit every arriving
   * packet through the back door and cycle between 400 and 460 ms behind
   * forever, counting an eviction each time and performing none. The
   * repair added the frames in hand to the re-measurement, and a
   * cross-family re-read found that repair still open: a second look is
   * only reached by the callers the FIRST look rejected, so two threads
   * that both cleared an unlocked outer check both wrote, and neither ever
   * arrived here. A re-measurement is not what makes an admission honest —
   * measuring inside the transaction that writes is. So makeRoom measures
   * under this monitor and this routine only evicts.
   */
  @Synchronized
  private fun flushStale(t: AudioTrack): Boolean {
    if (track !== t) {
      return false
    }
    try {
      ...
      // An uninitialised or dying track makes no room; the caller drops
      // the frame, which is what it did before this seam existed.
      return false
    }
  }
  ...
  /**
   * THE WRITE AND THE RELEASE SHARE ONE MONITOR. P7 crash, 2026-08-26
   * 13:20: SIGSEGV in AudioTrack::releaseBuffer under write(), on thread
   * "walkie-rx-aware" — stopInternal had released the track while an RX
   * thread was inside write() on the reference it fetched a moment before.
   * A released track's native buffer is null and the framework does not
   * check; there is no exception to catch, the process just dies. So a
   * write happens only against the CURRENT track, under the same monitor
   * ensureTrack and the release take, and a track that is no longer ours
   * is a frame honestly dropped (-1, which the callers already treat as
   * "took nothing"), never a crash. Holding the monitor across one
   * blocking write delays a release by at most a frame.
   *
   * THE ACCOUNTING RIDES THE SAME MONITOR AS THE WRITE (defect E, codex
   * cross-family read on 0ad0149). While the callers added the frames
   * AFTER writeTrack returned, there was a window one instruction wide
   * with the monitor OPEN in it: write succeeds, the monitor is released,
   * a sibling thread's flushStale (or stopInternal) empties the track and
   * zeroes the count under that monitor — and then the first thread's
   * increment lands and RESURRECTS depth the track no longer holds. That
   * phantom backlog never drains: the playback head is at zero, so the
   * subtraction reads a permanent lead and the guard flushes on every
   * frame from then on, which is a walkie that is silent rather than late.
   * Counting what the track took, in the same breath the track takes it,
   * is what makes framesWritten and the track one fact instead of two.
   *
   * AND THE ADMISSION RIDES IT TOO — measure, evict, write, account, all
   * inside ONE monitor (defect A's real face, codex→opus cross-family
   * re-read of 09ab350). While the depth was checked at the CALL SITE, the
   * check and the write were separated by an open window, and the three
   * threads that reach handleFrame walked straight through it: existing
   * lead 5440, RX1 admits 5440 + 960 = 6400 and pauses before writing, RX2
   * reads the same 5440 and admits too, RX1's write makes 6400 and RX2's
   * makes 7360 — 460 ms behind, no eviction, nothing over the bound at any
   * single instant either thread looked. Hardening the LOCKED re-check
   * could never close it, because that re-check only ran for callers the
   * stale outer check had already rejected. So the only measurement that
   * decides anything is the one taken here, with the write it authorises
   * held under the same lock: no second receiver can observe a pre-write
   * lead, because there is no moment when a lead is measured and the write
   * it admitted has not happened.
   */
  @Synchronized
  private fun writeTrack(
    ...
    if (!makeRoom(t, frames)) {
      return -1
    }
    val wrote = t.write(data, off, len)
    // What the track ACTUALLY took, never what we offered: a short or
    // refused write counted anyway is phantom depth that never drains.
    if (wrote > 0) {
      framesWritten.addAndGet(wrote / 2)
    }
    return wrote
  }
  ...
  private fun ensureTrack(): AudioTrack {
    ...
    t.play()
    // A fresh track has a playback head at zero, so the written count has
    // to start there too or the first frame reads as a huge backlog and
    // the guard drops the whole first breath of a conversation.
    framesWritten.set(0)
    track = t
    return t
  }
