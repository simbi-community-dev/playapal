...
class WalkieModule(private val ctx: ReactApplicationContext) :
  ...
  companion object {
     ...
     * would land more than this far ahead of what has actually PLAYED is
     * dropped on the floor and counted, never queued. Dropping is the rung
     * being honest about its bandwidth, exactly like the lo-fi badge;
     * playing ten-second-old voice is a lie about liveness, and a walkie
     * that lies about liveness is worse than silence on the playa — you
   ...
   * is broken. */
  private val lateDrops = java.util.concurrent.atomic.AtomicInteger(0)
  ...
  private fun stopInternal() {
    ...
    // zero — a stale count that silences a fresh walkie.
    framesWritten.set(0)
  }
    ...
    if ((head and 0x0F) == CODEC_ADPCM8K) {
      ...
      if (pcm8.isNotEmpty()) {
        ...
        if (!tooLate(t, frames)) {
          ...
          // Count what the track ACTUALLY took, never what we offered: a
          // short or refused write that was counted anyway leaves phantom
          // depth behind forever, and phantom depth silences a walkie that
          // is perfectly on time — the same bug wearing the opposite face.
          val wrote = t.write(out, 0, out.size)
          if (wrote > 0) {
      ...
      if (!tooLate(t, frames)) {
        ...
        val wrote = t.write(buf, HEADER, n - HEADER)
        if (wrote > 0) {
          framesWritten.addAndGet(wrote / 2)
        }
      }
  ...
  /**
   * THE LIVE-NOT-LATE GATE (see MAX_LEAD_MS). True means DROP this frame.
   *
   ...
   *
   * Deliberately placed BEFORE the pre-amp loop and before the upsample:
   * a frame we are not going to play should not cost the receive thread
   * the work of preparing it.
   */
  private fun tooLate(t: AudioTrack, frames: Int): Boolean {
    val lead = framesWritten.get() - t.playbackHeadPosition
    if (lead + frames <= MAX_LEAD_FRAMES) {
      return false
    }
    val dropped = lateDrops.incrementAndGet()
    val now = System.currentTimeMillis()
    if (now - lastLateLog > LATE_LOG_MS) {
      lastLateLog = now
      Log.i(TAG, "walkie//late-drop count=" + dropped + " lead=" + lead)
    }
    return true
  }
