...
class WalkieAwareLink(
  ...
  private data class AwarePeer(
    ...
    /** When the return path was last stamped by a real inbound frame —
     * the rotation guard's proof that the responder socket is alive. */
    @Volatile var returnStampMs: Long = 0L,
  ...
  /**
   * "LOOK AGAIN" for the own-link rung (WalkieModule.refreshDiscovery).
   *
   * THE SAFE HALF ONLY, and the line is drawn at what is already working.
   * A live datapath is a network the framework built and a socket carrying
   * voice; nothing here closes one, and nothing here closes a discovery
   * session that is up — re-filing a healthy subscribe would throw away
   * the peer handles every pending re-dial is built from, in order to look
   * for peers we have already found.
   *
   * What it does is fill HOLES, each with the same call the recovery arcs
   * already use: re-attach when the session is gone (an Aware flap can go
   * quiet with no callback at all — docs/WALKIE-LADDER.md §5b measured
   * that), re-publish or re-subscribe when one of those died alone, and
   * re-file the datapath request for any peer we can see but never
   * connected. maybeRequestDatapath is guarded on `requested`/`up`, so
   * peers that ARE connected are skipped by the very test that keeps the
   * ordinary path from double-filing.
   */
  fun refresh() {
    handler.post {
      if (stopped) {
      ...
      try {
        ...
        Log.i(TAG, "aware//look-again peers=" + discovered.size)
      } catch (_: Exception) {
        // Same fencing law as everywhere else in this file: the rung
        // contributes what it already contributed, and the rungs above it
        // are untouched.
      }
    }
  }
    ...
    try {
      ...
      mgr.attach(object : AttachCallback() {
        override fun onAttached(session: WifiAwareSession) {
          ...
          // Whether THIS phone could ever take the cross-platform bridge
          // (docs/WALKIE-LADDER.md §9a). Aware Pairing — NAN v4.0's PIN
          // ceremony, API 34+ — is the designed route to an iPhone, and it
          // is device-dependent, not Android-wide: community-confirmed on
          // a Galaxy S25, reported not working on a Pixel 9. Our Pixels
          // have never been asked. Apple's half is broken regardless, so
          // this answers the OTHER half: a `false` here means no Apple fix
          // could ever reach these phones, and that is worth knowing
          // before anyone budgets the pairing work. One log line, once per
          // attach, on the path that already proves the radio is alive.
          val pairing = if (Build.VERSION.SDK_INT >= 34) {
            // "unknown" and not "false" when Characteristics is null: an
            // unanswered question must never read as a measured no.
            mgr.characteristics?.isAwarePairingSupported?.toString() ?: "unknown"
          } else {
            "unknown"
          }
          Log.i(TAG, "aware//pairing-supported=$pairing")
          publish(session)
          subscribe(session)
        }
  ...
  private fun maybeRotateResponder() {
    ...
    // A FRESH return-path stamp is inbound PROOF the responder socket is
    // alive. getNetworkCapabilities can transiently answer null for a
    // network that is in fact carrying frames (codex 2026-08-26: the
    // instrument can lie), and rotating on that lie would break every
    // healthy responder path at once — the exact §1 violation the
    // rotation exists to cure. 15 s = one probe cadence past STALE_MS.
    val freshestStamp = discovered.values
      .filter { senderHash < it.hash }
      .maxOfOrNull { it.returnStampMs } ?: 0L
    if (now - freshestStamp < 15_000L) {
      return
    }
  ...
  fun noteReturnPath(hash: Long, host: java.net.InetAddress, port: Int, socket: DatagramSocket) {
    // Synchronized (codex 2026-08-26): frames arrive on more than one
    // receive thread, and the endpoint triple below must re-stamp
    // atomically or a port flap can tear it and storm onPeer.
    if (stopped || port <= 0) {
      return
    }
    ...
    peer.returnStampMs = System.currentTimeMillis()
  ...
  @Synchronized
  fun stop() {
    // Synchronized against ensureResponderRequest/maybeRotateResponder
    // (codex 2026-08-26): an unsynchronized stop could interleave with a
    // request being filed and leak the freshly registered callback past
    // the teardown loop — the near-100 request cap then kills the rung
